...
The Teams meeting pipeline reads meeting transcripts, recordings, and related artifacts from Microsoft Graph using **app-only** (daemon) autenticación — no user sign-in, no interactive consent per meeting. That requires an Azure AD application registration with admin-consented application permissions.
...
python -c "
import asyncio
from herramientas.microsoft_graph_auth import MicrosoftGraphTokenProveedor
proveedor = MicrosoftGraphTokenProveedor.from_env()
token = asyncio.run(proveedor.get_access_token())
print('Token acquired, length:', len(token))
print(proveedor.inspect_token_health())
"
```
...
3. Restart the puerta de enlace so the new secret is picked up: `hermes puerta de enlace restart`.
...
- **Webhook listener setup** — stand up the `msgraph_webhook` puerta de enlace platform that receives Graph change notifications.
- **Pipeline configuración** — configure the Teams meeting pipeline runtime and operator CLI.
