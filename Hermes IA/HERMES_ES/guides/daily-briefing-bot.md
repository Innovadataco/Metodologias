...
2. **Hermes spins up** a fresh agente session with your prompt
...
- **Puerta de enlace running** — the puerta de enlace daemon handles cron execution:
  ```bash
  hermes puerta de enlace install   # Install as a user service
  sudo hermes puerta de enlace install --system   # Linux servers: boot-time system service
  # or
  hermes puerta de enlace           # Run in foreground
  ```
- **Firecrawl API key** — set `FIRECRAWL_API_KEY` in your environment for web search
- **Messaging configured** (optional but recommended) — [Telegram](/guia-usuario/messaging/telegram) or Discord set up with a home channel
...
```
Search for the latest news about AI agentes and open source LLMs.
...
1. Qwen 3 Released with 235B Parameters
   Alibaba's latest open-weight modelo matches GPT-4.5 on several
   benchmarks while remaining fully open source.
   → https://qwenlm.github.io/blog/qwen3/

2. LangChain Launches Agente Protocol Standard
   A new open standard for agente-to-agente communication gains
   adoption from 15 major frameworks in its first week.
   → https://blog.langchain.dev/agente-protocol/

3. EU AI Act Enforcement Begins for General-Purpose Modelos
   The first compliance deadlines hit, with open source modelos
   receiving exemptions under the 10M parameter threshold.
   → https://artificialintelligenceact.eu/updates/

---
3 stories • Sources searched: 8 • Generated by Hermes Agente
```
...
Before creating cron jobs, ensure Hermes has a default modelo and proveedor configured globally. If you want a specific job to use different values, set explicit per-job modelo/proveedor overrides when creating it.
...
```
Every morning at 8am, search the web for the latest news about AI agentes
...
Hermes will create the cron job for you using the unified `cronjob` herramienta.
...
**Good prompt:**
```
Search the web for the latest news about AI agentes and open source LLMs.
...
The good prompt is specific about **what to search**, **how many articles**, **what format**, and **what tone**. It's everything the agente needs in one shot.
...
Topics:
1. AI and machine learning — focus on open source modelos and agente frameworks
...
For faster briefings, tell Hermes to delegate each topic to a sub-agente:

```
/cron add "0 8 * * *" "Create a morning briefing by delegating research to sub-agentes. Delegate three parallel tasks:
...
Each sub-agente searches independently and in parallel, then the main agente combines everything into one polished briefing. See the [Delegation docs](/guia-usuario/features/delegation) for more on how this works.
...
### Adding Personal Contexto with Memoria

If you have [memoria](/guia-usuario/features/memoria) enabled, you can store preferences that persist across sessions. But remember — cron jobs run in fresh sessions without conversational memoria. To add personal contexto, bake it directly into the prompt:

```
/cron add "0 8 * * *" "You are creating a briefing for a senior ML engineer who cares about: PyTorch ecosystem, transformer architectures, open-weight modelos, and AI regulation in the EU. Skip stories about product launches or funding rounds unless they involve open source.
...
Including details about who the briefing is *for* dramatically improves relevance. Tell the agente your role, interests, and what to skip.
:::
...
If the puerta de enlace isn't running, your jobs won't execute. Install it as a background service for reliability:
...
- **[Scheduled Tasks (Cron)](/guia-usuario/features/cron)** — Full reference for schedule formats, repeat limits, and delivery options
- **[Delegation](/guia-usuario/features/delegation)** — Deep dive into parallel sub-agente workflows
- **[Messaging Platforms](/guia-usuario/messaging)** — Set up Telegram, Discord, or other delivery targets
- **[Memoria](/guia-usuario/features/memoria)** — Persistent contexto across sessions
