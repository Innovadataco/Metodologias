<!-- source: website/docs/guides/automation-blueprints.md -->
# Automation Blueprints

# Automation Blueprints

Copy-paste blueprints for common automation patterns. Each blueprint uses Hermes's built-in [cron scheduler](/guia-usuario/features/cron) for time-based triggers and [webhook platform](/guia-usuario/messaging/webhooks) for event-driven triggers.

Every blueprint works with **any modelo** — not locked to a single proveedor.
...
| Trigger | How | Herramienta |
|---------|-----|------|
| **Schedule** | Runs on a cadence (hourly, nightly, weekly) | `cronjob` herramienta or `/cron` slash command |
...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/hermes-agente GitHub repo.

1. Run: gh issue list --repo NousResearch/hermes-agente --state open --json number,title,labels,author,createdAt --limit 30
...
- Performance concerns (N+1 queries, unbounded loops, memoria leaks)
...
hermes cron create "0 9 * * 1" \
  "Scan the NousResearch/hermes-agente repo for documentation drift.

1. Run: gh pr list --repo NousResearch/hermes-agente --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Herramienta schemas (herramientas/*.py) — may need docs/reference/herramientas-reference.md update
   - CLI commands (hermes_cli/commands.py, hermes_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (hermes_cli/config.py) — may need docs/guia-usuario/configuración.md update
...
hermes cron create "0 6 * * *" \
  "Run a dependency security audit on the hermes-agente project.

1. cd ~/.hermes/hermes-agente && source .venv/bin/activate
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agente": "Hermes-Monitor/1.0"})
...
hermes cron create "0 8 * * *" \
  "Scout these AI agente repositories for notable activity in the last 24 hours:
...
If there are findings, organize by repo with brief analysis of each item." \
  --habilidad competitive-pr-scout \
...
1. Search the web for major AI announcements, modelo releases, and research breakthroughs
2. Search for trending ML repositories on GitHub
3. Check arXiv for highly-cited papers on language modelos and agentes
...
```bash
hermes cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language modelo reasoning' OR 'herramienta-use agentes' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Hermes Agente development." \
  --habilidad arxiv --habilidad obsidian \
  --name "Paper digest" \
  --deliver local
```
...
## Multi-Habilidad Workflows

### Security Audit Pipeline

Combine multiple habilidads for a comprehensive weekly security review.

**Trigger:** Schedule (weekly)

```bash
hermes cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the hermes-agente codebase.
...
If nothing found, report a clean bill of health." \
  --habilidad codebase-security-audit \
...
hermes cron create "0 10 * * 3" \
  "Research and draft a technical blog post outline about a trending topic in AI agentes.

1. Search the web for the most discussed AI agente topics this week
2. Pick the most interesting one that's relevant to open-source AI agentes
...
This means you only get notified when the agente has something to report.
