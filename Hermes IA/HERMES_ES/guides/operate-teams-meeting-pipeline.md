...
Use this guide after you have already enabled the feature from [Teams Meetings](/guia-usuario/messaging/teams-meetings).
...
#### Option 1: Hermes cron (recommended if you already run the Hermes puerta de enlace)

Hermes ships a built-in cron scheduler. The `--no-agente` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:
...
- [Teams Meetings setup](/guia-usuario/messaging/teams-meetings)
