...
This guide shows how to actually use MCP with Hermes Agente in day-to-day workflows.
...
Use MCP when:
- a herramienta already exists in MCP form and you do not want to build a native Hermes herramienta
...
Do not use MCP when:
- a built-in Hermes herramienta already solves the job well
- the server exposes a huge dangerous herramienta surface and you are not prepared to filter it
- you only need one very narrow integration and a native herramienta would be simpler and safer

## Mental modelo

Think of MCP as an adapter layer:

- Hermes remains the agente
- MCP servers contribute herramientas
- Hermes discovers those herramientas at startup or reload time
- the modelo can use them like normal herramientas
...
- ask Hermes what herramientas it has available
...
```text
Tell me which MCP-backed herramientas are available right now.
```

## Step 4: start filtering immediately

Do not wait until later if the server exposes a lot of herramientas.
...
- you keep your real Windows browser perfil, cookies, and logins
- Hermes stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP herramientas instead of relying on Hermes core browser transport

### Recommended server

Use `chrome-devherramientas-mcp`.
...
```bash
hermes mcp add chrome-devherramientas-win --command cmd.exe --args /c npx -y chrome-devherramientas-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
hermes mcp test chrome-devherramientas-win
```
...
Once loaded, Hermes can use the MCP-prefixed browser herramientas directly. For example:

```text
调用 MCP 工具 mcp_chrome_devherramientas_win_list_pages，列出当前浏览器标签页。
```

### When `/browser connect` is the wrong herramienta

If Hermes runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.

Common reasons:

- WSL cannot reach the same host-local endpoint Chrome exposes to Windows herramientas
- newer Chrome live-debugging flows are not the same as a classic `ws://localhost:9222`
- the browser is easier to attach to from a Windows-side helper like `chrome-devherramientas-mcp`
...
- If `chrome-devherramientas-mcp --autoConnect` times out while enumerating pages, reduce background/frozen tabs in Chrome and retry.
...
1. Server-native MCP herramientas
- filtered with:
  - `herramientas.include`
  - `herramientas.exclude`

2. Hermes-added utility wrappers
- filtered with:
  - `herramientas.resources`
...
mcp_servers:
  fs:
    command: "npx"
    args: ["-y", "@modelocontextoprotocol/server-filesystem", "/home/user/project"]
...
```text
Review the project structure and identify where configuración lives.
```
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Hermes to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Hermes remains the agente; Open Scaffold remains the repo-local record.
...
```text
Use the Open Scaffold MCP herramientas to compile the current handoff packet and tell me the next legal action.
```
...
- Its write herramientas require the server to be started with `--allow-write`; do not enable that until you explicitly want Hermes to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Hermes to merge, publish, deploy, or spawn runtimes.
- Pin `open-scaffold@<version>` instead of `@latest` if you need reproducible herramienta schemas.
...
```text
Search the repo for uses of _discover_and_register_server and explain how MCP herramientas are registered.
```
...
- use `herramientas.include`
- start with the smallest set possible

### Disable unused utilities

If you do not want the modelo browsing server-provided resources/prompts, turn them off:
...
- internal API server with read-heavy herramienta exposure by default
...
### "The server connects but the herramientas I expected are missing"

Possible causes:
- filtered by `herramientas.include`
- excluded by `herramientas.exclude`
...
### "Why do I see fewer herramientas than the MCP server advertises?"
...
- [MCP (Modelo Contexto Protocol)](/guia-usuario/features/mcp)
- [FAQ](/reference/faq)
