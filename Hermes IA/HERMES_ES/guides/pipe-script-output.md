<!-- source: website/docs/guides/pipe-script-output.md -->
...
puerta de enlace, you don't need an LLM, and you don't need to re-paste bot tokens
into each of your scripts.

Use it for:

- System monitoring (memoria, disk, GPU temp, long-running job finished)
...
- Piping any herramienta's output anywhere (`make | hermes send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `hermes
puerta de enlace` already uses, so there's no second configuración surface to
maintain.
...
### Monitoring: Memoria / Disk Alerts
...
:::tip Don't alert the puerta de enlace about itself
For watchdogs that might fire when the puerta de enlace itself is struggling (OOM
...
## Does `hermes send` Need the Puerta de enlace Running?
...
A live puerta de enlace is only required for **complemento platforms** that rely on a
persistent adapter connection (for example, a custom complemento that keeps
a long-lived WebSocket open). In that case you'll get a clear error
pointing at the puerta de enlace; start it with `hermes puerta de enlace start` and retry.
...
puerta de enlace refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the puerta de enlace once (`hermes puerta de enlace
...
| Approach | Multi-platform | Reuses Hermes creds | Needs puerta de enlace | Best for |
|----------|----------------|---------------------|---------------|----------|
| `hermes send` | ✅ | ✅ | No (bot-token) | Everything below |
| Raw `curl` to each platform | Each scripted separately | Manual | No | Critical watchdogs |
| `cron` job with `--deliver` | ✅ | ✅ | No | Scheduled agente tasks |
| `send_message` agente herramienta | ✅ | ✅ | No | Inside an agente loop |

`hermes send` is intentionally the simplest possible surface. If you need
an agente to decide what to say, use the `send_message` herramienta from within a
...
- [Puerta de enlace Internals](/guia-desarrollador/puerta de enlace-internals) —
  the delivery router that `hermes send` shares with cron delivery.
- [Messaging Platform Setup](/guia-usuario/messaging/) —
  one-time configuración for each platform.
