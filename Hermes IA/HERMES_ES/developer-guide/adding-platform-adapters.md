<!-- source: website/docs/guia-desarrollador/adding-platform-adapters.md -->
# guia-desarrollador/adding-platform-adapters

# Adding a Platform Adapter

This guide covers adding a new messaging platform to the Hermes puerta de enlace. A platform adapter connects Hermes to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agente through that service.

:::tip
There are two ways to add a platform:
- **Complemento** (recommended for community/third-party): Drop a complemento directory into `~/.hermes/complementos/` — zero core code changes needed. See [Complemento Path](#complemento-path-recommended) below.
...
```
User ↔ Messaging Platform ↔ Platform Adapter ↔ Puerta de enlace Runner ↔ AIAgente
```

Every adapter extends `BasePlatformAdapter` from `puerta de enlace/platforms/base.py` and implements:
...
Inbound messages are received by the adapter and forwarded via `self.handle_message(event)`, which the base class routes to the puerta de enlace runner.

## Complemento Path (Recommended)

The complemento system lets you add a platform adapter without modifying any core Hermes code. Your complemento is a directory with two files:

```
~/.hermes/complementos/my-platform/
  complemento.yaml      # Complemento metadata
  adapter.py       # Adapter class + register() entry point
```

### complemento.yaml

Complemento metadata. The `requires_env` and `optional_env` blocks auto-populate `hermes config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
```python
import os
from puerta de enlace.platforms.base import (
    BasePlatformAdapter, SendResult, MessageEvent, MessageType,
)
from puerta de enlace.config import Platform, PlatformConfig
...
def register(ctx):
    """Complemento entry point — called by the Hermes complemento system."""
    ctx.register_platform(
        ...
        # Env-driven auto-configuración — seeds PlatformConfig.extra from
        # env vars before adapter construction. See "Env-Driven Auto-
        # Configuración" section below.
    ...
    # Optional: register platform-specific herramientas
    ctx.register_herramienta(
        name="my_platform_search",
        herramientaset="my_platform",
        schema={...},
...
### What the Complemento System Handles Automatically
...
|---|---|
| Puerta de enlace adapter creation | Registry checked before built-in if/elif chain |
...
| `hermes config` UI entries | `requires_env` / `optional_env` in `complemento.yaml` auto-populate |
| send_message herramienta | Routes through live puerta de enlace adapter |
| Webhook cross-platform delivery | Registry checked for known platforms |
| `/update` command access | `allow_update_command` flag |
| Channel directory | Complemento platforms included in enumeration |
| System prompt hints | `platform_hint` injected into LLM contexto |
| Message chunking | `max_message_length` for smart splitting |
| PII redaction | `pii_safe` flag |
| `hermes status` | Shows complemento platforms with `(complemento)` tag |
| `hermes puerta de enlace setup` | Complemento platforms appear in setup menu |
| `hermes herramientas` / `hermes habilidads` | Complemento platforms in per-platform config |
| Token lock (multi-perfil) | Use `acquire_scoped_lock()` in your `connect()` |
| Orphaned config warning | Descriptive log when complemento is missing |

## Env-Driven Auto-Configuración

Most users set up a platform by dropping env vars into `~/.hermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your complemento pick those env vars up **before** the adapter is constructed, so `hermes puerta de enlace status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.

```python
def _env_enablement() -> dict | None:
    """Seed PlatformConfig.extra from env vars.

    Called by the platform registry during load_puerta de enlace_config().
...
Some users prefer setting `config.yaml` keys (`my_platform.require_mention`, `my_platform.allowed_channels`, etc.) over env vars. The `apply_yaml_config_fn` hook lets your complemento own this translation instead of forcing core `puerta de enlace/config.py` to know your platform's YAML schema.
...
The hook is invoked during `load_puerta de enlace_config()` after the generic shared-key loop (which handles common keys like `unauthorized_dm_behavior`, `notice_delivery`, `reply_prefix`, `require_mention`, etc.) and before `_apply_env_overrides()`, so your complemento only needs to bridge **platform-specific** keys.

Exceptions raised by the hook are swallowed and logged at debug level — a misbehaving complemento never aborts puerta de enlace config load.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the puerta de enlace (i.e., `hermes cron run` separate from `hermes puerta de enlace`), register a `standalone_sender_fn`:
...
Why this hook is necessary: built-in platforms (Telegram, Discord, Slack, etc.) ship direct REST helpers in `herramientas/send_message_herramienta.py` so cron can deliver without holding the puerta de enlace in the same process. Complemento platforms historically depended on `_puerta de enlace_runner_ref()`, which returns `None` outside the puerta de enlace process, so without `standalone_sender_fn` the cron-side send fails with `No live adapter for platform '<name>'`.

The function receives the same `pconfig` and `chat_id` that the live adapter would, plus optional `thread_id`, `media_files`, and `force_document` keyword arguments. Returning `{"success": True, "message_id": ...}` is treated as a successful delivery; returning `{"error": "..."}` surfaces the message in cron's `delivery_errors`. Exceptions raised inside the function are caught by the dispatcher and reported as `Complemento standalone send failed: <reason>`. Reference implementations live in `complementos/platforms/{irc,teams,google_chat}/adapter.py`.

## Surfacing Env Vars in `hermes config`

`hermes_cli/config.py` scans `complementos/platforms/*/complemento.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.

```yaml
# complementos/platforms/my_platform/complemento.yaml
name: my_platform-platform
label: My Platform
kind: platform
version: 1.0.0
description: >
  My Platform puerta de enlace adapter for Hermes Agente.
author: Your Name
requires_env:
...
Bare-string entries (`- MY_PLATFORM_TOKEN`) still work — they get a generic description auto-derived from the complemento's `label`. If a hardcoded entry for the same var already exists in `OPTIONAL_ENV_VARS`, it wins (back-compat); the complemento.yaml form acts as the fallback.
...
These are real constraints the base `BasePlatformAdapter` can't anticipate. The complemento surface intentionally leaves the room for an adapter to layer platform-specific UX on top of the base typing loop without expanding the kwarg list.
...
- **Tear down the side task in `finally`.** When the LLM finishes (or `/stop` cancels the run), the puerta de enlace cancels the typing task. Your side task must observe that cancellation too, otherwise it lingers and may fire after the response was already delivered.
...
2. **System busy-ack** (`⚡ Interrupting`, `⏳ Queued`, `⏩ Steered`) → bypass the cache and send visibly so the user sees the puerta de enlace's response to their input.
...
`_SYSTEM_BYPASS_PREFIXES` are the puerta de enlace's own busy-acknowledgment prefixes (`⚡`, `⏳`, `⏩`, `💾`). Always let those through visibly, regardless of cached UX state.
...
See `complementos/platforms/line/adapter.py` for the full LINE postback implementation — a `RequestCache` state machine (`PENDING → READY → DELIVERED`, plus `ERROR` for `/stop`), a `_keep_typing` override that fires the Template Buttons bubble at threshold, a `send` override that routes through the cache, and an `interrupt_session_activity` override that resolves orphan PENDING entries.

### Referencia Implementations (Complemento Path)

See `complementos/platforms/irc/` in the repo for a complete working example — a full async IRC adapter with zero external dependencies. `complementos/platforms/teams/` covers Bot Framework / Adaptive Cards, `complementos/platforms/google_chat/` covers OAuth-based REST APIs, and `complementos/platforms/line/` covers webhook-driven Messaging APIs with platform-specific slow-LLM UX.
...
:::note
This checklist is for adding a platform directly to the Hermes core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Complemento Path](#complemento-path-recommended) above.
:::

### 1. Platform Enum

Add your platform to the `Platform` enum in `puerta de enlace/config.py`:
...
Create `complementos/platforms/newplat/adapter.py`:
...
### 3. Puerta de enlace Config (`puerta de enlace/config.py`)

Three touchpoints:

1. **`get_connected_platforms()`** — Add a check for your platform's required credentials
2. **`load_puerta de enlace_config()`** — Add token env map entry: `Platform.NEWPLAT: "NEWPLAT_TOKEN"`
3. **`_apply_env_overrides()`** — Map all `NEWPLAT_*` env vars to config

### 4. Puerta de enlace Runner (`puerta de enlace/run.py`)
...
1. **`puerta de enlace/platforms/webhook.py`** — Add `"newplat"` to the delivery type tuple
...
2. **`hermes_cli/puerta de enlace.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`hermes_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_herramientaset (used by `habilidads_config` and `herramientas_config` TUIs)
4. **`hermes_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `puerta de enlace.py`) and add tuple to the messaging platforms list
...
1. **`herramientas/send_message_herramienta.py`** — Add `"newplat": Platform.NEWPLAT` to platform map
2. **`herramientas/cronjob_herramientas.py`** — Add `newplat` to the delivery target description string

### 8. Herramientasets

1. **`herramientasets.py`** — Add `"hermes-newplat"` herramientaset definition with `_HERMES_CORE_TOOLS`
2. **`herramientasets.py`** — Add `"hermes-newplat"` to the `"hermes-puerta de enlace"` includes list

### 9. Optional: Platform Hints

**`agente/prompt_builder.py`** — If your platform has specific rendering limitations (no markdown, message length limits, etc.), add an entry to the `_PLATFORM_HINTS` dict. This injects platform-specific guidance into the system prompt:
...
Not all platforms need hints — only add one if the agente's behavior should differ.

### 10. Tests

Create `tests/puerta de enlace/test_newplat.py` covering:
...
| File | What to add |
|------|-------------|
| `website/docs/guia-usuario/messaging/newplat.md` | Full platform setup page |
| `website/docs/guia-usuario/messaging/index.md` | Platform comparison table, architecture diagram, herramientasets table, security section, next-steps link |
| `website/docs/reference/environment-variables.md` | All NEWPLAT_* env vars |
| `website/docs/reference/herramientasets-reference.md` | hermes-newplat herramientaset |
| `website/docs/integrations/index.md` | Platform link |
| `website/sidebars.ts` | Sidebar entry for the docs page |
| `website/docs/guia-desarrollador/architecture.md` | Adapter count + listing |
| `website/docs/guia-desarrollador/puerta de enlace-internals.md` | Adapter file listing |
...
For platforms with tight response deadlines (e.g., WeCom's 5-second limit), always acknowledge immediately and deliver the agente's reply proactively via API later. Agente sessions run 3–30 minutes — inline replies within a callback response window are not feasible.

### Token Locks

If the adapter holds a persistent connection with a unique credential, add a scoped lock to prevent two perfils from using the same credential:

```python
from puerta de enlace.status import acquire_scoped_lock, release_scoped_lock

async def connect(self):
    if not acquire_scoped_lock("newplat", self._token):
        logger.error("Token already in use by another perfil")
        return False
    # ... connect
...
| `complementos/platforms/irc/adapter.py` | Long-poll + IRC protocol | High | Full-featured complemento adapter with scoped token lock |
