<!-- source: website/docs/guia-desarrollador/trajectory-format.md -->
# guia-desarrollador/trajectory-format.md

# Trajectory Format

Hermes Agente saves conversation trajectories in ShareGPT-compatible JSONL format
for use as training data, debugging artifacts, and reinforcement learning datasets.

Source files: `agente/trajectory.py`, `run_agente.py` (search for `_save_trajectory`), `batch_runner.py`
...
```json
{
  ...
  "partial": false,
  "api_calls": 7,
  "herramientasets_used": ["code_herramientas", "file_herramientas"],
  "herramienta_stats": {
...
The `herramienta_stats` and `herramienta_error_counts` dictionaries are normalized to include
ALL possible herramientas (from `modelo_herramientas.TOOL_TO_TOOLSET_MAP`) with zero defaults,
...
```json
{
  "conversations": [
    {
      "from": "system",
      "value": "You are a function calling AI modelo. You are provided with function signatures within <herramientas> </herramientas> XML tags. You may call one or more functions to assist with the user query. If available herramientas are not relevant in assisting with user query, just respond in natural conversational language. Don't make assumptions about what values to plug into functions. After calling & executing the functions, you will be provided with function results within <herramienta_response> </herramienta_response> XML tags. Here are the available herramientas:\n<herramientas>\n[{\"name\": \"terminal\", \"description\": \"Execute shell commands\", \"parameters\": {\"type\": \"object\", \"properties\": {\"command\": {\"type\": \"string\"}}}, \"required\": null}]\n</herramientas>\nFor each function call return a JSON object, with the following pydantic modelo json schema for each:\n{'title': 'FunctionCall', 'type': 'object', 'properties': {'name': {'title': 'Name', 'type': 'string'}, 'arguments': {'title': 'Arguments', 'type': 'object'}}, 'required': ['name', 'arguments']}\nEach function call should be enclosed within <herramienta_call> </herramienta_call> XML tags.\nExample:\n<herramienta_call>\n{'name': <function-name>,'arguments': <args-dict>}\n</herramienta_call>"
    },
    {
      "from": "human",
    ...
    {
      "from": "herramienta",
      "value": "<herramienta_response>\n{\"herramienta_call_id\": \"call_abc123\", \"name\": \"terminal\", \"content\": \"Python 3.11.6\"}\n</herramienta_response>"
    },
    {
      "from": "gpt",
...
of how the modelo originally produced it:

1. **Native thinking tokens** (`msg["reasoning"]` field from proveedors like
   Anthropic, OpenAI o-series): Wrapped as `<think>\n{reasoning}\n</think>\n`
   and prepended before the content.

2. **REASONING_SCRATCHPAD XML** (when native thinking is disabled and the modelo
...
### Herramienta Call Normalization

Herramienta calls from the API format (with `herramienta_call_id`, function name, arguments as
...
- Multiple herramienta calls in one assistant turn produce multiple `<herramienta_call>` blocks
  in a single `gpt` message

### Herramienta Response Normalization

All herramienta results following an assistant message are grouped into a single `herramienta`
turn with XML-wrapped JSON responses:

```
<herramienta_response>
{"herramienta_call_id": "call_abc123", "name": "terminal", "content": "output here"}
</herramienta_response>
```

- If herramienta content looks like JSON (starts with `{` or `[`), it's parsed so the
  content field contains a JSON object/array rather than a string
- Multiple herramienta results are joined with newlines in one message
- The herramienta name is matched by position against the parent assistant's `herramienta_calls`
  array
...
- `<herramientas>` XML block containing the JSON herramienta definitions
- Schema reference for `FunctionCall` objects
- `<herramienta_call>` example

Herramienta definitions include `name`, `description`, `parameters`, and `required`
...
The normalized `herramienta_stats` schema ensures all entries have the same columns,
...
Or via the `--save-trajectories` flag. When the agente initializes with
