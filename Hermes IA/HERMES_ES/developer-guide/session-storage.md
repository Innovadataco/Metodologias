<!-- source: website/docs/guia-desarrollador/session-storage.md -->
# guia-desarrollador/session-storage

# Session Storage

Hermes Agente uses a SQLite database (`~/.hermes/state.db`) to persist session
metadata, full message history, and modelo configuración across CLI and puerta de enlace
...
├── messages_fts          — FTS5 virtual table (content + herramienta_name + herramienta_calls)
...
- **WAL mode** for concurrent readers + one writer (puerta de enlace multi-platform)
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    herramienta_call_count INTEGER DEFAULT 0,
    input_tokens INTEGER DEFAULT 0,
    output_tokens INTEGER DEFAULT 0,
    cache_read_tokens INTEGER DEFAULT 0,
    cache_write_tokens INTEGER DEFAULT 0,
    reasoning_tokens INTEGER DEFAULT 0,
    billing_proveedor TEXT,
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    role TEXT NOT NULL,
    content TEXT,
    herramienta_call_id TEXT,
    herramienta_calls TEXT,
    herramienta_name TEXT,
...
Notes:
- `herramienta_calls` is stored as a JSON string (serialized list of herramienta call objects)
- `reasoning_details`, `codex_reasoning_items`, and `codex_message_items` are stored as JSON strings
- `reasoning` stores the raw reasoning text for proveedors that expose it
...
| 5 | Add billing columns: `cache_read_tokens`, `cache_write_tokens`, `reasoning_tokens`, `billing_proveedor`, `billing_base_url`, `billing_mode`, `estimated_cost_usd`, `actual_cost_usd`, `cost_status`, `cost_source`, `pricing_version` |
...
| 11 | Re-index `messages_fts` and `messages_fts_trigram` to cover `herramienta_name` + `herramienta_calls` and switch from external-content to inline mode; drop old triggers and backfill every message row |
...
Multiple hermes processes (puerta de enlace + CLI sessions + worktree agentes) share one
...
msg_id = db.append_message(
    ...
    herramienta_calls=[{"id": "call_1", "function": {"name": "terminal", "arguments": "{}"}}],
    token_count=150,
    finish_reason="stop",
...
# Exclude puerta de enlace sessions
...
- `contexto` — 1 message before and after the match (content truncated to 200 chars)
- `source`, `modelo`, `session_started` — from the parent session
...
Sessions can form chains via `parent_session_id`. This happens when contexto
compression triggers a session split in the puerta de enlace.
...
SELECT id, title, modelo, input_tokens + output_tokens AS total_tokens,
       estimated_cost_usd
FROM sessions
...
The database file, WAL file (`state.db-wal`), and shared-memoria file
