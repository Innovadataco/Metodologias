<!-- source: website/docs/guia-desarrollador/browser-supervisor.md -->
# Browser CDP Supervisor

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Hermes' browser herramientaing:

1. **Native JS dialogs** (`alert`/`confirm`/`prompt`/`beforeunload`) block the
   page's JS thread. Without supervision, the agente has no way to know a
   dialog is open — subsequent herramienta calls hang or throw opaque errors.
2. **Cross-origin iframes (OOPIFs)** are invisible to top-level
   `Runtime.evaluate`. The agente can see iframe nodes in the DOM snapshot but
...
into `browser_snapshot`, and exposing a `browser_dialog` herramienta for explicit
responses.
...
From the page's perspective, `prompt()` still returns the agente-supplied
string. From the agente's perspective, it's the same `browser_dialog(action=...)`
API either way.
...
- **Session map** — `Dict[session_id, SessionInfo]` so interaction herramientas can route to the right attached session for OOPIF operations
...
- `Runtime.enable` — `executionContextoCreated`, `consoleAPICalled`, `exceptionThrown`
- `Target.setAutoAttach {autoAttach: true, flatten: true}` — surfaces child OOPIF targets; supervisor enables `Page`+`Runtime` on each

Thread-safe state access via a snapshot lock; herramienta handlers (sync) read the
...
- **`must_respond`** (default) — capture, surface in `browser_snapshot`, wait
  ...
  with no response, auto-dismiss and log. Prevents a buggy agente from stalling
  forever.
- `auto_dismiss` — record and dismiss immediately; agente sees it after the
...
Herramienta is response-only. The agente reads pending dialogs from `browser_snapshot`
...
- **`pending_dialogs`** — dialogs currently blocking the page's JS thread.
  The agente must call `browser_dialog(action=...)` to respond. Empty on
...
- **`recent_dialogs`** — ring buffer of up to 20 recently-closed dialogs with
  a `closed_by` tag: `"agente"` (we responded), `"auto_policy"` (local
  ...
  how agentes on Browserbase still get visibility into what happened.
...
- **`frame_tree`** — frame structure including cross-origin (OOPIF) children.
  ...
  pages. `truncated: true` surfaces when limits were hit; agentes needing
  the full tree can use `browser_cdp` with `Page.getFrameTree`.

No new herramienta schema surface for any of these — the agente reads the snapshot it
already requests.

### Availability gating

Both surfaces gate on `_browser_cdp_check` (supervisor can only run when a CDP
endpoint is reachable). On Camofox / no-backend sessions, the dialog herramienta is
...
`sessionId`. Agentes pick frame_ids out of
...
agente uses `contentWindow`/`contentDocument` from a top-level
...
- `herramientas/browser_supervisor.py` — `CDPSupervisor`, `SupervisorRegistry`, `PendingDialog`, `FrameInfo`
- `herramientas/browser_dialog_herramienta.py` — `browser_dialog` herramienta handler
- `herramientas/browser_herramienta.py` — `browser_navigate` start-hook, `browser_snapshot` merge, `/browser connect` reattach, `_cleanup_browser_session` teardown
- `herramientasets.py` — registers `browser_dialog` in `browser`, `hermes-acp`, `hermes-api-server`, and core herramientasets (gated on CDP reachability)
...
- Streaming dialog/frame events live to the user (would require puerta de enlace hooks)
- Persisting dialog history across sessions (in-memoria only)
- Per-iframe dialog policies (agente can express this via `dialog_id`)
- Replacing `browser_cdp` — it stays as the escape hatch for the long tail (cookies, viewport, network throttling)

## Testing

Unit tests (`tests/herramientas/test_browser_supervisor.py`) use an asyncio mock CDP
