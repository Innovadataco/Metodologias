<!-- source: website/docs/guia-desarrollador/extending-the-cli.md -->
# Extending the CLI
...
| `_build_tui_style_dict()` | Custom prompt_herramientakit styles | You need custom colors or styling (pre-existing hook) |
...
from prompt_herramientakit.layout import FormattedTextControl, Window
from prompt_herramientakit.filters import Condition
...
Returns a list of prompt_herramientakit widgets to insert into the TUI layout. Widgets appear **between the spacer and the status bar** — above the input area but below the main output.
...
Each widget should be a prompt_herramientakit container (e.g., `Window`, `ConditionalContainer`, `HSplit`). Use `ConditionalContainer` or `filter=Condition(...)` to make widgets toggleable.

```python
from prompt_herramientakit.layout import ConditionalContainer, Window, FormattedTextControl
from prompt_herramientakit.filters import Condition
...
Parameters:
- **`kb`** — The `KeyBindings` instance for the prompt_herramientakit application
...
def _build_tui_layout_children(self, *, sudo_widget, secret_widget,
    approval_widget, clarify_widget, modelo_picker_widget=None,
...
[
    ...
    modelo_picker_widget,    # modelo picker overlay (conditional)
    ...
    *self._get_extra_tui_widgets(),  # YOUR WIDGETS GO HERE
    status_bar,             # modelo/token/contexto status line
...
4. **Status bar** — modelo, contexto %, elapsed time
...
- **Invalidate the display** after state changes: call `self._invalidate()` to trigger a prompt_herramientakit redraw.
- **Access agente state**: `self.agente`, `self.modelo`, `self.conversation_history` are all available.
