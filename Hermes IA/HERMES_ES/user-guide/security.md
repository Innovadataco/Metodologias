<!-- source: website/docs/guia-usuario/security.md -->
# Security

# Security

Hermes Agente is designed with a defense-in-depth security modelo. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.

## Descripción General

The security modelo has seven layers:

1. **User authorization** — who can talk to the agente (allowlists, DM pairing)
...
5. **Contexto file scanning** — prompt injection detection in project files
...
7. **Input sanitization** — working directory parameters in terminal herramienta backends are validated against an allowlist to prevent shell injection
...
approvals:
  ...
  mcp_reload_confirm: true        # /reload-mcp asks before invalidating the MCP herramienta cache
...
| `cron_mode` | `deny` | How [cron jobs](./features/cron.md) behave headlessly when they trigger a dangerous-command prompt. `deny` blocks the command (the agente must find another path); `approve` auto-approves everything in cron contexto. |
| `mcp_reload_confirm` | `true` | When true, `/reload-mcp` asks before rebuilding the MCP herramienta set. Rebuilding invalidates the proveedor prompt cache (herramienta schemas live in the system prompt), so the next message re-sends full input tokens. Users who click **Always Approve** flip this key to `false`. |
...
YOLO mode is available in both CLI and puerta de enlace sessions. Internally, it sets the `HERMES_YOLO_MODE` environment variable which is checked before every command execution.
...
The blocklist is the floor below `--yolo`. It trips **before** the approval layer even sees the command, and there's no override flag. Patterns currently covered (not exhaustive; kept in sync with `herramientas/approval.py::UNRECOVERABLE_BLOCKLIST`):
...
If you hit the blocklist, the herramienta call returns an explanatory error to the agente and nothing runs. If a legitimate workflow needs one of these commands (you're the operator of a wipe-and-reinstall pipeline, for example), run it outside the agente.
...
The following patterns trigger approval prompts (defined in `herramientas/approval.py`):
...
| `pkill`/`killall` hermes/puerta de enlace | Self-termination prevention |
| `puerta de enlace run` with `&`/`disown`/`nohup`/`setsid` | Prevents starting puerta de enlace outside service manager |
...
### Approval Flow (Puerta de enlace/Messaging)

On messaging platforms, the agente sends the dangerous command details to the chat and waits for the user to reply:

- Reply **yes**, **y**, **approve**, **ok**, or **go** to approve
- Reply **no**, **n**, **deny**, or **cancel** to deny

The `HERMES_EXEC_ASK=1` environment variable is automatically set when running the puerta de enlace.
...
## User Authorization (Puerta de enlace)

When running the messaging puerta de enlace, Hermes controls who can interact with the bot through a layered authorization system.
...
:::warning
If **no allowlists are configured** and `GATEWAY_ALLOW_ALL_USERS` is not set, **all users are denied**. The puerta de enlace logs a warning at startup:
...
Every container runs with these flags (defined in `herramientas/environments/docker.py`):
...
terminal:
  ...
  container_memoria: 5120  # MB (default 5GB)
...
:::tip
For production puerta de enlace deployments, use `docker`, `modal`, or `daytona` backend to isolate agente commands from your host system. This eliminates the need for dangerous command approval entirely.
:::
...
| **docker** | Container | ❌ Skipped (container is boundary) | Production puerta de enlace |
...
Both `execute_code` and `terminal` strip sensitive environment variables from child processes to prevent credential exfiltration by LLM-generated code. However, habilidads that declare `required_environment_variables` legitimately need access to those vars.

### How It Works

Two mechanisms allow specific variables through the sandbox filters:

**1. Habilidad-scoped passthrough (automatic)**

When a habilidad is loaded (via `habilidad_view` or the `/habilidad` command) and declares `required_environment_variables`, any of those vars that are actually set in the environment are automatically registered as passthrough. Missing vars (still in setup-needed state) are **not** registered.

```yaml
# In a habilidad's SKILL.md frontmatter
...
After loading this habilidad, `TENOR_API_KEY` passes through to `execute_code`, `terminal` (local), **and remote backends (Docker, Modal)** — no manual configuración needed.

:::info Docker & Modal
Prior to v0.5.1, Docker's `forward_env` was a separate system from the habilidad passthrough. They are now merged — habilidad-declared env vars are automatically forwarded into Docker containers and Modal sandboxes without needing to add them to `docker_forward_env` manually.
:::

**2. Config-based passthrough (manual)**

For env vars not declared by any habilidad, add them to `terminal.env_passthrough` in `config.yaml`:
...
Some habilidads need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active perfil's `HERMES_HOME`. Habilidads declare these in frontmatter:
...
When loaded, Hermes checks if these files exist in the active perfil's `HERMES_HOME` and registers them for mounting:
...
Paths are relative to `~/.hermes/`. Files are mounted to `/root/.hermes/` inside the container. This list is read by `herramientas/credential_files.py` (`terminal.credential_files`) — it lives under the `terminal:` block but is loaded by the credential-files module, not the core terminal backend, so it isn't part of the bundled `DEFAULT_CONFIG` snapshot.
...
| **terminal** (local) | Blocks explicit Hermes infrastructure vars (proveedor keys, puerta de enlace tokens, herramienta API keys) | ✅ Passthrough vars bypass the blocklist |
...
- The passthrough only affects vars you or your habilidads explicitly declare — the default security posture is unchanged for arbitrary LLM-generated code
- Credential files are mounted **read-only** into Docker containers
- Habilidads Guard scans habilidad content for suspicious env access patterns before installation
- Missing/unset vars are never registered (you can't leak what doesn't exist)
- Hermes infrastructure secrets (proveedor API keys, puerta de enlace tokens) should never be added to `env_passthrough` — they have dedicated mechanisms

## MCP Credential Handling

MCP (Modelo Contexto Protocol) server subprocesses receive a **filtered environment** to prevent accidental credential leakage.
...
Error messages from MCP herramientas are sanitized before being returned to the LLM. The following patterns are replaced with `[REDACTED]`:
...
You can restrict which websites the agente can access through its web and browser herramientas. This is useful for preventing the agente from accessing internal services, admin panels, or other sensitive URLs.
...
When a blocked URL is requested, the herramienta returns an error explaining the domain is blocked by policy. The blocklist is enforced across `web_search`, `web_extract`, `browser_navigate`, and all URL-capable herramientas.

See [Website Blocklist](/guia-usuario/configuración#website-blocklist) in the configuración guide for full details.

### SSRF Protection

All URL-capable herramientas (web search, web extract, vision, browser) validate URLs before fetching them to prevent Server-Side Request Forgery (SSRF) attacks. Blocked addresses include:
...
When on, web herramientas, the browser, vision URL fetches, and puerta de enlace media downloads no longer reject RFC 1918 / loopback / link-local / CGNAT / cloud-metadata destinations. **This is a deliberate trust boundary** — only enable it on machines where the agente running arbitrary prompt-injected URLs against the local network is an acceptable risk. Public-facing puerta de enlaces should leave it off.
...
### Contexto File Injection Protection

Contexto files (AGENTS.md, .cursorrules, SOUL.md) are scanned for prompt injection before being included in the system prompt. The scanner checks for:
...
### Puerta de enlace Deployment Checklist
...
3. **Restrict resource limits** — set appropriate CPU, memoria, and disk limits
...
7. **Set `terminal.cwd`** — don't let the agente operate from sensitive directories
8. **Run as non-root** — never run the puerta de enlace as root
...
For maximum security, run the puerta de enlace on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.hermes/.env`:
...
```bash
# ~/.hermes/.env
TERMINAL_SSH_HOST=agente-worker.local
TERMINAL_SSH_USER=hermes
TERMINAL_SSH_KEY=~/.ssh/hermes_agente_key
```

The SSH connection details live in `.env` (not `config.yaml`) so they aren't checked in or shared along with perfil exports. This keeps the puerta de enlace's messaging connections separate from the agente's command execution.
...
- **Puerta de enlace startup.** Logged to `puerta de enlace.log`; the first interactive message gets a short operator banner.
...
Many features (Mistral TTS, ElevenLabs, Honcho memoria, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Hermes installs these **lazily** on first use rather than eagerly under `hermes-agente[all]`. The implementation lives in `herramientas/lazy_deps.py`.
...
- **Bloat.** A user who only ever talks to one proveedor no longer pulls hundreds of packages they will never import.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `hermes herramientas`.

Security guarantees enforced by `herramientas/lazy_deps.py`:
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `hermes herramientas`.
