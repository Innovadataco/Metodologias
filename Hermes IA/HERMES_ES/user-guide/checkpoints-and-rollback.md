<!-- source: website/docs/guia-usuario/checkpoints-and-rollback.md -->
# Checkpoints and /rollback

# Checkpoints and `/rollback`

Hermes Agente can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.hermes/checkpoints/store/` — your real project `.git` is never touched. Every project the agente works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.

## What Triggers a Checkpoint

Checkpoints are taken automatically before:

- **File herramientas** — `write_file` and `patch`
...
The agente creates **at most one checkpoint per directory per turn**, so long-running sessions don't spam snapshots.
...
- Hermes detects when herramientas are about to **modify files** in your working tree.
...
flowchart LR
  ...
  agente["AIAgente\n(run_agente.py)"]
  herramientas["File & terminal herramientas"]
  cpMgr["CheckpointManager"]
  store["Shared shadow store\n~/.hermes/checkpoints/store/"]

  user --> agente
  agente -->|"herramienta call"| herramientas
  herramientas -->|"before mutate\nensure_checkpoint()"| cpMgr
  cpMgr -->|"git add/commit-tree/update-ref"| store
  cpMgr -->|"OK / skipped"| herramientas
  herramientas -->|"apply changes"| agente
```
...
Projects:        12
  ...
  /home/you/code/hermes-agente                                        20       2h ago  live
...
4. **Undoes the last conversation turn** so the agente's contexto matches the restored filesystem state.
...
- **Per-file size cap** — files larger than `max_file_size_mb` (default 10 MB) are excluded from the snapshot. Prevents accidentally swallowing datasets, modelo weights, or generated media.
...
- **Non-fatal errors** — all errors inside the Checkpoint Manager are logged at debug level; your herramientas continue to run.
...
- **Enable checkpoints only when you need them** — `hermes chat --checkpoints` or per-perfil `enabled: true`.
- **Use `/rollback diff` before restoring** — preview what will change to pick the right checkpoint.
- **Use `/rollback` instead of `git reset`** when you want to undo agente-driven changes only.
...
For running multiple agentes in parallel on the same repo, see the guide on [Git worktrees](./git-worktrees.md).
