<!-- source: website/docs/guia-usuario/windows-native.md -->
...
If you just want to install, the one-liner on the [landing page](/) or [Installation page](../inicio-rapido/installation#windows-native-powershell) is all you need. Come back here when something surprises you.

:::tip Want WSL instead?
If you prefer a real POSIX environment (for the panel's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\hermes`, WSL data lives under `~/.hermes`.
:::
...
```powershell
iex (irm https://raw.githubusercontent.com/NousResearch/hermes-agente/main/scripts/install.ps1)
```
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/NousResearch/hermes-agente/main/scripts/install.ps1))) -NoVenv -SkipSetup -Branch main
```
...
| `-InstallDir` | `%LOCALAPPDATA%\hermes\hermes-agente` | Override code location |
...
On first launch (and on demand when a missing herramienta is detected), Hermes runs a small Python bootstrapper — `hermes_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:

| Dependency | Why Hermes needs it |
|---|---|
| **PortableGit** | Provides `bash.exe` for the terminal herramienta and `git` for in-session clones. Provisioned at install time, not by `dep_ensure`. |
| **Node.js 22** | Required for the browser herramienta (`agente-browser`), the TUI's web bridge, and the WhatsApp bridge. |
| **ffmpeg** | Audio format conversion for TTS / voice messages. |
| **ripgrep** | Fast file search — falls back to `grep` if unavailable. |
| **npm packages** | `agente-browser`, Playwright Chromium, and any per-herramientaset Node deps are installed once at first browser-herramienta use. |

Each dep has a `shutil.which(...)`-style check; if a binary is missing and the run is interactive, `dep_ensure` offers to install it (deferring to `scripts\install.ps1 -ensure <dep>` for the actual install logic). Non-interactive runs (puerta de enlace, cron, headless desktop launches) skip the prompt and surface a clear `this feature needs <dep>` error instead.
...
5. **Clones the repo** to `%LOCALAPPDATA%\hermes\hermes-agente` and creates a virtualenv inside it.
6. **Tiered `uv pip install`** — tries `.[all]` first, falls back to progressively smaller sets (`[messaging,panel,ext]` → `[messaging]` → `.`) if a `git+https` dep flakes on rate-limited GitHub. Prevents "single flake drops you to a bare install" failure mode.
...
9. **Adds `%LOCALAPPDATA%\hermes\hermes-agente\venv\Scripts` to User PATH and sets `HERMES_HOME=%LOCALAPPDATA%\hermes`** — exposes the `hermes` command (and points it at your data dir) after you open a new terminal.
10. **Runs `hermes setup`** — the normal first-run wizard (modelo, proveedor, herramientasets). Skip with `-SkipSetup`.

:::tip Skip proveedor hunting on Windows
On Windows, per-herramienta API key setup (Firecrawl, FAL, Browser Use, OpenAI TTS) is the highest-friction part of getting a useful agente. A [Nous Portal](/guia-usuario/features/herramienta-puerta de enlace) subscription covers the modelo **and** all of those herramientas through one OAuth login. After the installer finishes, run `hermes setup --portal` to wire everything up.
:::

## Feature matrix

Everything except the panel's embedded terminal pane runs natively on Windows.

| Feature | Native Windows | WSL2 |
|---|---|---|
| CLI (`hermes chat`, `hermes setup`, `hermes puerta de enlace`, …) | ✓ | ✓ |
| Interactive TUI (`hermes --tui`) | ✓ | ✓ |
| Messaging puerta de enlace (Telegram, Discord, Slack, WhatsApp, 15+ platforms) | ✓ | ✓ |
| Cron scheduler | ✓ | ✓ |
| Browser herramienta (Chromium via Node) | ✓ | ✓ |
| MCP servers (stdio and HTTP) | ✓ | ✓ |
| Local Ollama / LM Studio / llama-server | ✓ | ✓ (via WSL networking) |
| Web panel (sessions, jobs, metrics, config) | ✓ | ✓ |
| Panel `/chat` embedded terminal pane | ✗ (needs POSIX PTY) | ✓ |
| Auto-start at login | ✓ (schtasks) | ✓ (systemd) |

The panel's `/chat` tab embeds a real terminal via a POSIX PTY (`ptyprocess`). Native Windows has no equivalent primitive; Python's `pywinpty` / Windows ConPTY would work but is a separate implementation — treat as future work. **The rest of the panel works natively** — only that one tab shows a "use WSL2 for this" banner.

## How Hermes runs shell commands on Windows

Hermes's terminal herramienta runs commands through **Git Bash**, same strategy Claude Code uses. This sidesteps the POSIX-vs-Windows gap without rewriting every herramienta.
...
Python's default stdio on Windows uses the console's active code page (usually cp1252 or cp437). Hermes's banner, slash-command list, herramienta feed, Rich panels, and habilidad descriptions all contain Unicode. Without intervention, any of that crashes with `UnicodeEncodeError: 'charmap' codec can't encode character…`.

The fix is in `hermes_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `hermes_cli/main.py::main`, `puerta de enlace/run.py::main`). It:
...
Pre-#21561, pressing `Ctrl-X Ctrl-E` or typing `/edit` silently did nothing on Windows. prompt_herramientakit has a hardcoded POSIX-absolute fallback list (`/usr/bin/nano`, `/usr/bin/pico`, `/usr/bin/vi`, …) that never resolves on Windows — even with full Git for Windows installed.
...
Set it permanently in your PowerShell perfil:
...
## Running the puerta de enlace at Windows login

`hermes puerta de enlace install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.
...
1. `schtasks /Create /SC ONLOGON /RL LIMITED /TN HermesPuerta de enlace` — registers a task that runs at your login with standard (non-elevated) permissions. No UAC prompt.
...
3. Spawns the puerta de enlace **detached via `pythonw.exe`** — not `python.exe`. `pythonw.exe` has no console attached, which immunizes it against `CTRL_C_EVENT` broadcasts from sibling processes (a real issue that used to kill the puerta de enlace when you Ctrl+C'd anything in the same process group).
...
```powershell
hermes puerta de enlace status      # Merged view: schtasks + Startup folder + running PID
hermes puerta de enlace start       # Starts the scheduled task now
hermes puerta de enlace stop        # Graceful SIGTERM equivalent (TerminateProcess via psutil)
hermes puerta de enlace restart
hermes puerta de enlace uninstall   # Removes schtasks entry, Startup shortcut, pid file
```

`hermes puerta de enlace status` is idempotent — call it a thousand times in a row and it will never accidentally kill the puerta de enlace. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)

### Why not a Windows Service?

Services require admin rights to install and tie the puerta de enlace's lifecycle to machine boot, not user login. The typical Hermes user wants: log in → puerta de enlace available, log out → puerta de enlace gone. Scheduled Tasks do exactly that without elevation. If you genuinely want a service, use `nssm` or `sc create` manually — but you probably don't.

## Data layout

| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\hermes\hermes-agente\` | Git checkout + venv. `venv\Scripts\hermes.exe` is the command added to User PATH. Safe to `Remove-Item -Recurse` and reinstall. |
...
| `%LOCALAPPDATA%\hermes\` (root) | Your config, auth, habilidads, sessions, logs (`config.yaml`, `.env`, `habilidads\`, `sessions\`, `logs\`, …). **Survives reinstalls.** |

On native Windows the installer sets `HERMES_HOME=%LOCALAPPDATA%\hermes`, so your data and the disposable install live under the **same** `%LOCALAPPDATA%\hermes` root: the install/runtime is the `hermes-agente\`, `git\`, `node\`, and `bin\` subdirectories, while your data files sit directly in `%LOCALAPPDATA%\hermes`. Reinstalling only replaces the `hermes-agente\` checkout, so your data survives — but because the two share a root, **don't** `Remove-Item -Recurse %LOCALAPPDATA%\hermes` if you want to keep your data; delete the `hermes-agente\` subdirectory instead. Your data directory is identical in shape to a Linux `~/.hermes`, so you can mirror it between machines.
...
The browser herramienta uses `agente-browser` (a Node helper) to drive Chromium. On Windows:

- The installer puts `agente-browser` on PATH via npm.
- `shutil.which("agente-browser", path=...)` picks up the `.cmd` shim automatically — `CreateProcessW` can't execute an extensionless shebang, so Hermes always resolves to the `.CMD` wrapper. Don't manually invoke the shebang script; always go through the `.cmd`.
...
The installer adds `%LOCALAPPDATA%\hermes\hermes-agente\venv\Scripts` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.

Verify:

```powershell
Get-Command hermes        # should print C:\Users\<you>\AppData\Local\hermes\hermes-agente\venv\Scripts\hermes.exe
hermes --version
```
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `hermes.cmd` shim, deletes `%LOCALAPPDATA%\hermes\hermes-agente\`, and trims the User PATH. It leaves the rest of `%LOCALAPPDATA%\hermes\` alone (your config, auth, habilidads, sessions, logs) in case you're reinstalling.
...
Consequence: any codepath that said "check if this PID is alive" via `os.kill(pid, 0)` on Windows was silently killing the target. Hermes migrated every such site (14 across 11 files) to `puerta de enlace.status._pid_exists()`, which uses `psutil.pid_exists()` (which in turn uses `OpenProcess + GetExitCodeProcess` on Windows — no signals). If you're writing a complemento or patch, use `psutil.pid_exists()` directly or `puerta de enlace.status._pid_exists()` — never `os.kill(pid, 0)`.
...
**`WinError 193: %1 is not a valid Win32 application` when running a herramienta.**
You hit a shebang-script invocation that bypassed the `.cmd` shim. Hermes resolves commands through `shutil.which(cmd, path=local_bin)` so PATHEXT picks up `.CMD` — if you're invoking the herramienta via a hardcoded path instead, switch to the `.cmd` variant (e.g., `npx.cmd`, not `npx`).
...
**Puerta de enlace won't stay running after restart.**
Check `hermes puerta de enlace status` — it merges the schtasks entry, the Startup-folder shortcut (if used), and the live PID. If schtasks is registered but not running, group policy may be blocking `ONLOGON` triggers. Run `schtasks /Query /TN HermesPuerta de enlace /V /FO LIST` to see the task's failure reason, or fall back to the Startup-folder path by uninstalling and reinstalling with `HERMES_GATEWAY_FORCE_STARTUP=1`.
...
**Browser herramienta launches but herramientas time out.**
...
**`agente-browser` fails with a weird Node version error.**
...
**Puerta de enlace can't send Telegram photos — "`BadRequest: payload contains invalid characters`".**
This is unrelated to Windows but sometimes surfaces first there. Usually it means your file path contains unescaped backslashes in a JSON body. Telegram should be receiving paths Hermes normalizes, not raw Windows paths — if you're seeing this inside a custom complemento, make sure you're passing the Hermes-provided path, not `str(Path(...))` from user input.

**"Works on my other machine" encoding weirdness after `git pull`.**
If you edited Hermes config or a habilidad on Windows using a non-UTF-8 editor (Notepad on older Windows versions, some Chinese IMEs), the file may have been saved with a BOM. Hermes tolerates `utf-8-sig` on most config reads, but a BOM inside a folded YAML scalar (`description: >`) silently breaks YAML parsing. Re-save the file as plain UTF-8 without BOM.

## Where to go next

- **[Installation](../inicio-rapido/installation.md)** — the full install page, including Linux/macOS/WSL2/Termux.
- **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)** — if you want POSIX semantics or the panel terminal pane.
...
- **[Messaging Puerta de enlace](./messaging/index.md)** — running Telegram/Discord/Slack on Windows.
