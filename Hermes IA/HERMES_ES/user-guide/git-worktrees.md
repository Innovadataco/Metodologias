<!-- source: website/docs/guia-usuario/git-worktrees.md -->
# Git Worktrees

# Git Worktrees

Hermes Agente is often used on large, long‑lived repositories. When you want to:

- Run **multiple agentes in parallel** on the same project, or
- Keep experimental refactors isolated from your main branch,

Git **worktrees** are the safest way to give each agente its own checkout without duplicating the entire repository.
...
- Messaging puerta de enlaces: the directory set by `terminal.cwd` in `~/.hermes/config.yaml`

If you run multiple agentes in the **same checkout**, their changes can interfere with each other:

- One agente may delete or rewrite files the other is using.
- It becomes harder to understand which changes belong to which experiment.

With worktrees, each agente gets:
...
- Use that directory for contexto files, code edits, and herramientas.
- Use a **separate checkpoint history** for `/rollback` scoped to this worktree.

## Running Multiple Agentes in Parallel
...
- Pairing CLI + puerta de enlace sessions against the same upstream repo.
...
- **Commit frequently**
  ...
  - Use [checkpoints and /rollback](./checkpoints-and-rollback.md) as a safety net for herramienta‑driven edits in between.
- **Avoid running Hermes from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agente has a clear scope.
...
For parallel agentes, open multiple terminals and run `hermes -w` in each — every invocation gets its own worktree and branch automatically.
...
- Strong guarantees that different agentes and experiments do not step on each other.
