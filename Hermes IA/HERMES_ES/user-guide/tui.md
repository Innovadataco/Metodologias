<!-- source: website/docs/guia-usuario/tui.md -->
# TUI

# TUI

The TUI is the modern front-end for Hermes — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agente, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.
...
The classic CLI remains the shipped default. Anything documented in [CLI Interface](cli.md) — slash commands, quick commands, habilidad preloading, personalities, multi-line input, interrupts — works in the TUI identically.
...
- **Non-blocking input** — type and queue messages before the session is ready. Your first prompt sends the moment the agente comes online.
- **Rich overlays** — modelo picker, session picker, approval and clarification prompts all render as modal panels rather than inline flows.
- **Live session panel** — herramientas and habilidads fill in progressively as they initialize.
...
Same [skins](features/skins.md) and [personalities](features/personality.md) apply. Switch mid-session with `/skin ares`, `/personality pirate`, and the UI repaints live. See [Skins & Themes](features/skins.md) for the full list of customizable keys and which ones apply to classic vs TUI — the TUI honors the banner palette, UI colors, prompt glyph/color, session display, completion menu, selection bg, `herramienta_prefix`, and `help_header`.
...
|---------|---------------|
| Herramientas | Open |
| Habilidads | Collapsed |
| System Prompt | Collapsed |
| MCP Servers | Collapsed |

Click anywhere on a section header (or its chevron) to toggle it. The Herramientas list opens by default because it's the most-checked section at session start; Habilidads, System Prompt, and MCP Servers collapse by default so the banner stays compact even when you've installed dozens of habilidads or wired up many MCP servers. State is local to the banner instance, so the next launch resets to the defaults.
...
- **`Ctrl+X`** opens the live session switcher. When a queued message is highlighted (sent while the agente was still running), it still deletes that queued message instead. **`Esc`** cancels editing and unhighlights without deleting.
...
| `/modelo` | Modal modelo picker grouped by proveedor, with cost hints |
| `/skin` | Live preview — theme change applies as you browse |
| `/details` | Toggle verbose herramienta-call details (global or per-section) |
| `/usage` | Rich token / cost / contexto panel |
| `/agentes` (alias `/tasks`) | Observability overlay — live subagente tree with kill/pause controls, per-branch cost / token / file rollups, turn-by-turn history |
...
Every other slash command (including installed habilidads, quick commands, and personality toggles) works identically to the classic CLI. See [Slash Commands Reference](../reference/slash-commands.md).
...
- Select `+new`, type a prompt, and press `Enter` to dispatch a new live session. Press `Tab` first if you want to choose a modelo just for that new session.
...
The status-bar busy indicator is pluggable — the default rotates Hermes' kawaii face palette every 2.5 seconds during agente work. Pick a different style via config or the `/indicator` slash command:
...
The TUI's status line tracks agente state in real time:

| Status | Meaning |
|--------|---------|
| `starting agente…` | Session ID is live; herramientas and habilidads still coming online. You can type — messages queue and send when ready. |
| `ready` | Agente is idle, accepting input. |
| `thinking…` / `running…` | Agente is reasoning or running a herramienta. |
...
- **Working directory with git branch** — `~/projects/hermes-agente (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
The TUI respects all standard Hermes config: `~/.hermes/config.yaml`, perfils, personalities, skins, quick commands, credential pools, memoria proveedors, herramienta/habilidad enablement. No TUI-specific config file exists.
...
display:
  ...
  sections:                  # optional: per-section overrides (any subset)
    ...
    herramientas: expanded          # always open
...
- `/details <section> [hidden|collapsed|expanded|reset]` — override one section
  (sections: `thinking`, `herramientas`, `subagentes`, `activity`)
...
- `thinking` — **expanded**. Reasoning streams inline as the modelo emits it.
- `herramientas` — **expanded**. Herramienta calls and their results render open.
- `subagentes` — falls through to the global `details_mode` (collapsed under
  chevron by default — stays quiet until a delegation actually happens).
- `activity` — **hidden**. Ambient meta (puerta de enlace hints, terminal-parity
  nudges, background notifications) is noise for most day-to-day use. Herramienta
  failures still render inline on the failing herramienta row; ambient
...
- `display.sections.herramientas: collapsed` — put herramienta calls back under a chevron
...
## How the TUI talks to its puerta de enlace

By default the TUI spawns its own in-process puerta de enlace, so each TUI instance is self-contained — there's nothing to configure.

You may see a `HERMES_TUI_GATEWAY_URL` env var referenced in the codebase or logs. This is an **internal wiring detail of the web panel**, not a user-facing remote-attach knob. When you open the panel's "Chat" tab (`hermes panel` → `/chat`), the panel's web server spawns an embedded TUI child process and injects `HERMES_TUI_GATEWAY_URL` so that child attaches to the panel's own in-process `tui_puerta de enlace` over a loopback WebSocket (`/api/ws`). The `/api/ws` endpoint exists only inside the panel server (`hermes_cli/web_server.py`) and is bound to that process's lifetime and auth.

There is no general "point any TUI at any standalone puerta de enlace port" mode. In particular, the OpenAI-compatible API server (`hermes puerta de enlace` / the `api_server` platform) does **not** serve `/api/ws` — it's the modelo-backend surface (`/v1/chat/completions`, `/v1/modelos`, …) and deliberately does not expose the TUI's JSON-RPC control channel. Setting `HERMES_TUI_GATEWAY_URL` to that port will 404.

If you want multiple surfaces to share one set of sessions, use the shared `~/.hermes/state.db` (see [Sessions](sessions.md)) or the web panel's embedded chat (see [Web Panel](features/web-panel.md#chat)) — not a hand-set puerta de enlace URL.

## Reverting to the classic CLI

Launching `hermes` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.hermes/config.yaml` (persistent) or `HERMES_TUI=1` in your shell perfil (per-shell). To go back, set `interface: cli` / unset the env var, or pass `hermes --cli` for a one-off.
...
- [Configuración](configuración.md) — all config keys
