<!-- source: website/docs/guia-usuario/features/skins.md -->
# Skins & Themes

# Skins & Themes

Skins control the **visual presentation** of the Hermes CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the herramienta activity prefix.

Conversational style and visual style are separate concepts:

- **Personality** changes the agente's tone and wording.
...
| Skin | Description | Agente branding | Visual character |
|------|-------------|----------------|------------------|
| `default` | Classic Hermes — gold and kawaii | `Hermes Agente` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
| `ares` | War-god theme — crimson and bronze | `Ares Agente` | Deep crimson borders with bronze accents. Aggressive spinner verbs ("forging", "marching", "tempering steel"). Custom sword-and-shield ASCII art banner. |
| `mono` | Monochrome — clean grayscale | `Hermes Agente` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Hermes Agente` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Hermes Agente` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal perfils. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Hermes Agente` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
| `poseidon` | Ocean-god theme — deep blue and seafoam | `Poseidon Agente` | Deep blue to seafoam gradient. Ocean-themed spinners ("charting currents", "sounding the depth"). Trident ASCII art banner. |
| `sisyphus` | Sisyphean theme — austere grayscale with persistence | `Sisyphus Agente` | Light grays with stark contrast. Boulder-themed spinners ("pushing uphill", "resetting the boulder", "enduring the loop"). Boulder-and-hill ASCII art banner. |
| `charizard` | Volcanic theme — burnt orange and ember | `Charizard Agente` | Warm burnt orange to ember gradient. Fire-themed spinners ("banking into the draft", "measuring burn"). Dragon-silhouette ASCII art banner. |
...
| `banner_accent` | Section headers in the banner (Available Herramientas, etc.) | `#FFBF00` (amber) |
| `banner_dim` | Muted text in the banner (separators, secondary labels) | `#B8860B` (dark goldenrod) |
| `banner_text` | Body text in the banner (herramienta names, habilidad names) | `#FFF8DC` (cornsilk) |
...
| `response_border` | Border around the agente's response box (ANSI escape) | `#FFD700` |
...
| `thinking_faces` | list of strings | Faces cycled during modelo reasoning | `["(⚔)", "(⌁)", "(<>)"]` |
...
|-----|-------------|---------|
| `agente_name` | Name shown in banner title and status display | `Hermes Agente` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Hermes Agente! Type your message or /help for commands.` |
...
|-----|------|-------------|---------|
| `herramienta_prefix` | string | Character prefixed to herramienta output lines in the CLI | `┊` |
| `herramienta_emojis` | dict | Per-herramienta emoji overrides for spinners and progress (`{herramienta_name: emoji}`) | `{}` |
...
branding:
  agente_name: "My Agente"
  welcome: "Welcome to My Agente! Type your message or /help for commands."
  goodbye: "See you later! ⚡"
  response_label: " ⚡ My Agente "
  prompt_symbol: "⚡"
  help_header: "(⚡) Available Commands"

herramienta_prefix: "┊"

# Per-herramienta emoji overrides (optional)
herramienta_emojis:
  terminal: "⚔"
  web_search: "🔮"
  read_file: "📄"
...
- Opens any skin into a visual editor with all Hermes skin fields (colors, spinner, branding, herramienta prefix, herramienta emojis)
...
Hermes Mod respects the `HERMES_HOME` environment variable, so it works with [perfils](/guia-usuario/perfils) too.
