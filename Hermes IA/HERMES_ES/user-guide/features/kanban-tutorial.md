<!-- source: website/docs/guia-usuario/features/kanban-tutorial.md -->
# guia-usuario/features/kanban-tutorial

# Kanban tutorial

A walkthrough of the four use-cases the Hermes Kanban system was designed for, with the panel open in a browser. If you haven't read the [Kanban overview](./kanban) yet, start there — this assumes you know what a task, run, assignee, and dispatcher are.

## Setup

```bash
hermes kanban init           # optional; first `hermes kanban <anything>` auto-inits
hermes panel             # opens http://127.0.0.1:9119 in your browser
# click Kanban in the left nav
```

The panel is the most comfortable place for **you** to watch the system. Agente workers the dispatcher spawns never see the panel or the CLI — they drive the board through a dedicated `kanban_*` [herramientaset](./kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`). All three surfaces — panel, CLI, worker herramientas — route through the same per-board SQLite DB (`~/.hermes/kanban.db` for the default board, `~/.hermes/kanban/boards/<slug>/kanban.db` for any board you create later), so each board is consistent no matter which side of the fence a change came from.

This tutorial uses the `default` board throughout. If you want multiple isolated queues (one per project / repo / domain), see [Boards (multi-project)](./kanban#boards-multi-project) in the overview — the same CLI / panel / worker flows apply per board, and workers physically cannot see tasks on other boards.

Throughout the tutorial, **code blocks labelled `bash` are commands *you* run.** Code blocks labelled `# worker herramienta calls` are what the spawned worker's modelo emits as herramienta calls — shown here so you can see the loop end-to-end, not because you'd ever run them yourself.
...
- **Triage** — raw ideas. By default the dispatcher auto-runs the **decomposer** on tasks here: the built-in decomposer uses `auxiliary.kanban_decomposer`, reads your perfil roster + descriptions, and produces a graph of child tasks routed to the best-fit specialists. The original task is held alive as the parent so its assignee (`kanban.orchestrator_perfil`, or the active default perfil when unset) wakes back up to judge completion when everything finishes. Flip the **Orchestration: Auto/Manual** pill at the top of the kanban page to switch modes. In Manual mode click **⚗ Decompose** on a card, or run `hermes kanban decompose <id>` / `/kanban decompose <id>`. For single tasks that don't need fan-out, **✨ Specify** does a one-shot spec rewrite (goal, approach, acceptance criteria) and promotes to `todo`. Configure the modelos under `auxiliary.kanban_decomposer` and `auxiliary.triage_specifier` in `config.yaml`. See [Auto vs Manual orchestration](./kanban#auto-vs-manual-orchestration) in the main Kanban guide.
- **Todo** — created but waiting on dependencies, or not yet assigned.
- **Ready** — assigned and waiting for the dispatcher to claim.
- **In progress** — a worker is actively running the task. With "Lanes by perfil" on (the default), this column sub-groups by assignee so you can see at a glance what each worker is doing.
- **Blocked** — a worker asked for human input, or the circuit breaker tripped.
- **Done** — completed.

The top bar has filters for search, tenant, and assignee, plus a `Lanes by perfil` toggle and a `Nudge dispatcher` button that runs one dispatch tick right now instead of waiting for the daemon's next interval. Clicking any card opens its drawer on the right.

### Flat view

If the perfil lanes are noisy, toggle "Lanes by perfil" off and the In Progress column collapses to a single flat list ordered by claim time:

![Board with lanes by perfil off](/img/kanban-tutorial/02-board-flat.png)
...
On the next dispatcher tick (60s by default, or immediately if you hit **Nudge dispatcher**) the `backend-dev` perfil spawns as a worker with `HERMES_KANBAN_TASK=$SCHEMA` in its env. Here's what the worker's herramienta-call loop looks like from inside the agente:

```python
# worker herramienta calls — NOT commands you run
kanban_show()
# → returns title, body, worker_contexto, parents, prior attempts, comments

# (worker reads worker_contexto, uses terminal/file herramientas to design the schema,
...
Start the puerta de enlace and walk away — it hosts the embedded dispatcher
that picks up all three specialist perfils' tasks on the same
kanban.db:
...
The panel view, filtered by `auth-project`:
...
The interesting one is the implementation task, because it was blocked and retried. Here's the full three-agente choreography, shown as the herramienta calls each worker's modelo makes:

```python
# --- PM worker spawns on $SPEC and writes the acceptance criteria ---
# worker herramienta calls
kanban_show()
kanban_complete(
...
# worker herramienta calls
kanban_show()   # reads $SPEC's summary + acceptance metadata in worker_contexto
...
Now you (the human, or a separate reviewer perfil) read the block reason, decide the fix direction is clear, and unblock from the panel's "Unblock" button — or from the CLI / slash command:
...
# worker herramienta calls
kanban_show()
# → worker_contexto now includes the run 1 block reason, so this worker knows
...
Each run is a row in `task_runs` with its own outcome, summary, and metadata. Retry history is not a conceptual afterthought layered on top of a "latest state" task — it's the primary representation. When a retrying worker opens the task, `build_worker_contexto` shows it the prior attempts, so the second-pass worker sees why the first pass was blocked and addresses those specific findings instead of re-running from scratch.
...
A deploy task that can't spawn its worker because `AWS_ACCESS_KEY_ID` isn't set in the perfil's environment:
...
If Telegram / Discord / Slack is wired in, a puerta de enlace notification fires on the `gave_up` event so you hear about the outage without having to check the board.
...
The example in the seed data is a migration that was running out of memoria:
...
When a worker on task B is spawned and calls `kanban_show()`, the `worker_contexto` it gets back includes:
...
- [Kanban overview](./kanban) — the full data modelo, event vocabulary, and CLI reference.
...
- `hermes kanban notify-subscribe <task> --platform telegram --chat-id <id>` — get a puerta de enlace ping when a specific task finishes.
