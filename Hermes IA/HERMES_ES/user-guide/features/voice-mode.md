<!-- source: website/docs/guia-usuario/features/voice-mode.md -->
# Voice Mode

# Voice Mode

Hermes Agente supports full voice interaction across CLI and messaging platforms. Talk to the agente using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configuracións and real usage patterns, see [Use Voice Mode with Hermes](/guides/use-voice-mode-with-hermes).

## Prerequisites

Before using voice features, make sure you have:

1. **Hermes Agente installed** — via the install script (see [Installation](/inicio-rapido/installation))
2. **An LLM proveedor configured** — run `hermes modelo` or set your preferred proveedor credentials in `~/.hermes/.env`
3. **A working base setup** — run `hermes` to verify the agente responds to text before enabling voice
...
A paid [Nous Portal](/guia-usuario/features/herramienta-puerta de enlace) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Herramienta Puerta de enlace — no separate OpenAI key needed. On a fresh install, `hermes setup --portal` wires both up at once.
:::
...
|---------|----------|-------------|
| **Interactive Voice** | CLI | Press Ctrl+B to record, agente auto-detects silence and responds |
| **Auto Voice Reply** | Telegram, Discord | Agente sends spoken audio alongside text responses |
...
cd ~/.hermes/hermes-agente && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.hermes/hermes-agente && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.hermes/hermes-agente && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.hermes/hermes-agente && uv pip install -e ".[all]"
```
...
| `tts-premium` | `elevenlabs` | ElevenLabs TTS proveedor |

Optional local TTS proveedor: install `neutts` separately with `python -m pip install -U neutts[all]`. On first use it downloads the modelo automatically.
...
| **espeak-ng** | Phonemizer backend | Local NeuTTS proveedor |
...
```bash
# Speech-to-Text — local proveedor needs NO key at all
...
:::tip
If `faster-whisper` is installed, voice mode works with **zero API keys** for STT. The modelo (~150 MB for `base`) downloads automatically on first use.
:::
...
6. Audio is transcribed via Whisper and sent to the agente
7. If TTS is enabled, the agente's reply is spoken aloud
...
When TTS is enabled, the agente speaks its reply **sentence-by-sentence** as it generates text — you don't wait for the full response:
...
Whisper sometimes generates phantom text from silence or background noise ("Thank you for watching", "Subscribe", etc.). The agente filters these out using a set of 26 known hallucination phrases across multiple languages, plus a regex pattern that catches repetitive variations.

---

## Puerta de enlace Voice Reply (Telegram & Discord)
...
Start the puerta de enlace to connect to your messaging platforms:

```bash
hermes puerta de enlace        # Start the puerta de enlace (connects to configured platforms)
hermes puerta de enlace setup  # Interactive setup wizard for first-time configuración
```
...
|------|------------|-----------------|-------|
| **Direct Message (DM)** | Open the bot's perfil → "Message" | No | Works immediately |
...
Voice mode setting is persisted across puerta de enlace restarts.
...
The most immersive voice feature: the bot joins a Discord voice channel, listens to users speaking, transcribes their speech, processes through the agente, and speaks the reply back in the voice channel.
...
:::warning
Re-inviting the bot to a server it's already in will update its permissions without removing it. You won't lose any data or configuración.
:::

#### 2. Privileged Puerta de enlace Intents

In the [Developer Portal](https://discord.com/developers/applications) → your application → **Bot** → **Privileged Puerta de enlace Intents**, enable all three:
...
The Opus codec library must be installed on the machine running the puerta de enlace:
...
# STT — local proveedor needs no key (pip install faster-whisper)
...
```bash
hermes puerta de enlace        # Start with existing configuración
```
...
4. **Processes** through the full agente pipeline (session, herramientas, memoria)
...
- Agente responses are sent as text in the channel AND spoken in the VC
...
stt:
  enabled: true                     # set to false to skip auto-transcription —
                                    # the puerta de enlace still caches the audio file and
                                    # passes its path to the agente as part of the
                                    # inbound message, useful for custom pipelines
                                    # (diarization, alignment, archival, etc.)
  proveedor: "local"                  # "local" (free) | "groq" | "openai" | "mistral" | "xai"
  local:
    modelo: "base"                    # tiny, base, small, medium, large-v3
  # modelo: "whisper-1"              # Legacy: used when proveedor is not set

# Text-to-Speech
tts:
  proveedor: "edge"                 # "edge" (free) | "elevenlabs" | "openai" | "neutts" | "minimax" | "mistral" | "gemini" | "xai" | "kittentts" | "piper"
  edge:
...
```bash
# Speech-to-Text proveedors (local needs no key)
...
STT_GROQ_MODEL=whisper-large-v3-turbo    # Override default Groq STT modelo
STT_OPENAI_MODEL=whisper-1               # Override default OpenAI STT modelo
...
# Text-to-Speech proveedors (Edge TTS and NeuTTS need no key)
ELEVENLABS_API_KEY=***             # ElevenLabs (premium quality)
# VOICE_TOOLS_OPENAI_KEY above also enables OpenAI TTS
...
### STT Proveedor Comparison

| Proveedor | Modelo | Speed | Quality | Cost | API Key |
|----------|-------|-------|---------|------|---------|
...
Proveedor priority (automatic fallback): **local** > **groq** > **openai**

### TTS Proveedor Comparison

| Proveedor | Quality | Cost | Latency | Key Required |
|----------|---------|------|---------|-------------|
...
If you are running Hermes inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/guia-usuario/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
- Check the LLM modelo is configured and accessible
- Review puerta de enlace logs: `tail -f ~/.hermes/logs/puerta de enlace.log`

### Bot responds in text but not in voice channel

- TTS proveedor may be failing — check API key and quota
