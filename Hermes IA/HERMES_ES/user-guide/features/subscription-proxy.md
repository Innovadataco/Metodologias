<!-- source: website/docs/guia-usuario/features/subscription-proxy.md -->
# Subscription Proxy
...
chat completions — use your Hermes-managed proveedor subscription as their
...
|---|---|---|
| What it serves | Your agente (full herramientaset, memoria, habilidads) | Raw modelo inference |
...
| Herramienta calls | Yes — the agente runs herramientas | No — passthrough only |

Use the API server when you want the **agente** as a backend. Use the
proxy when you just want **the modelo** through your subscription.

## Quick Start

### 1. Log into your proveedor (one-time)
...
Modelo:      Hermes-4-70B    # or Hermes-4.3-36B, Hermes-4-405B
```
...
```bash
hermes proxy proveedors
```

Currently shipped: `nous` (Nous Portal) and `xai` (xAI / Grok). More
OAuth proveedors can be added by implementing the `UpstreamAdapter`
...
[OpenViking](https://github.com/volcengine/OpenViking) is a contexto
database that needs an LLM proveedor for its VLM (vision/language modelo
used to extract memories) and embedding modelo. With the proxy, you can
...
embedding modelo side still needs its own proveedor — Portal does serve
`/v1/embeddings` but the modelo selection depends on what your tier
supports; check `portal.nousresearch.com/modelos`.
...
No transformation. No logging of request bodies. No agente loop. The
proxy is a credential-attaching pass-through.

## Future: more OAuth proveedors

The adapter system is pluggable. Adding a new proveedor (e.g.
HuggingFace, GitHub Copilot's chat endpoint, Anthropic via OAuth)
requires implementing `UpstreamAdapter` in
`hermes_cli/proxy/adapters/<proveedor>.py` and registering it in
`adapters/__init__.py`. Proveedors that aren't OpenAI-compatible at the
