<!-- source: website/docs/guia-usuario/features/web-search.md -->
# guia-usuario/features/web-search.md

# Web Search & Extract

Hermes Agente includes two modelo-callable web herramientas backed by multiple proveedors:

- **`web_search`** — search the web and return ranked results
- **`web_extract`** — fetch and extract readable content from one or more URLs

Both are configured through a single backend selection. Proveedors are chosen via `hermes herramientas` or set directly in `config.yaml`.

## Backends

| Proveedor | Env Var | Search | Extract | Free tier |
|----------|---------|--------|---------|-----------|
...
Brave Search, DDGS, and xAI are **search-only** — pair any of them with Firecrawl/Tavily/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let Hermes lazy-install it on first use). xAI runs Grok's server-side `web_search` herramienta on the Responses API — results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all modelo output (see the [trust-modelo caveat](#xai-grok) below).

**Per-capability split:** you can use different proveedors for search and extract independently — for example SearXNG (free) for search and Firecrawl for extract. See [Per-capability configuración](#per-capability-configuración) below.

:::tip Nous Subscribers
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, web search and extract are available through the **[Herramienta Puerta de enlace](herramienta-puerta de enlace.md)** via managed Firecrawl — no API key needed. New installs can run `hermes setup --portal` to log in and turn on all puerta de enlace herramientas at once; existing installs can flip just web via `hermes herramientas`.
:::
...
Backends return raw page markdown, which can be huge (forum threads, docs sites, news articles with embedded comments). To keep your contexto window usable and your costs down, `web_extract` runs returned content through the **`web_extract` auxiliary modelo** before handing it to the agente. Behavior is purely size-driven:

| Page size (characters) | What happens |
|------------------------|--------------|
| Under 5 000 | Returned as-is — no LLM call, full markdown reaches the agente |
| 5 000 – 500 000 | Single-pass summary via the `web_extract` auxiliary modelo, capped at ~5 000 chars of output |
...
### Which modelo does the summarizing?

The `web_extract` auxiliary task. By default (`auxiliary.web_extract.proveedor: "auto"`), this is your **main chat modelo** — same proveedor, same modelo as `hermes modelo`. That's fine for most setups, but on expensive reasoning modelos (Opus, MiniMax M2.7, etc.) every long-page extract adds meaningful cost.

To route extraction summaries to a cheap, fast modelo regardless of your main:
...
Or pick interactively: `hermes modelo` → **Configure auxiliary modelos** → `web_extract`.

See [Auxiliary Modelos](/guia-usuario/configuración#auxiliary-modelos) for the full reference and per-task override patterns.

### When summarization gets in the way

If you specifically need raw, unsummarized page content — for example, you're scraping a structured page where the LLM summary would drop important fields — use `browser_navigate` + `browser_snapshot` instead. The browser herramienta returns the live accessibility tree without auxiliary-modelo rewriting (subject to its own 8 000-char snapshot cap on huge pages).

---

## Setup

### Quick setup via `hermes herramientas`

Run `hermes herramientas`, navigate to **Web Search & Extract**, and pick a proveedor. The wizard prompts for the required URL or API key and writes it to your config.
...
SearXNG is **search-only** — `web_extract` requires a separate extract proveedor.
...
Or set via `hermes herramientas` → Web Search & Extract → SearXNG.
...
#### Pair SearXNG with an extract proveedor

SearXNG handles search; you need a separate proveedor for `web_extract`. Use the per-capability keys:
...
Routes `web_search` through Grok's server-side [web_search herramienta](https://docs.x.ai/developers/herramientas/web-search) on the Responses API. Grok runs the actual searching and returns the top results as structured JSON.
...
web:
  backend: "xai"
  xai:
    modelo: grok-build-0.1        # reasoning modelo required by web_search (default)
...
**Search-only** — pair with Firecrawl / Tavily / Exa / Parallel if you also need `web_extract`. On 401 the proveedor performs a single forced OAuth-token refresh and retries (covers mid-window revocation and opaque tokens the proactive expiry check can't decode); env-var credentials skip the retry.

:::caution Trust modelo
Unlike index-backed proveedors (Brave, Tavily, Exa) which return verbatim search-engine results, xAI is an LLM choosing which URLs to surface and writing the titles and descriptions itself. The *content* of the query influences the output, so a maliciously crafted query (e.g. injected via untrusted upstream input the agente picked up) can in principle steer Grok into emitting attacker-chosen URLs. Treat returned URLs the same way you'd treat any modelo-generated link — validate before fetching, especially if the query came from untrusted input.
:::
...
Set one proveedor for all web capabilities:
...
### Per-capability configuración

Use different proveedors for search vs extract. This lets you combine free search (SearXNG) with a paid extract proveedor, or vice versa:
...
```bash
# Activate the venv and run the web herramientas module directly
source ~/.hermes/hermes-agente/.venv/bin/activate
python -m herramientas.web_herramientas
```
...
SearXNG cannot extract URL content. Set `web.extract_backend` to a proveedor that supports extraction:
...
The auxiliary modelo didn't finish summarizing within the configured timeout. Either:

- Raise `auxiliary.web_extract.timeout` in `config.yaml` (default 360s on fresh installs, 30s if the key is missing)
- Switch the `web_extract` auxiliary task to a faster modelo (e.g. `google/gemini-3-flash-preview`) — see [How `web_extract` handles long pages](#how-web_extract-handles-long-pages)
- For pages where summarization is the wrong herramienta, use `browser_navigate` instead

---

## Optional habilidad: `searxng-search`

For agentes that need to use SearXNG via `curl` directly (e.g. as a fallback when the web herramientaset isn't available), install the `searxng-search` optional habilidad:

```bash
hermes habilidads install official/research/searxng-search
```

This adds a habilidad that teaches the agente how to:
