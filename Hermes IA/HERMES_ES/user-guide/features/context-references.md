<!-- source: website/docs/guia-usuario/features/contexto-references.md -->
# Contexto References

# Contexto References

Type `@` followed by a reference to inject content directly into your message. Hermes expands the reference inline and appends the content under an `--- Attached Contexto ---` section.
...
Contexto references are bounded to prevent overwhelming the modelo's contexto window:

| Threshold | Value | Behavior |
|-----------|-------|----------|
| Soft limit | 25% of contexto length | Warning appended, expansion proceeds |
| Hard limit | 50% of contexto length | Expansion refused, original message returned unchanged |
...
Contexto references are primarily a **CLI feature**. They work in the interactive CLI where `@` triggers tab completion and references are expanded before the message is sent to the agente.

In **messaging platforms** (Telegram, Discord, etc.), the `@` syntax is not expanded by the puerta de enlace — messages are passed through as-is. The agente itself can still reference files via the `read_file`, `search_files`, and `web_extract` herramientas.

## Interaction with Contexto Compression

When conversation contexto is compressed, the expanded reference content is included in the compression summary. This means:

- Large file contents injected via `@file:` contribute to contexto usage
