<!-- source: website/docs/guia-usuario/features/vision.md -->
# guia-usuario/features/vision

# Vision & Image Paste

Hermes Agente supports **multimodal vision** — you can paste images from your clipboard directly into the CLI and ask the agente to analyze, describe, or work with them. Images are sent to the modelo as base64-encoded content blocks, so any vision-capable modelo can process them.

:::tip
Portal subscribers get vision-capable modelos (Claude, GPT-5, Gemini) in the same catalog — no extra credentials needed. See [Nous Portal](/integrations/nous-portal).
:::
...
5. On submit, the image is sent to the modelo as a vision content block
...
2. **Use a URL** — If the image is accessible online, just paste the URL in your message. The agente can use `vision_analyze` to look at any image URL directly.
...
This is why Hermes uses a separate clipboard check — instead of receiving image data through the terminal paste event, it calls OS-level herramientas (`osascript`, `powershell.exe`, `xclip`, `wl-paste`) directly via subprocess to read the clipboard independently.

## Supported Modelos

Image paste works with any vision-capable modelo. The image is sent as a base64-encoded data URL in the OpenAI vision content format:
...
Most modern modelos support this format, including GPT-4 Vision, Claude (with vision), Gemini, and open-source multimodal modelos served through OpenRouter.

## Image Routing (Vision-Capable vs Text-Only Modelos)

When a user attaches an image — from the CLI clipboard, the puerta de enlace (Telegram/Discord photo), or any other entry point — Hermes routes it based on whether your current modelo actually supports vision:

| Your modelo | What happens to the image |
|---|---|
| **Vision-capable** (GPT-4V, Claude with vision, Gemini, Qwen-VL, MiMo-VL, etc.) | Sent as **real pixels** using the proveedor's native image content format above. No text summary layer. |
| **Text-only** (DeepSeek V3, smaller open-source modelos, older chat-only endpoints) | Routed through the `vision_analyze` auxiliary herramienta — an auxiliary vision modelo describes the image, and the text description is injected into the conversation. |

You don't configure this — Hermes looks up your current modelo's capability in the proveedor metadata and picks the right path automatically. The practical effect: you can switch between vision and non-vision modelos mid-session and image handling "just works" without changing your workflow. Text-only modelos get coherent contexto about the image rather than a broken multimodal payload they'd have to reject.

Which auxiliary modelo handles the text-description path is configurable under `auxiliary.vision` — see [Auxiliary Modelos](/guia-usuario/configuración#auxiliary-modelos).

### `vision_analyze` has the same dual behavior

The `vision_analyze` herramienta itself follows the same routing. When the active main modelo is vision-capable **and** its proveedor supports image content inside herramienta results (currently the Anthropic, OpenAI, Azure-OpenAI, and Gemini 3.x stacks), `vision_analyze` short-circuits the auxiliary describer and returns the raw image pixels as a multimodal herramienta-result envelope. The main modelo sees the image natively on its next turn — no aux call, no text-summary information loss, no extra latency.

For text-only main modelos (or proveedors whose herramienta-result channel doesn't carry images), `vision_analyze` falls back to the legacy path: it asks the configured auxiliary vision modelo to describe the image and returns the description as plain text. Either way the calling herramienta signature is the same — the herramienta decides which path to take at runtime based on the active modelo.
