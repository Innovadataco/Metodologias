<!-- source: website/docs/guia-usuario/features/spotify.md -->
...
- A Spotify account. **Free** works for search, playlist, library, and activity herramientas. **Premium** is required for playback control (play, pause, skip, seek, volume, queue add, transfer).
- Hermes Agente installed and running.
- For playback herramientas: an **active Spotify Connect device** — the Spotify app must be open on at least one device (phone, desktop, web player, speaker) so the Web API has something to control. If nothing is active you'll get a `403 Forbidden` with a "no active device" message; open Spotify on any device and retry.

## Setup

### One-shot: `hermes herramientas` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `hermes setup` / `hermes setup herramientas` flow. Spotify stays opt-in, so enabling it there runs the same proveedor-aware configuración as `hermes herramientas`.

Hermes drops you straight into the OAuth flow — if you don't have a Spotify app yet, it walks you through creating one inline. Once you finish, the herramientaset is enabled AND authenticated in one pass.

If you prefer to do the steps separately (or you're re-authing later), use the two-step flow below.

### Two-step flow

#### 1. Enable the herramientaset

```bash
hermes herramientas
```

Toggle `🎵 Spotify` on, save, and when the inline wizard opens, dismiss it (Ctrl+C). The herramientaset stays on; only the auth step is deferred.
...
The 7 Spotify herramientas only appear in the agente's herramientaset after step 1 — they're off by default so users who don't want them don't ship extra herramienta schemas on every API call.

If no `HERMES_SPOTIFY_CLIENT_ID` is set, Hermes walks you through the app registration inline:

1. Opens `https://developer.spotify.com/panel` in your browser
...
After you approve, tokens are written under `proveedors.spotify` in `~/.hermes/auth.json`. The active inference proveedor is NOT changed — Spotify auth is independent of your LLM proveedor.

### Creating the Spotify app (what the wizard asks for)

When the panel opens, click **Create app** and fill in:

| Field | Value |
|-------|-------|
| App name | anything (e.g. `hermes-agente`) |
...
Once logged in, the agente has access to 7 Spotify herramientas. You talk to the agente naturally — it picks the right herramienta and action. For the best behavior, the agente loads a companion habilidad that teaches canonical usage patterns (single-search-then-play, when not to preflight `get_state`, etc.).
...
| `play` | Start/resume playback. Optional: `contexto_uri`, `uris`, `offset`, `position_ms` | Yes |
| `pause` | Pause playback | Yes |
| `next` / `previous` | Skip track | Yes |
| `seek` | Jump to `position_ms` | Yes |
| `set_repeat` | `state` = `track` / `contexto` / `off` | Yes |
...
Read-only herramientas work on Free accounts. Anything that mutates playback or the queue requires Premium.
...
Because Spotify herramientas are regular Hermes herramientas, a cron job running in a Hermes session can trigger playback on any schedule. No new code needed.
...
2. Agente reads the prompt, calls `spotify_devices list` to find "kitchen speaker" by name, then `spotify_devices transfer` → `spotify_playback set_volume` → `spotify_playback set_shuffle` → `spotify_search` + `spotify_playback play`.
3. Music starts on the target speaker. Total cost: one session, a few herramienta calls, no human input.
...
- **The cron agente inherits your active herramientasets.** Spotify must be enabled in `hermes herramientas` for the cron session to see the Spotify herramientas.
- **Cron jobs run with `skip_memoria=True`** so they don't write to your memoria store.
...
**Wizard doesn't open the browser** — If you're over SSH or in a container without a display, Hermes detects it and skips the auto-open. Copy the panel URL it prints and open it manually.

## Advanced: custom scopes

By default Hermes requests the scopes needed for every shipped herramienta. Override if you want to restrict access:

```bash
hermes auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```

Scope reference: [Spotify Web API scopes](https://developer.spotify.com/documentation/web-api/concepts/scopes). If you request fewer scopes than a herramienta needs, that herramienta's calls will fail with 403.
...
| File | Contents |
|------|----------|
| `~/.hermes/auth.json` → `proveedors.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.hermes/.env` | `HERMES_SPOTIFY_CLIENT_ID`, optional `HERMES_SPOTIFY_REDIRECT_URI` |
| Spotify app | owned by you at [developer.spotify.com/panel](https://developer.spotify.com/panel); contains the Client ID and the redirect URI allow-list |
