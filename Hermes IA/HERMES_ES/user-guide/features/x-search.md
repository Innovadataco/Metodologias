<!-- source: website/docs/guia-usuario/features/x-search.md -->
# guia-usuario/features/x-search.md

# X (Twitter) Search

The `x_search` herramienta lets the agente search X (Twitter) posts, perfils, and threads directly. It's backed by xAI's built-in `x_search` herramienta on the Responses API at `https://api.x.ai/v1/responses` — Grok itself runs the search server-side and returns synthesized results with citations to the originating posts.
...
:::tip
If you're paying Portal for an xAI modelo anyway, Live Search calls bill against the same xAI key configured for chat. See [Nous Portal](/integrations/nous-portal).
:::
...
The herramienta's `check_fn` runs the xAI credential resolver every time the modelo's herramienta list is rebuilt. A `True` return means the bearer is fetchable AND non-empty AND (if it had expired) successfully refreshed. Revoked tokens with a failed refresh hide the herramienta from the schema; the modelo simply can't see it.

## Enabling the herramienta

Auto-enables when xAI credentials (OAuth token or `XAI_API_KEY`) are present. Disable explicitly via `hermes herramientas` → Search → x_search if you don't want this.
...
Either choice satisfies the gating. You can pick whichever credentials you already have; the herramienta works identically with both. If both end up configured, OAuth is preferred at call time.
...
# ~/.hermes/config.yaml
x_search:
  # xAI modelo used for the Responses call.
  # grok-4.20-reasoning is the recommended default; any Grok modelo
  # with x_search herramienta access works.
...
The agente calls `x_search` with these arguments:
...
The herramienta returns JSON with:
...
- `degraded` — `true` when any narrowing filter (`allowed_x_handles`, `excluded_x_handles`, `from_date`, `to_date`) was set AND both citation channels came back empty. In that case the `answer` was synthesized from the modelo's own knowledge rather than the X index, so treat it as unsourced. `false` otherwise (including the "no filters set" case — a broad unsourced answer is just an answer, not a filter miss)
...
- `modelo`, `query`, `proveedor`, `herramienta`, `success`
...
Validation failures surface as a structured `{"error": "..."}` herramienta result, never as an HTTP call to xAI.

## Example

Talking to the agente:
...
The herramienta surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.hermes/.env` or run `hermes auth add xai-oauth` and complete the browser login. Then restart your session so the agente re-reads the herramienta registry.

### "`x_search` is not enabled for this modelo"

The configured `x_search.modelo` doesn't have access to the server-side `x_search` herramienta. Switch to `grok-4.20-reasoning` (the default) or another Grok modelo that supports it. Check the [xAI documentation](https://docs.x.ai/) for the current list.

### Herramienta doesn't appear in the schema

Two possible causes:

1. **Herramientaset not enabled.** Run `hermes herramientas` and confirm `🐦 X (Twitter) Search` is checked.
...
- **xAI index gap.** Some active accounts intermittently fail to surface in `x_search` even when they post regularly. Retry after a few minutes, or use the `xurl` habilidad for direct X API reads when you need an exact handle's timeline.
...
- [Herramientas Reference](../../reference/herramientas-reference.md) — full herramienta catalog
