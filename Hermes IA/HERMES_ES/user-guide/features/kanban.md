<!-- source: website/docs/guia-usuario/features/kanban.md -->
# Kanban (Multi-Agente Board)

# Kanban — Multi-Agente Perfil Collaboration

> **Want a walkthrough?** Read the [Kanban tutorial](./kanban-tutorial) — four user stories (solo dev, fleet farming, role pipeline with retry, circuit breaker) with panel screenshots of each. This page is the reference; the tutorial is the narrative.

Hermes Kanban is a durable task board, shared across all your Hermes perfils, that lets multiple named agentes collaborate on work without fragile in-process subagente swarms. Every task is a row in `~/.hermes/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the modelo talks through herramientas, you talk through the CLI

The board has two front doors, both backed by the same `~/.hermes/kanban.db`:

- **Agentes drive the board through a dedicated `kanban_*` herramientaset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these herramientas already in its schema; orchestrator perfils can also enable the `kanban` herramientaset explicitly. The modelo reads and routes tasks by calling herramientas directly, *not* by shelling out to `hermes kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `hermes kanban …`** on the CLI, `/kanban …` as a slash command, or the panel. These are for humans and automation — the places without a herramienta-calling modelo behind them.

Both surfaces route through the same `kanban_db` layer, so reads see a consistent view and writes can't drift. The rest of this page shows CLI examples because they're easy to copy-paste, but every CLI verb has a herramienta-call equivalent the modelo uses.
...
- **Digital twins** — persistent named assistants (`inbox-triage`, `ops-review`) that accumulate memoria over time.
...
| Child identity | Anonymous subagente | Named perfil with persistent memoria |
...
| Agentes per task | One call = one subagente | N agentes over task's life (retry, review, follow-up) |
| Audit trail | Lost on contexto compression | Durable rows in SQLite forever |
| Coordination | Hierarchical (caller → callee) | Peer — any perfil reads/writes any task |

**One-sentence distinction:** `delegate_task` is a function call; Kanban is a work queue where every handoff is a row any perfil (or human) can see and edit.

**Use `delegate_task` when** the parent agente needs a short reasoning answer before continuing, no humans involved, result goes back into the parent's contexto.

**Use Kanban when** work crosses agente boundaries, needs to survive restarts, might need human input, might be picked up by a different role, or needs to be discoverable after the fact.
...
- **Task** — a row with title, optional body, one assignee (a perfil name), status (`triage | todo | ready | running | blocked | done | archived`), optional tenant namespace, optional idempotency key (dedup for retried automation).
- **Link** — `task_links` row recording a parent → child dependency. The dispatcher promotes `todo → ready` when all parents are `done`.
- **Comment** — the inter-agente protocol. Agentes and humans append comments; when a worker is (re-)spawned it reads the full comment thread as part of its contexto.
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `dir:<path>` — an existing shared directory (Obsidian vault, mail ops dir, per-account folder). **Must be an absolute path.** Relative paths like `dir:../tenants/foo/` are rejected at dispatch because they'd resolve against whatever CWD the dispatcher happens to be in, which is ambiguous and a confused-deputy escape vector. The path is otherwise trusted — it's your box, your filesystem, the worker runs with your uid. This is the trusted-local-user threat modelo; kanban is single-host by design. **Preserved on completion.**
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned perfils. Runs **inside the puerta de enlace** by default (`kanban.dispatch_in_puerta de enlace: true`). One dispatcher sweeps all boards per tick; workers are spawned with `HERMES_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose perfil doesn't exist, workspace can't mount, etc.
- **Tenant** — optional string namespace *within* a board. One specialist fleet can serve multiple businesses (`--tenant business-a`) with data isolation by workspace path and memoria key prefix. Tenants are a soft filter; boards are the hard isolation boundary.
...
- Workers spawned for a task see **only** their board's tasks — the
  ...
  `kanban_*` herramienta the worker has access to reads it.
...
### Managing boards from the panel

`hermes panel` → Kanban tab shows a board switcher at the top as soon
...
All panel API endpoints accept `?board=<slug>` for board scoping. The
...
- **Upload** — open a task in the panel drawer and use the
...
- **What the worker sees** — when the dispatcher hands a task to a worker,
  the worker's contexto includes an **Attachments** section listing each
  file's name and its **absolute path**. The worker has full file/terminal
  herramienta access, so it reads attachments directly (`read_file`, or shell
  herramientas like `pdftotext`).
...
sandbox so the absolute paths in the worker contexto are reachable.
:::


## Quick start

The commands below are **you** (the human) setting up the board and creating tasks. Once a task is assigned, the dispatcher spawns the assigned perfil as a worker, and from there **the modelo drives the task through `kanban_*` herramienta calls, not CLI commands** — see [How workers interact with the board](#how-workers-interact-with-the-board).

```bash
# 1. Create the board (you)
hermes kanban init

# 2. Start the puerta de enlace (hosts the embedded dispatcher)
hermes puerta de enlace start

# 3. Create a task (you — or an orchestrator agente via kanban_create)
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` perfil, the very first thing that worker's modelo does is call `kanban_show()` to read its task. It doesn't run `hermes kanban show t_abcd`.

### Puerta de enlace-embedded dispatcher (default)

The dispatcher runs inside the puerta de enlace process. Nothing to install, no
separate service to manage — if the puerta de enlace is up, ready tasks get picked
up on the next tick (60s by default).

```yaml
# config.yaml
kanban:
  dispatch_in_puerta de enlace: true        # default
  dispatch_interval_seconds: 60    # default
```

Override the config flag at runtime via `HERMES_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard puerta de enlace supervision applies: run `hermes puerta de enlace
start` directly, or wire the puerta de enlace up as a systemd user unit (see the
puerta de enlace docs). Without a running puerta de enlace, `ready` tasks stay where they are
...
use the puerta de enlace. If you truly cannot run the puerta de enlace (headless host
...
a puerta de enlace-embedded dispatcher AND a standalone daemon against the same
...
**Workers do not shell out to `hermes kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban herramientaset** in the modelo's schema. The same herramientaset is also available to orchestrator perfils that enable `kanban` in their herramientasets config. These herramientas read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other herramienta; it never sees or needs the `hermes kanban` CLI.

| Herramienta | Purpose | Required params |
|---|---|---|
| `kanban_show` | Read the current task (title, body, prior attempts, parent handoffs, comments, full pre-formatted `worker_contexto`). Defaults to the env's task id. | — |
...
| `kanban_create` | (Orchestrators) fan out into child tasks with an `assignee`, optional `parents`, `habilidads`, etc. | `title`, `assignee` |
...
```
# Modelo's herramienta calls, in order:
kanban_show()                                     # no args — uses HERMES_KANBAN_TASK
# (modelo reads the returned worker_contexto, does the work via terminal/file herramientas)
...
The "(Orchestrators)" herramientas — `kanban_list`, `kanban_create`, `kanban_link`, `kanban_unblock`, and `kanban_comment` on foreign tasks — are available through the same herramientaset; the convention (encoded in the auto-injected kanban guidance) is that worker perfils don't fan out or route unrelated work, and orchestrator perfils don't execute implementation work. Dispatcher-spawned workers are still task-scoped for destructive lifecycle operations and cannot mutate unrelated tasks.

### Why herramientas instead of shelling to `hermes kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal herramienta points at a remote backend (Docker / Modal / Singularity / SSH) would run `hermes kanban complete` *inside* the container, where `hermes` isn't installed and `~/.hermes/kanban.db` isn't mounted. The kanban herramientas run in the agente's own Python process and always reach `~/.hermes/kanban.db` regardless of terminal backend.
2. **No shell-quoting fragility.** Passing `--metadata '{"files": [...]}'` through shlex + argparse is a latent footgun. Structured herramienta args skip it entirely.
3. **Better errors.** Herramienta results are structured JSON the modelo can reason about, not stderr strings it has to parse.

**Zero schema footprint on normal sessions.** A regular `hermes chat` session has zero `kanban_*` herramientas in its schema unless the active perfil explicitly enables the `kanban` herramientaset for orchestrator work. Dispatcher-spawned task workers get task-scoped herramientas because `HERMES_KANBAN_TASK` is set; orchestrator perfils get the broader routing surface through config. No herramienta bloat for users who never touch kanban.

The auto-injected kanban guidance teaches the modelo which herramienta to call when and in what order.
...
machine-readable handoff that downstream agentes, reviewers, or panels can
...
Every perfil that works kanban tasks automatically gets the worker lifecycle — it's injected into the worker's system prompt at spawn (the `KANBAN_GUIDANCE` block), so there is **nothing to install or configure**. It teaches the worker the full lifecycle in **herramienta calls**, not CLI commands:

1. On spawn, call `kanban_show()` to read title + body + parent handoffs + prior attempts + full comment thread.
2. `cd $HERMES_KANBAN_WORKSPACE` (via the terminal herramienta) and do the work there.
...
of respawning it into the same loop. This usually means the modelo wrote a
plain-text answer and exited without using the Kanban herramienta surface.

The lifecycle plus the load-bearing reference details (workspace kinds, deliverable `artifacts`, claiming created cards) ship in that system-prompt block, so every worker has them regardless of which perfil it runs under — no per-perfil habilidad setup required.

### Pinning extra habilidads to a specific task

Sometimes a single task needs specialist contexto the assignee perfil doesn't carry by default — a translation job that needs the `translation` habilidad, a review task that needs `github-code-review`, a security audit that needs `security-pr-audit`. Rather than editing the assignee's perfil every time, attach the habilidads directly to the task.

**From an orchestrator agente** (the usual case — one agente routing work to another), use the `kanban_create` herramienta's `habilidads` array:

```
kanban_create(
    title="translate README to Japanese",
    assignee="linguist",
    habilidads=["translation"],
)

kanban_create(
    title="audit auth flow",
    assignee="reviewer",
    habilidads=["security-pr-audit", "github-code-review"],
)
```

**From a human (CLI / slash command)**, repeat `--habilidad` for each one:

```bash
hermes kanban create "translate README to Japanese" \
    --assignee linguist \
    --habilidad translation

hermes kanban create "audit auth flow" \
    --assignee reviewer \
    --habilidad security-pr-audit \
    --habilidad github-code-review
```

**From the panel**, type the habilidads comma-separated into the **habilidads** field of the inline create form.

The dispatcher emits one `--habilidads <name>` flag per habilidad listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The habilidad names must match habilidads that are actually installed on the assignee's perfil (run `hermes habilidads list` to see what's available); there's no runtime install.

### Goal-mode cards (`--goal`)

By default each worker gets **one shot** at its card — do the work, call `kanban_complete`/`kanban_block`, exit. Pass `--goal` (CLI) or `goal_mode=True` (the `kanban_create` herramienta / panel) to instead run that worker in a **goal loop**, the same Ralph-style engine behind the `/goal` slash command: after every turn an auxiliary judge checks the worker's output against the card's title + body (treated as the acceptance criteria), and if the work isn't done — and the turn budget remains — the worker keeps going **in the same session** until the judge agrees, the worker terminates the task itself, or the budget runs out (which **blocks** the card for human review rather than exiting silently).
...
A **well-behaved orchestrator does not do the work itself.** It decomposes the user's goal into tasks, links them, assigns each to one of the perfils you've set up, and steps back. The orchestrator guidance — anti-temptation rules, a Step-0 perfil-discovery prompt (the dispatcher silently fails on unknown assignee names, so the orchestrator must ground every card in perfils that actually exist on your machine), and a decomposition playbook keyed on `kanban_create` / `kanban_link` / `kanban_comment` — is injected into the worker's system prompt automatically; there is nothing to install.
...
The orchestrator guidance ships in the worker's system prompt automatically — there is nothing to install or sync per perfil.

For best results, pair it with a perfil whose herramientasets are restricted to board operations (`kanban`, `puerta de enlace`, `memoria`) so the orchestrator literally cannot execute implementation tasks even if it tries.

## Panel (GUI)

The `/kanban` CLI and slash command are enough to run the board headlessly, but a visual board is often the right interface for humans-in-the-loop: triage, cross-perfil supervision, reading comment threads, and dragging cards between columns. Hermes ships this as a **bundled panel complemento** at `complementos/kanban/` — not a core feature, not a separate service — following the modelo laid out in [Extending the Panel](./extending-the-panel).

Open it with:

```bash
hermes kanban init      # one-time: create kanban.db if not already present
hermes panel        # "Kanban" tab appears in the nav, after "Habilidads"
```

### What the complemento gives you

- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas. By default (`kanban.auto_decompose: true`), the dispatcher auto-runs the **decomposer** on tasks that land here. The built-in decomposer uses the `auxiliary.kanban_decomposer` modelo path, reads your perfil roster (with descriptions), and fans the task out into a small graph of child tasks routed to the best-fit specialists. The original task stays alive as the parent of every child so its assignee (`kanban.orchestrator_perfil`, or the active default perfil when unset) wakes back up to judge completion when everything finishes. Flip the **Orchestration: Auto/Manual** pill at the top of the page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `hermes kanban specify` - that's still available as a single-task spec rewrite when you don't want fan-out.
- Cards show the task id, title, priority badge, tenant tag, assigned perfil, comment/link counts, a **progress pill** (`N/M` children done when the task has dependents), and "created N ago". A per-card checkbox enables multi-select.
- **Per-perfil lanes inside Running** — herramientabar checkbox toggles sub-grouping of the Running column by assignee.
- **Live updates via WebSocket** — the complemento tails the append-only `task_events` table on a short poll interval; the board reflects changes the instant any perfil (CLI, puerta de enlace, or another panel tab) acts. Reloads are debounced so a burst of events triggers a single refetch.
- **Drag-drop** cards between columns to change status. The drop sends `PATCH /api/complementos/kanban/tasks/:id` which routes through the same `kanban_db` code the CLI uses — the three surfaces can never drift. Moves into destructive statuses (`done`, `archived`, `blocked`) prompt for confirmation. Touch devices use a pointer-based fallback so the board is usable from a tablet.
...
- **Multi-select with bulk actions** — shift/ctrl-click a card or tick its checkbox to add it to the selection. A bulk action bar appears at the top with batch status transitions, archive, and reassign (by perfil dropdown, or "(unassign)"). Destructive batches confirm first. Per-id partial failures are reported without aborting the rest.
- **Click a card** (without shift/ctrl) to open a side drawer (Escape or click-outside closes) with:
  ...
  - **Status action row** (→ triage / → ready / → running / block / unblock / complete / archive) with confirm prompts for destructive transitions. For cards in the **Triage** column the row also exposes two LLM-driven actions: **⚗ Decompose** fans the task out into a graph of child tasks routed to specialist perfils by description, and **✨ Specify** does a single-task spec rewrite. Decompose falls back to specify-style promotion when the LLM decides the task doesn't benefit from fan-out, so it's a strict superset. Both are reachable from the CLI (`hermes kanban decompose <id>` / `specify <id>` / `--all`), from any puerta de enlace platform (`/kanban decompose <id>`), and programmatically via `POST /api/complementos/kanban/tasks/:id/decompose` and `…/specify`. Configure the modelos under `auxiliary.kanban_decomposer` and `auxiliary.triage_specifier` in `config.yaml`.
  - Result section (also markdown-rendered), comment thread with Enter-to-submit, the last 20 events.
- **Herramientabar filters** — free-text search, tenant dropdown (defaults to `panel.kanban.default_tenant` from `config.yaml`), assignee dropdown, "show archived" toggle, "lanes by perfil" toggle, and a **Nudge dispatcher** button so you don't have to wait for the next 60 s tick.

Visually the target is the familiar Linear / Fusion layout: dark theme, column headers with counts, coloured status dots, pill chips for priority and tenant. The complemento reads only theme CSS vars (`--color-*`, `--radius`, `--font-mono`, ...), so it reskins automatically with whichever panel theme is active.

### Auto vs Manual orchestration

The kanban board has two ways to handle a task you drop into the Triage column:

**Auto (default)** — `kanban.auto_decompose: true`. The puerta de enlace-embedded dispatcher runs the **decomposer** on each tick, capped by `kanban.auto_decompose_per_tick` (default 3 tasks per tick) so a bulk-load of triage tasks doesn't burst-spend the auxiliary LLM. The decomposer uses the built-in decomposition prompt plus the `auxiliary.kanban_decomposer` modelo path, reads your installed perfils + their descriptions, and asks the LLM to produce a JSON task graph: which tasks to spawn, who they go to, and which depend on which. The original triage task becomes the parent of every leaf in the graph, so it stays alive until the whole graph completes - and then promotes back to `ready` so its assignee (`kanban.orchestrator_perfil`, or the active default perfil when unset) can judge completion and add more tasks if the work isn't done. This is the "drop a one-liner, walk away" flow.
...
The decomposer's routing decisions depend on perfil descriptions, which is a per-perfil labeling primitive you set with `hermes perfil create --description "..."`, `hermes perfil describe <name> --text "..."`, `hermes perfil describe <name> --auto` (LLM-generates from the perfil's installed habilidads + modelo), or the panel's per-perfil editor in the expanded **Orchestration settings** panel. Perfils without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown perfil, the child gets routed to `kanban.default_assignee` (or the active default perfil if that's unset).

`kanban.orchestrator_perfil` does not load that perfil's prompt, habilidads, or custom logic into the decomposition call. It controls who owns the root/orchestration task after fan-out. To change the decomposer's modelo/proveedor, configure `auxiliary.kanban_decomposer`. To use a perfil's custom task-splitting logic instead of the built-in decomposer, switch to Manual mode and have that perfil create or decompose tasks explicitly.
...
| `orchestrator_perfil` | `""` | Perfil assigned to the root/orchestration task after decomposition. Empty = fall back to active default perfil. |
| `default_assignee` | `""` | Where a child task lands when the LLM picks an unknown perfil. Empty = fall back to active default. |
| `auto_subscribe_on_create` | `true` | When a worker calls `kanban_create` from inside a session with a persistent delivery channel (messaging puerta de enlace or TUI), the originating session is auto-subscribed to the new task's completion/block events. The dispatcher still drives the delivery — this only changes whether the caller's chat/key shows up in the notify-sub table. Set to `false` to require explicit `kanban_notify-subscribe` calls per task. |

And the two auxiliary LLM slots:

| Key | Purpose |
|---|---|
| `auxiliary.kanban_decomposer` | Modelo that produces the task graph (called by Decompose). Set `proveedor`/`modelo` to override the main chat modelo. |
| `auxiliary.perfil_describer` | Modelo that auto-generates perfil descriptions (called by `hermes perfil describe --auto`). |
...
│  complementos/kanban/       │     the CLI /kanban verbs use      │
│  panel/complemento_api.py                                    │
└──────────┬─────────────┘                                    │
           │                                                  │
           ▼                                                  │
┌────────────────────────┐                                    │
...
All routes are mounted under `/api/complementos/kanban/` and protected by the panel's ephemeral session token:
...
| `GET` | `/perfils` | List installed perfils with their descriptions (consumed by the panel's perfil-description editor and the orchestrator picker). |
| `PATCH` | `/perfils/:name` | Set or clear a perfil's description (user-authored — `description_auto: false`). Returns `{ok, perfil, description}`. |
| `POST` | `/perfils/:name/describe-auto` | Generate a description for a perfil via `auxiliary.perfil_describer`. Persists with `description_auto: true` so the panel can surface a "review" badge. |
| `GET` | `/orchestration` | Read the kanban orchestration settings (`orchestrator_perfil`, `default_assignee`, `auto_decompose`) plus the *resolved* effective values after fallbacks. |
| `PUT` | `/orchestration` | Update one or more of the three orchestration keys in `config.yaml`. Validates that non-empty perfil names actually exist. |
...
| `GET` | `/config` | Read `panel.kanban` preferences from `config.yaml` — `default_tenant`, `lane_by_perfil`, `include_archived_by_default`, `render_markdown` |
| `WS` | `/events?since=<event_id>` | Live stream of `task_events` rows |

Every handler is a thin wrapper — the complemento is ~700 lines of Python (router + WebSocket tail + bulk batcher + config reader) and adds no new business logic. A tiny `_conn()` helper auto-initializes `kanban.db` on every read and write, so a fresh install works whether the user opened the panel first, hit the REST API directly, or ran `hermes kanban init`.

### Panel config

Any of these keys under `panel.kanban` in `~/.hermes/config.yaml` changes the tab's defaults — the complemento reads them at load time via `GET /config`:

```yaml
panel:
  kanban:
    default_tenant: acme              # preselects the tenant filter
    lane_by_perfil: true             # default for the "lanes by perfil" toggle
...
The panel's HTTP auth middleware [explicitly skips `/api/complementos/`](./extending-the-panel#backend-api-routes) — complemento routes are unauthenticated by design because the panel binds to localhost by default. That means the kanban REST surface is reachable from any process on the host.

The WebSocket takes one additional step: it requires the panel's ephemeral session token as a `?token=…` query parameter (browsers can't set `Authorization` on an upgrade request), matching the pattern used by the in-browser PTY bridge.

If you run `hermes panel --host 0.0.0.0`, every complemento route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.hermes/kanban.db` are perfil-agnostic on purpose (that's the coordination primitive). If you open the panel with `hermes -p <perfil> panel`, the board still shows tasks created by any other perfil on the host. Same user owns all perfils, but this is worth knowing if multiple personas coexist.
...
The complemento uses the standard Hermes panel complemento contract — see [Extending the Panel](./extending-the-panel) for the full manifest reference, shell slots, page-scoped slots, and the Complemento SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this complemento.

To disable without removing: add `panel.complementos.kanban.enabled: false` to `config.yaml` (or delete `complementos/kanban/panel/manifest.json`).

### Scope boundary

The GUI is deliberately thin. Everything the complemento does is reachable from the CLI; the complemento just makes it comfortable for humans. Auto-assignment, budgets, governance gates, and org-chart views remain user-space — a router perfil, another complemento, or a reuse of `herramientas/approval.py` — exactly as listed in the out-of-scope section of the design spec.

## CLI command reference

This is the surface **you** (or scripts, cron, the panel) use to drive the board. Workers running inside the dispatcher use the `kanban_*` [herramienta surface](#how-workers-interact-with-the-board) for the same operations — the CLI here and the herramientas there both route through `kanban_db`, so the two surfaces agree by construction.

```
hermes kanban init                                     # create kanban.db + print daemon hint
hermes kanban create "<title>" [--body ...] [--assignee <perfil>]
...
hermes kanban assign <id> <perfil>                    # or 'none' to unassign
hermes kanban reassign <id>... <perfil>               # bulk re-assign tasks to a perfil
...
hermes kanban assignees [--json]                       # perfils on disk + per-assignee task counts
hermes kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
hermes kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `hermes puerta de enlace start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
hermes kanban stats [--json]                           # per-status + per-assignee counts
hermes kanban log <id> [--tail BYTES]                  # worker log from ~/.hermes/kanban/logs/
hermes kanban notify-subscribe <id>                    # puerta de enlace bridge hook (used by /kanban in the puerta de enlace)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
hermes kanban notify-list [<id>] [--json]
hermes kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
hermes kanban contexto <id>                             # what a worker sees
...
All commands are also available as a slash command in the interactive CLI and in the messaging puerta de enlace (see [`/kanban` slash command](#kanban-slash-command) below).
...
| `kanban.max_in_progress_per_perfil` | unset (unlimited) | Per-perfil variant of `max_in_progress` — caps how many tasks any single assignee perfil may run concurrently. Useful when one perfil is slow or rate-limited but others should keep flowing. Applies alongside the board-wide `max_in_progress`; both must allow a spawn for it to proceed. |
...
### Drag-to-delete and bulk delete (panel)

The panel exposes a **trash drop zone** on the kanban page — drag any card into it to delete the task (cascades through `task_events`, child links, and subscriptions). A confirmation prompt protects against accidents. Bulk delete is also reachable via `DELETE /api/complementos/kanban/tasks` with a JSON body `{"ids": ["t_abc", "t_def", ...]}`.

### Worker visibility endpoints

The panel complemento API now exposes these read-only endpoints (plus a run-control verb) for external monitors:

| Endpoint | Returns |
|----------|---------|
| `GET /api/complementos/kanban/workers/active` | Currently spawned workers with PID, perfil, task id, started-at, last heartbeat |
| `GET /api/complementos/kanban/runs/{id}` | Single-run detail — task id, status, started/ended, exit code, log path |
| `POST /api/complementos/kanban/runs/{run_id}/terminate` | Terminate a reclaimable run — stops the worker and frees the task for re-dispatch |
| `GET /api/complementos/kanban/inspect` | Combined dispatcher snapshot — backlog, in-progress count vs. `max_in_progress`, recent events |

All of these are gated by the same panel complemento auth as the rest of the kanban complemento API.

### Kanban Swarm topology helper

`hermes kanban swarm` creates a durable **Kanban Swarm v1** graph in one shot: a completed root/blackboard card, N parallel worker cards, a verifier card gated on all workers, and a synthesizer card gated on the verifier. Shared swarm contexto (the "blackboard") is stored as structured JSON comments on the root card so any worker can read it.
...
### Mid-run usage: `/kanban` bypasses the running-agente guard

The puerta de enlace normally queues slash commands and user messages while an agente is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.hermes/kanban.db`, not in the running agente's state, so reads (`list`, `show`, `contexto`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
- You spot a card that needs human contexto → `/kanban comment t_xyz "use the 2026 schema, not 2025"` lands on the task thread and the *next* run of that task will read it in `kanban_show()`.
...
### Auto-subscribe on `/kanban create` (puerta de enlace only)

When you create a task from the puerta de enlace with `/kanban create "…"`, the originating chat (platform + chat id + thread id) is automatically subscribed to that task's terminal events (`completed`, `blocked`, `gave_up`, `crashed`, `timed_out`). You'll get one message back per terminal event — including the first line of the worker's result summary on `completed` — without having to poll or remember the task id.
...
Puerta de enlace platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`hermes kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.

### Autocomplete

In the interactive CLI, typing `/kanban ` and hitting Tab cycles through the built-in subcommand list (`list`, `ls`, `show`, `create`, `assign`, `link`, `unlink`, `claim`, `comment`, `complete`, `block`, `unblock`, `archive`, `tail`, `dispatch`, `contexto`, `init`, `gc`). The remaining verbs listed in the CLI reference above (`watch`, `stats`, `runs`, `log`, `assignees`, `heartbeat`, `notify-subscribe`, `notify-list`, `notify-unsubscribe`, `daemon`) also work — they're just not in the autocomplete hint list yet.
...
| **P4 Long-running journal** | same perfil + shared dir + cron | Obsidian vault |
...
| **P7 Thread-scoped workspace** | `/kanban here` in a thread | per-project puerta de enlace threads |
| **P8 Fleet farming** | one perfil, N subjects | 50 social accounts |
...
Workers receive `$HERMES_TENANT` and namespace their memoria writes by prefix. The board, the dispatcher, and the perfil definitions are all shared; only the data is scoped.

## Puerta de enlace notifications

When you run `/kanban create …` from the puerta de enlace (Telegram, Discord, Slack, etc.), the originating chat is automatically subscribed to the new task. The puerta de enlace's background notifier polls `task_events` every few seconds and delivers one message per terminal event (`completed`, `blocked`, `gave_up`, `crashed`, `timed_out`) to that chat. Completed tasks also send the first line of the worker's `--result` so you see the outcome without having to `/kanban show`.
...
- `summary` (herramienta param) / `--summary` (CLI) — human handoff; goes on the run; downstream children see it in their `build_worker_contexto`.
- `metadata` (herramienta param) / `--metadata` (CLI) — free-form JSON dict on the run; children see it serialized alongside the summary.
- `result` (herramienta param) / `--result` (CLI) — short log line that goes on the task row (legacy field, kept for back-compat).
...
```
# What a worker actually does — a herramienta call, from inside the agente loop:
kanban_complete(
...
The same handoff is reachable from the CLI when you (the human) need to close out a task a worker can't — e.g. a task that was abandoned, or one you marked done manually from the panel:
...
Runs are exposed on the panel (Run History section in the drawer, one coloured row per attempt) and on the REST API (`GET /api/complementos/kanban/tasks/:id` returns a `runs[]` array). `PATCH /api/complementos/kanban/tasks/:id` with `{status: "done", summary, metadata}` forwards both to the kernel, so the panel's "mark done" button is CLI-equivalent. `task_events` rows carry the `run_id` they belong to so the UI can group them by attempt, and the `completed` event embeds the first-line summary in its payload (capped at 400 chars) so puerta de enlace notifiers can render structured handoffs without a second SQL round-trip.
...
**Reclaimed runs from status changes.** If you drag a running task off `running` in the panel (back to `ready`, or straight to `todo`), or archive a task that was still running, the in-flight run closes with `outcome='reclaimed'` rather than being orphaned. The `task_runs` row is always in a terminal state when `tasks.current_run_id` is `NULL`, and vice versa — that invariant holds across CLI, panel, dispatcher, and notifier.

**Synthetic runs for never-claimed completions.** Completing or blocking a task that was never claimed (e.g. a human closes a `ready` task from the panel with a summary, or a CLI user runs `hermes kanban complete <ready-task> --summary X`) would otherwise drop the handoff. Instead the kernel inserts a zero-duration run row (`started_at == ended_at`) carrying the summary / metadata / reason so attempt history stays complete. The `completed` / `blocked` event's `run_id` points at that row.

**Live drawer refresh.** When the panel's WebSocket event stream reports new events for the task the user is currently viewing, the drawer reloads itself (via a per-task event counter threaded into its `useEffect` dependency list). Closing and reopening is no longer required to see a run's new row or updated outcome.
...
| `status` | `{status}` | Panel drag-drop wrote a status directly (e.g. `todo → ready`). Carries the `run_id` of the run that was reclaimed when dragging off `running`; otherwise `run_id` is NULL. |
