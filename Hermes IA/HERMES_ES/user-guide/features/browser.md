<!-- source: website/docs/guia-usuario/features/browser.md -->
# guia-usuario/features/browser

# Browser Automation

Hermes Agente includes a full browser automation herramientaset with multiple backend options:

- **Browserbase cloud mode** via [Browserbase](https://browserbase.com) for managed cloud browsers and anti-bot herramientaing
- **Browser Use cloud mode** via [Browser Use](https://browser-use.com) as an alternative cloud browser proveedor
...
- **Local Chromium-family CDP** — connect browser herramientas to your own Chrome, Brave, Chromium, or Edge instance using `/browser connect`
- **Local browser mode** via the `agente-browser` CLI and a local Chromium installation

In all modes, the agente can navigate websites, interact with page elements, fill forms, and extract information.

## Descripción General

Pages are represented as **accessibility trees** (text-based snapshots), making them ideal for LLM agentes. Interactive elements get ref IDs (like `@e1`, `@e2`) that the agente uses for clicking and typing.

Key capabilities:

- **Multi-proveedor cloud execution** — Browserbase, Browser Use, or Firecrawl — no local browser needed
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Herramienta Puerta de enlace](herramienta-puerta de enlace.md)** without any separate API keys. New installs can run `hermes setup --portal` to log in and turn on every puerta de enlace herramienta at once; existing installs can pick **Nous Subscription** as the browser proveedor via `hermes modelo` or `hermes herramientas`.
:::
...
To use Browser Use as your cloud browser proveedor, add:
...
To use Firecrawl as your cloud browser proveedor, add:
...
Get your API key at [firecrawl.dev](https://firecrawl.dev). Then select Firecrawl as your browser proveedor:

```bash
hermes setup herramientas
...
When a cloud proveedor is configured, Hermes auto-spawns a **local Chromium sidecar**
...
cloud proveedor in the same conversation.

This solves the common "I'm developing locally but using Browserbase" workflow —
the agente can screenshot your panel at `http://localhost:3000` AND scrape
`https://github.com` without you switching proveedors or disabling the SSRF guard.
The cloud proveedor never sees the private URL.

The feature is **on by default**. To disable it (all URLs go to the configured
cloud proveedor, as before):

```yaml
# ~/.hermes/config.yaml
browser:
  cloud_proveedor: browserbase
...
`browser.allow_private_urls: true` (which lets the cloud proveedor attempt them —
usually won't work since Browserbase etc. can't reach your LAN).

Requirements: the local sidecar uses the same `agente-browser` CLI as pure local
mode, so you need it installed (`hermes setup herramientas → Browser Automation`
...
`make up` starts the default container immediately. If you want custom runtime settings such as a larger Node heap, VNC, or a persistent perfil directory, build the image first and then run it yourself:
...
Or configure via `hermes herramientas` → Browser Automation → Camofox.

When `CAMOFOX_URL` is set, all browser herramientas automatically route through Camofox instead of Browserbase or agente-browser.

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agente restarts. To enable persistent browser sessions, add the following to `~/.hermes/config.yaml`:
...
##### What Hermes does
- Sends a deterministic perfil-scoped `userId` to Camofox so the server can reuse the same Firefox perfil across sessions.
- Skips server-side contexto destruction on cleanup, so cookies and logins survive between agente tasks.
- Scopes the `userId` to the active Hermes perfil, so different Hermes perfils get different browser perfils (perfil isolation).

##### What Hermes does not do
- It does not force persistence on the Camofox server. Hermes only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox perfil directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContexto()` without loading a stored perfil), Hermes cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based perfil persistence.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Hermes after editing `config.yaml`, and verify your Camofox server version supports persistent per-user perfils.

##### Where state lives

Hermes derives the stable `userId` from the perfil-scoped directory `~/.hermes/browser_auth/camofox/` (or the equivalent under `$HERMES_HOME` for non-default perfils). The actual browser perfil data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent perfil, clear it on the Camofox server and remove the corresponding Hermes perfil's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agente), configure Hermes to operate inside that same identity instead of spawning its own isolated perfil.
...
- Hermes skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/perfil survive.
...
1. On the first browser herramienta call after a process start, Hermes issues `GET /tabs?userId=<user_id>` (5-second timeout).
...
Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser herramienta call will surface a Camofox error — Hermes does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Hermes to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Hermes generates a per-task `session_key` (`task_<id>`) — Hermes will share cookies and the perfil with the external app, but will open its own tab alongside instead of reusing one.
...
When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Hermes automatically discovers this and includes the VNC URL in navigation responses, so the agente can share a link for you to watch the browser live.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud proveedor, you can attach Hermes browser herramientas to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevHerramientas Protocol (CDP). This is useful when you want to see what the agente is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the puerta de enlace. If you try to run it inside a WebUI, Telegram, Discord, or other puerta de enlace chat, the message will be sent to the agente as plain text and the command will not execute. Start Hermes from the terminal (`hermes` or `hermes chat`) and issue `/browser connect` there.
:::
...
:::tip
To start a Chromium-family browser manually with CDP enabled, use a dedicated user-data-dir so the debug port actually comes up even if the browser is already running with your normal perfil:
...
For that setup, prefer `chrome-devherramientas-mcp` through Hermes MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Hermes can still use the browser herramientas through a local Chromium install driven by `agente-browser`.
...
### Install agente-browser CLI

```bash
npm install -g agente-browser
# Or install locally in the repo:
npm install
```

:::info
The `browser` herramientaset must be included in your config's `herramientasets` list or enabled via `hermes config set herramientasets '["hermes-cli", "browser"]'`.
:::
...
Navigate to a URL. Must be called before any other browser herramienta. Initializes the Browserbase session.
...
:::tip
For simple information retrieval, prefer `web_search` or `web_extract` — they are faster and cheaper. Use browser herramientas when you need to **interact** with a page (click buttons, fill forms, handle dynamic content).
:::
...
```
Type "hermes agente" into the search field @e3
```
...
The screenshot is saved persistently and the file path is returned alongside the AI analysis. On messaging platforms (Telegram, Discord, Slack, WhatsApp), you can ask the agente to share the screenshot — it will be sent as a native photo attachment via the `MEDIA:` mechanism.
...
`browser_console` also evaluates JavaScript when called with an `expression` argument — same shape as DevHerramientas console, the result comes back parsed (JSON-serialized objects become dicts; primitive values stay primitive).
...
When a CDP supervisor is active for the current session (typical for any session that's run `browser_navigate` against a CDP-capable backend), evaluation runs over the supervisor's persistent WebSocket — no subprocess startup cost. Falls through to the standard agente-browser CLI path otherwise. Behaviour is identical either way; only latency changes.

### `browser_cdp`

Raw Chrome DevHerramientas Protocol passthrough — the escape hatch for browser operations not covered by the other herramientas. Use for native dialog handling, iframe-scoped evaluation, cookie/network control, or any CDP verb the agente needs.

**Only available when a CDP endpoint is reachable at session start** — meaning `/browser connect` has attached to a running Chrome, Brave, Chromium, or Edge browser, or `browser.cdp_url` is set in `config.yaml`. The default local agente-browser mode, Camofox, and cloud proveedors (Browserbase, Browser Use, Firecrawl) do not currently expose CDP to this herramienta — cloud proveedors have per-session CDP URLs but live-session routing is a follow-up.

**CDP method reference:** https://chromedevherramientas.github.io/devherramientas-protocol/ — the agente can `web_extract` a specific method's page to look up parameters and return shape.
...
Responds to a native JS dialog (`alert` / `confirm` / `prompt` / `beforeunload`). Before this herramienta existed, dialogs would silently block the page's JavaScript thread and subsequent `browser_*` calls would hang or throw; now the agente sees pending dialogs in `browser_snapshot` output and responds explicitly.
...
**Detection happens automatically** via a persistent CDP supervisor — one WebSocket per task that subscribes to Page/Runtime/Target events. The supervisor also populates a `frame_tree` field in the snapshot so the agente can see the iframe structure of the current page, including cross-origin (OOPIF) iframes.

**Availability matrix:**

| Backend | Detection via `pending_dialogs` | Response (`browser_dialog` herramienta) |
|---|---|---|
| Local Chrome via `/browser connect` or `browser.cdp_url` | ✓ | ✓ full workflow |
| Browserbase | ✓ | ✓ full workflow (via injected XHR bridge) |
| Camofox / default local agente-browser | ✗ | ✗ (no CDP endpoint) |

**How it works on Browserbase.** Browserbase's CDP proxy auto-dismisses real native dialogs server-side within ~10ms, so we can't use `Page.handleJavaScriptDialog`. The supervisor injects a small script via `Page.addScriptToEvaluateOnNewDocument` that overrides `window.alert`/`confirm`/`prompt` with a synchronous XHR. We intercept those XHRs via `Fetch.enable` — the page's JS thread stays blocked on the XHR until we call `Fetch.fulfillRequest` with the agente's response. `prompt()` return values round-trip back into page JS unchanged.

**Dialog policy** is configured in `config.yaml` under `browser.dialog_policy`:

| Policy | Behavior |
|--------|----------|
| `must_respond` (default) | Capture, surface in snapshot, wait for explicit `browser_dialog()` call. Safety auto-dismiss after `browser.dialog_timeout_s` (default 300s) so a buggy agente can't stall forever. |
| `auto_dismiss` | Capture, dismiss immediately. Agente still sees the dialog in `browser_state` history but doesn't have to act. |
| `auto_accept` | Capture, accept immediately. Useful when navigating pages with aggressive `beforeunload` prompts. |

**Frame tree** inside `browser_snapshot.frame_tree` is capped to 30 frames and OOPIF depth 2 to keep payloads bounded on ad-heavy pages. A `truncated: true` flag surfaces when limits were hit; agentes needing the full tree can use `browser_cdp` with `Page.getFrameTree`.
...
- **Session timeout** — cloud sessions expire based on your proveedor's plan settings
- **Cost** — cloud sessions consume proveedor credits; sessions are automatically cleaned up when the conversation ends or after inactivity. Use `/browser connect` for free local browsing.
