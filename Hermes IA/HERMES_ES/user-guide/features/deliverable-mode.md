<!-- source: website/docs/guia-usuario/features/deliverable-mode.md -->
# guia-usuario/features/deliverable-mode.md

# Deliverable Mode

When Hermes Agente runs inside a messaging puerta de enlace (Slack, Discord, Telegram,
...
download. A spreadsheet uploads as `.xlsx`. The agente does not need to
write a `MEDIA:` tag or do anything special — it just generates the file
and mentions its absolute path in the response. The puerta de enlace picks the path
...
1. **The agente has herramientas that produce files.** `execute_code` for charts via
   matplotlib, the `latex-pdf-report` habilidad for PDFs, the `powerpoint` habilidad
   for decks, `image_generate` for images, `text_to_speech` for audio, and so
   on.

2. **The puerta de enlace scans agente responses for file paths.** Any absolute path
...
3. **The puerta de enlace dispatches by file type.** Images embed inline where the
...
the agente doesn't auto-ship arbitrary source files; if you want to send code
to the user, use a code block.

## Encouraging the agente to produce artifacts

The agente doesn't reach for artifacts by default — it has to know to.
...
`.cursorrules` in a project the agente works from, to your global
persona in `~/.hermes/SOUL.md`, or as a named preset under
`agente.personalities` in `~/.hermes/config.yaml` (switchable per session
via `/personality`).

The mechanic the agente has to use is simple: render the file to an
absolute path (e.g. `/tmp/q3-revenue.png`) and mention that path as
plain text in the reply. The puerta de enlace does the rest. Paths inside
fenced code blocks or backticks are ignored so code samples are never
mutilated.

## Kanban: artifacts ride completion notifications

If you use Hermes' kanban multi-agente workflow, workers can attach
...
When the puerta de enlace notifier delivers the "task completed" message to whoever
...
Beyond the artifact-delivery pipeline, the agente can reach into other
services via MCP (Modelo Contexto Protocol). The MCP ecosystem ships
community servers for most popular herramientas — install whichever you need:
...
the agente generates a deliverable (chart, PDF, slide deck) and posts it
back into the thread as a native attachment. Hermes Agente's deliverable
