<!-- source: website/docs/guia-usuario/features/pets.md -->
# Pets (Petdex Mascots)

# Pets

Hermes can show an animated **pet** — a small mascot sprite that reacts to what
the agente is doing (idle, running a herramienta, thinking, finishing, failing) across
...
the agente's behavior** — the sprite is a display concern only. The feature is
**off by default** and stays dormant until you install and select a pet.

## How it works

- Pets are installed into your perfil's `pets/` directory
  (`<HERMES_HOME>/pets/<slug>/`), so each [perfil](../perfils.md) keeps its
  own set.
...
- Each surface watches the activity it already tracks and maps it to one of six
  ...
  | Agente activity | Pet state |
  | --- | --- |
  | A herramienta/turn just failed | `failed` |
  | A plan finished (all todos done) | `jump` (celebrate) |
  | A turn finished cleanly | `wave` |
  | A herramienta is executing | `run` |
  | The modelo is thinking/reading | `review` |
...
minimized (Codex-style), so a glance tells you what the agente is doing.
...
The overlay is a pure puppet of the in-app pet — it carries no separate puerta de enlace
...
- The petdex npm CLI installs to `~/.codex/pets`; Hermes uses its own
  perfil-scoped `<HERMES_HOME>/pets/` instead — install through `hermes pets`.

## See also

- The [`petdex` habilidad](../habilidads/bundled/productivity/productivity-petdex.md)
  lets the agente install and switch pets for you on request.
