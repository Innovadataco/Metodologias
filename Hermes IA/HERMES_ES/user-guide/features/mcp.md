<!-- source: website/docs/guia-usuario/features/mcp.md -->
# MCP (Modelo Contexto Protocol)

# MCP (Modelo Contexto Protocol)

MCP lets Hermes Agente connect to external herramienta servers so the agente can use herramientas that live outside Hermes itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Hermes to use a herramienta that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external herramienta ecosystems without writing a native Hermes herramienta first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic herramienta discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP herramientas you actually want Hermes to see
...
Hermes will discover the MCP server's herramientas and use them like any other herramienta.
...
github       installed (disabled)   GitHub repo + PR herramientas
```
...
`optional-mcps/` in the hermes-agente repo — presence in that directory means
...
- **OAuth** (third-party proveedor like Google/GitHub) — Hermes points you at
  `hermes auth <proveedor>` if you haven't authenticated already.

### Herramienta selection at install time

After credentials are configured, Hermes probes the MCP server to list every
herramienta it exposes and presents a checklist:

```
Select herramientas for 'linear' (SPACE toggle, ENTER confirm)
...
2. **The manifest's `herramientas.default_enabled`** if the entry declares one (some
   catalog entries pre-prune mutating or rarely-useful herramientas)
3. **Everything** if neither applies

Submit the checklist with ENTER. Only the checked herramientas end up in
`mcp_servers.<name>.herramientas.include`. If you select everything, no filter is
...
`herramientas.default_enabled` is applied directly (if declared), or no filter is
...
the hermes-agente repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/hermes-agente/tree/main/optional-mcps)
on GitHub. The picker also prints the manifest's `source:` URL at install
time so you can quickly verify the upstream repo. The web panel's MCP
...
### Updating herramienta selection later
...
when you want more herramientas enabled, or when the server has added new herramientas that
...
[`optional-mcps/`](https://github.com/NousResearch/hermes-agente/tree/main/optional-mcps).
...
**Pitfall — proveedors that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `herramientas/list` *without* auth, so `hermes mcp login` can list the herramientas and look like it worked, but every real herramienta call later times out. `hermes mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the proveedor's console and add it to config:
...
Remote HTTP MCP servers that require mutual TLS (client-certificate autenticación) are supported via `client_cert` / `client_key`. Hermes passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
| `timeout` | number | Herramienta call timeout |
| `connect_timeout` | number | Initial connection timeout |
| `enabled` | bool | If `false`, Hermes skips the server entirely |
| `supports_parallel_herramienta_calls` | bool | If `true`, herramientas from this server may run concurrently |
| `herramientas` | mapping | Per-server herramienta filtering and utility policy |
...
mcp_servers:
  filesystem:
    command: "npx"
    args: ["-y", "@modelocontextoprotocol/server-filesystem", "/tmp"]
```
...
## How Hermes registers MCP herramientas

Hermes prefixes MCP herramientas so they do not collide with built-in names:

```text
mcp_<server_name>_<herramienta_name>
```

Examples:

| Server | MCP herramienta | Registered name |
|---|---|---|
| `filesystem` | `read_file` | `mcp_filesystem_read_file` |
| `github` | `create-issue` | `mcp_github_create_issue` |
| `my-api` | `query.data` | `mcp_my_api_query_data` |

In practice, you usually do not need to call the prefixed name manually — Hermes sees the herramienta and chooses it during normal reasoning.

## MCP utility herramientas

When supported, Hermes also registers utility herramientas around MCP resources and prompts:
...
These utility herramientas are now capability-aware:
...
So a server that exposes callable herramientas but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which herramientas each MCP server contributes to Hermes, allowing fine-grained management of your herramienta namespace.
...
Only those MCP server herramientas are registered.

### Blacklist server herramientas
...
All server herramientas are registered except the excluded ones.
...
### Filter utility herramientas too
...
That means:
- `herramientas.resources: false` disables `list_resources` and `read_resource`
- `herramientas.prompts: false` disables `list_prompts` and `get_prompt`
...
If your config filters out all callable herramientas and disables or omits all supported utilities, Hermes does not create an empty runtime MCP herramientaset for that server.

That keeps the herramienta list clean.

## Runtime behavior

### Discovery time

Hermes discovers MCP servers at startup and registers their herramientas into the normal herramienta registry.

### Dynamic Herramienta Discovery

MCP servers can notify Hermes when their available herramientas change at runtime by sending a `notifications/herramientas/list_changed` notification. When Hermes receives this notification, it automatically re-fetches the server's herramienta list and updates the registry — no manual `/reload-mcp` required.

This is useful for MCP servers whose capabilities change dynamically (e.g. a server that adds herramientas when a new database schema is loaded, or removes herramientas when a service goes offline).
...
This reloads MCP servers from config and refreshes the available herramienta list. For runtime herramienta changes pushed by the server itself, see [Dynamic Herramienta Discovery](#dynamic-herramienta-discovery) above.

### Herramientasets

Each configured MCP server also creates a runtime herramientaset when it contributes at least one registered herramienta:

```text
mcp-<server>
```

That makes MCP servers easier to reason about at the herramientaset level.
...
- disable dangerous herramientas you do not want the modelo to see
...
mcp_servers:
  project_fs:
    command: "npx"
    args: ["-y", "@modelocontextoprotocol/server-filesystem", "/home/user/my-project"]
```
...
cd ~/.hermes/hermes-agente && uv pip install -e ".[mcp]"
...
### Herramientas not appearing

Possible causes:
- the server failed to connect
- discovery failed
- your filter config excluded the herramientas
...
This is intentional and keeps the herramienta list honest.

## Parallel Herramienta Calls

By default, MCP herramientas run sequentially — one at a time. If your MCP server exposes herramientas that are safe to run concurrently (e.g. read-only queries, independent API calls), you can opt-in to parallel execution:

```yaml
mcp_servers:
  docs:
    command: "docs-server"
    supports_parallel_herramienta_calls: true
```

When `supports_parallel_herramienta_calls` is `true`, Hermes may execute multiple herramientas from that server at the same time within a single herramienta-call batch, just like it does for built-in read-only herramientas (web_search, read_file, etc.).

:::caution
Only enable parallel calls for MCP servers whose herramientas are safe to run at the same time. If herramientas read and write shared state, files, databases, or external resources, review the read/write race conditions before enabling this setting.
:::

## MCP Sampling Support

MCP servers can request LLM inference from Hermes via the `sampling/createMessage` protocol. This allows an MCP server to ask Hermes to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own modelo access.
...
mcp_servers:
  my_server:
    ...
    sampling:
      ...
      modelo: "openai/gpt-4o"  # Override modelo for sampling requests (optional)
      ...
      max_herramienta_rounds: 5       # Max herramienta-use rounds in sampling loops (default: 5)
      allowed_modelos: []       # Allowlist of modelo names the server may request (empty = any)
      log_level: "info"        # Audit log level: debug, info, or warning (default: info)
```

The sampling handler includes a sliding-window rate limiter, per-request timeouts, and herramienta-loop depth limits to prevent runaway usage. Metrics (request count, errors, tokens used) are tracked per server instance.
...
In addition to connecting **to** MCP servers, Hermes can also **be** an MCP server. This lets other MCP-capable agentes (Claude Code, Cursor, Codex, or any MCP client) use Hermes's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agente to send and read Telegram/Discord/Slack messages through Hermes
- You want a single MCP server that bridges to all of Hermes's connected messaging platforms at once
- You already have a running Hermes puerta de enlace with connected platforms
...
```json
{
  "mcpServers": {
    "hermes": {
      "command": "/home/user/.hermes/hermes-agente/venv/bin/hermes",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 herramientas, matching OpenClaw's channel bridge surface plus a Hermes-specific channel browser:
...
The event queue is in-memoria and starts when the bridge connects. Older messages are available through `messages_read`.
...
The MCP server reads conversation data directly from Hermes's session store (`~/.hermes/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memoria event queue. For sending messages, it uses the same `send_message` infrastructure as the Hermes agente itself.

The puerta de enlace does NOT need to be running for read operations (listing conversations, reading history, polling events). It DOES need to be running for send operations, since the platform adapters need active connections.
