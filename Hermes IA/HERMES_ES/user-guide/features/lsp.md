<!-- source: website/docs/guia-usuario/features/lsp.md -->
...
lint check used by `write_file` and `patch`. When the agente edits a
...
This is the same architecture top-tier coding agentes use. Hermes
ships it self-contained: no editor host required, no complementos to
install, no separate daemon to manage.

## When LSP runs

LSP is gated on **git workspace detection**. When the agente's working
...
stays dormant — useful for messaging puerta de enlaces where the cwd is the
...
agente sees a syntax-clean file with semantic problems as
...
For "manual" entries, install the server through whatever herramientachain
...
removed when you reset the perfil.
...
install the binary manually through the language's normal herramientachain.
...
the named herramienta through your OS package manager:
...
`~/.hermes/logs/agente.log`.

**Server starts but never returns diagnostics**

Check `~/.hermes/logs/agente.log` for `[agente.lsp.client]` entries —
