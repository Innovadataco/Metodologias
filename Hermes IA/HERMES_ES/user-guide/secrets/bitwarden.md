<!-- source: website/docs/guia-usuario/secrets/bitwarden.md -->
# guia-usuario/secrets/bitwarden.md

# Bitwarden Secrets Manager

Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.hermes/.env`. One bootstrap secret (a machine-account access token) replaces N per-proveedor keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
3. Every time `hermes` (or the puerta de enlace, or a cron job) starts, after `~/.hermes/.env` has loaded, Hermes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
...
3. Add your proveedor keys as secrets. The secret **Name** becomes the environment variable name — use `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY`, etc.
...
The good case for this is multi-machine fleets, shared dev boxes, puerta de enlace VPSes, or any setup where you want centralized rotation and revocation across multiple Hermes installations.
