<!-- source: website/docs/guia-usuario/cli.md -->
# CLI Interface

# CLI Interface

Hermes Agente's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming herramienta output. Built for people who live in the terminal.
...
# With a specific modelo
hermes chat --modelo "anthropic/claude-sonnet-4"

# With a specific proveedor
hermes chat --proveedor nous        # Use Nous Portal
hermes chat --proveedor openrouter  # Force OpenRouter

# With specific herramientasets
hermes chat --herramientasets "web,terminal,habilidads"

# Start with one or more habilidads preloaded
hermes -s hermes-agente-dev,github-auth
...
# Isolated git worktree (for running multiple agentes in parallel)
...
The welcome banner shows your modelo, terminal backend, working directory, available herramientas, and installed habilidads at a glance.
...
| Element | Description |
|---------|-------------|
| Modelo name | Current modelo (truncated if longer than 26 chars) |
| Token count | Contexto tokens used / max contexto window |
| Contexto bar | Visual fill indicator with color-coded thresholds |
| Cost | Estimated session cost (or `n/a` for unknown/zero-priced modelos) |
| 🗜️ N | **Contexto compression count** — how many times the running session has been auto-compressed. Appears once the first compression fires. |
...
The bar adapts to terminal width — full layout at ≥ 76 columns, compact at 52–75, minimal (modelo + duration, plus the YOLO badge when active) below 52.

**Contexto color coding:**
...
| `Ctrl+C` | Interrupt agente (double-press within 2s to force exit) |
| `Ctrl+D` | Exit |
...
**Markdown stripping in final responses.** The CLI strips the most verbose markdown fences and `**bold**` / `*italic*` wrappers from *final* agente replies so they render as readable terminal prose rather than raw source. Code blocks and lists are preserved. This does not affect puerta de enlace platforms or herramienta results — they keep their markdown for native rendering.

## Slash Commands

Type `/` to see the autocomplete dropdown. Hermes supports a large set of CLI slash commands, dynamic habilidad commands, and user-defined quick commands.
...
| `/modelo` | Show or change the current modelo |
| `/herramientas` | List currently available herramientas |
| `/habilidads browse` | Browse the habilidads hub and official optional habilidads |
...
| `/status` | Show session info — modelo/perfil/tokens/duration — followed by a local **Session recap** block (recent turn counts, top herramientas used, files touched, latest user prompt + assistant reply). Pure local compute; no LLM call. |
...
For setup, proveedors, silence tuning, and messaging/Discord voice usage, see [Voice Mode](features/voice-mode.md).

:::tip
Commands are case-insensitive — `/HELP` works the same as `/help`. Installed habilidads also become slash commands automatically.
:::
...
quick_commands:
  ...
  gpu:
    type: exec
    command: nvidia-smi --query-gpu=utilization.gpu,memoria.used --format=csv,noheader
  restart:
    type: alias
    target: /puerta de enlace restart
```

Then type `/status`, `/gpu`, or `/restart` in any chat. See the [Configuración guide](/guia-usuario/configuración#quick-commands) for more examples.

## Preloading Habilidads at Launch

If you already know which habilidads you want active for the session, pass them at launch time:
...
Hermes loads each named habilidad into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Habilidad Slash Commands

Every installed habilidad in `~/.hermes/habilidads/` is automatically registered as a slash command. The habilidad name becomes the command:
...
Set a predefined personality to change the agente's tone:
...
You can interrupt the agente at any point:

- **Type a new message + Enter** while the agente is working — it interrupts and processes your new instructions
...
The `display.busy_input_mode` config key controls what happens when you press Enter while the agente is working:

| Mode | Behavior |
|------|----------|
| `"interrupt"` (default) | Your message interrupts the current operation and is processed immediately |
| `"queue"` | Your message is silently queued and sent as the next turn after the agente finishes |
| `"steer"` | Your message is injected into the current run via `/steer`, arriving at the agente after the next herramienta call — no interrupt, no new turn |
...
`"queue"` mode is useful when you want to prepare follow-up messages without accidentally canceling in-flight work. `"steer"` mode is useful when you want to redirect the agente mid-task without interrupting — e.g. "actually, also check the tests" while it's still editing code. Unknown values fall back to `"interrupt"`.

`"steer"` has two automatic fallbacks: if the agente hasn't started yet, or if images are attached, the message falls back to `"queue"` behavior so nothing is lost.
...
```
Hermes Agente has been suspended. Run `fg` to bring Hermes Agente back.
```

Type `fg` in your shell to resume the session exactly where you left off. This is not supported on Windows.

## Herramienta Progress Display

The CLI shows animated feedback as the agente works:
...
Cycle through display modes with `/verbose`: `off → new → all → verbose`. This command can also be enabled for messaging platforms — see [configuración](/guia-usuario/configuración#display-settings).

### Herramienta Preview Length

The `display.herramienta_preview_length` config key controls the maximum number of characters shown in herramienta call preview lines (e.g. file paths, terminal commands). The default is `0`, which means no limit — full paths and commands are shown.

```yaml
# ~/.hermes/config.yaml
display:
  herramienta_preview_length: 80   # Truncate herramienta previews to 80 chars (0 = no limit)
```

This is useful on narrow terminals or when herramienta arguments contain very long file paths.
...
Duration:       12m 34s
Messages:       28 (5 user, 18 herramienta calls)
```
...
Resuming restores the full conversation history from SQLite. The agente sees all previous messages, herramienta calls, and responses — just as if you never left.
...
Long conversations are automatically summarized when approaching contexto limits:

```yaml
# In ~/.hermes/config.yaml
compression:
  enabled: true
  threshold: 0.50    # Compress at 50% of contexto limit by default

# Summarization modelo configured under auxiliary:
auxiliary:
  compression:
    modelo: ""  # Leave empty to use the main chat modelo (default). Or pin a cheap fast modelo, e.g. "google/gemini-3-flash-preview".
```
...
Each `/background` prompt spawns a **completely separate agente session** in a daemon thread:

- **Isolated conversation** — the background agente has no knowledge of your current session's history. It receives only the prompt you provide.
- **Same configuración** — the background agente inherits your modelo, proveedor, herramientasets, reasoning settings, and fallback modelo from the current session.
...
- Suppresses verbose logging from herramientas
