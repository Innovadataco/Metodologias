<!-- source: website/docs/guia-usuario/sessions.md -->
# Sessions
...
Hermes Agente automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
- Modelo name and configuración
- System prompt snapshot
- Full message history (role, content, herramienta calls, herramienta results)
- Token counts (input/output)
- Timestamps (started_at, ended_at)
- Parent session ID (for compression-triggered session splitting)

### What Counts Toward Contexto

Hermes stores session history so it can resume conversations, but it does not
keep re-sending every byte it has ever handled. On each turn, the modelo sees
...
- Images may be attached natively to the next modelo call, or pre-analyzed into
  a text description when the active modelo does not support native vision.
...
script. Future turns do not automatically carry the original JPEG in contexto.
...
The most common cause of contexto growth is not the media file itself. It is
verbose text: pasted transcripts, full logs, large herramienta outputs, long diffs,
repeated status reports, and detailed proof dumps. Prefer summaries, file
paths, focused excerpts, and herramienta-backed lookups over copying large artifacts
into chat.
...
storage. Compression reduces the active contexto; it is not a privacy delete.
...
- **Collapses herramienta calls** to a count with herramienta names (e.g., `[3 herramienta calls: terminal, web_search]`)
- **Hides** system messages, herramienta results, and internal reasoning
...
:::tip
Session IDs follow the format `YYYYMMDD_HHMMSS_<hex>` — CLI/TUI sessions use a 6-char hex suffix (e.g. `20250305_091523_a1b2c3`), puerta de enlace sessions use an 8-char suffix (e.g. `20250305_091523_a1b2c3d4`). You can resume by ID (full or unique prefix) or by title — both work with `-c` and `-r`.
:::

## Cross-Platform Handoff

Use `/handoff <platform>` from a CLI session to transfer the live conversation to a messaging platform's home channel. The agente picks up exactly where the CLI left off — same session id, full role-aware transcript, herramienta calls and all.
...
2. The CLI marks the session pending and **block-polls the puerta de enlace**. It refuses if the agente is mid-turn — wait for the current response to finish first.
3. The puerta de enlace watcher claims the handoff and asks the destination adapter for a fresh thread:
...
4. The puerta de enlace re-binds the destination key to your existing CLI session id, then forges a synthetic user turn asking the agente to confirm and summarize. The reply lands in the new thread.
5. When the puerta de enlace acknowledges success, the CLI prints a `/resume` hint and exits cleanly:
...
- Platform not enabled / puerta de enlace not running → CLI times out at 60s with a clear message and your CLI session stays intact.
...
Hermes automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary modelo, so it adds no latency. You'll see auto-generated titles when browsing sessions with `hermes sessions list` or `hermes sessions browse`.

Auto-titling only fires once per session and is skipped if you've already set a title manually.

### Setting a Title Manually

Use the `/title` slash command inside any chat session (CLI or puerta de enlace):
...
When a session's contexto is compressed (manually via `/compress` or automatically), Hermes creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:
...
The `/title` command works in all puerta de enlace platforms (Telegram, Discord, Slack, WhatsApp):
...
For deeper analytics — token usage, cost estimates, herramienta breakdown, and activity patterns — use [`hermes insights`](/reference/cli-commands#hermes-insights).

## Session Search Herramienta

The agente has a built-in `session_search` herramienta that performs full-text search across all past conversations using SQLite's FTS5 engine — and lets the agente scroll through any session it finds. No LLM calls, no summarization, no truncation. Every shape returns actual messages from the DB.

### Three calling shapes

The herramienta infers what you want from which arguments you set. There's no `mode` parameter.
...
- `messages` — ±5 messages around the FTS5 match, with the anchor message flagged (the hit in contexto)
...
Returns a window of ±`window` messages centered on the anchor. No FTS5, no bookends — just the slice. Use after a discovery call when you need more contexto than the ±5 default window.
...
- `role_filter` — comma-separated roles to include. Discovery defaults to `user,assistant` (herramienta output is usually noise). Pass `user,assistant,herramienta` to include herramienta output (debugging herramienta behaviour) or `herramienta` to search herramienta output only.

### When It's Used

The agente is prompted to use session search automatically:

> *"When the user references something from a past conversation or you suspect relevant prior contexto exists, use session_search to recall it before asking them to repeat themselves."*
...
|-----------|--------------------|----------|
| Telegram DM | `agente:main:telegram:dm:<chat_id>` | One session per DM chat |
| Discord DM | `agente:main:discord:dm:<chat_id>` | One session per DM chat |
| WhatsApp DM | `agente:main:whatsapp:dm:<canonical_identifier>` | One session per DM user (LID/phone aliases collapse to one identity when mapping exists) |
| Group chat | `agente:main:<platform>:group:<chat_id>:<user_id>` | Per-user inside the group when the platform exposes a user ID |
| Group thread/topic | `agente:main:<platform>:group:<chat_id>:<thread_id>` | Shared session for all thread participants (default). Per-user with `thread_sessions_per_user: true`. |
| Channel | `agente:main:<platform>:channel:<chat_id>:<user_id>` | Per-user inside the channel when the platform exposes a user ID |
...
- one user's long herramienta-heavy task does not pollute another user's contexto window
- interrupt handling also stays per-user because the running-agente key matches the isolated session key
...
That reverts groups/channels to a single shared session per room, which preserves shared conversational contexto but also shares token costs, interrupt state, and contexto growth.

### Session Reset Policies

Puerta de enlace sessions are automatically reset based on configurable policies:
...
Before a session is auto-reset, the agente is given a turn to save any important memories or habilidads from the conversation.
...
| Puerta de enlace messages    | `~/.hermes/state.db`   | SQLite — canonical store for all session messages |
| Puerta de enlace routing index | `~/.hermes/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the puerta de enlace's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.hermes/sessions/sessions.json` is the **puerta de enlace routing index** — it maps
messaging session keys (`agente:main:<platform>:...`) to active session IDs.
It only ever contains puerta de enlace/messaging entries, so if you run a messaging
platform you'll see only those (e.g. `agente:main:whatsapp:dm:...`).

This is **expected** and does **not** mean your CLI sessions are missing.
`hermes sessions list`, `/sessions`, and the panel all read `state.db`,
which holds **every** session (CLI, TUI, and puerta de enlace). The `/save` snapshots
...
- **sessions** — session metadata (id, source, user_id, modelo, title, timestamps, token counts). Titles have a unique index (NULL titles allowed, only non-NULL must be unique).
- **messages** — full message history (role, content, herramienta_calls, herramienta_name, token_count)
...
- Puerta de enlace sessions auto-reset based on the configured reset policy
- Before reset, the agente saves memories and habilidads from the expiring session
- Opt-in auto-pruning: when `sessions.auto_prune` is `true`, ended sessions older than `sessions.retention_days` (default 90) are pruned at CLI/puerta de enlace startup
