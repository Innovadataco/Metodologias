<!-- source: website/docs/guia-usuario/messaging/weixin.md -->
# Weixin (WeChat)
...
- iLink typically **does not deliver ordinary WeChat group events** (including `@`-mentions of the personal account used for QR login) to the puerta de enlace for most bot-type accounts.
...
In practice, most deployments only get DMs to the iLink bot working reliably. If group delivery doesn't work after configuración, the limitation is on the iLink side, not in Hermes. The puerta de enlace logs a `WARNING` at startup whenever `WEIXIN_GROUP_POLICY` is set to anything other than `disabled`.
:::
...
- **QR code login** — scan-to-connect setup via `hermes puerta de enlace setup`
...
- **Contexto token persistence** — disk-backed reply continuity across restarts
...
- **Typing indicators** — shows "typing…" status in the WeChat client while the agente processes
...
| `text_batch_delay_seconds` | `3.0` | Quiet period (seconds) before a buffered burst of rapid text messages is flushed as one combined request. iLink delivers messages individually, so this debounce avoids one agente invocation per fragment. Set `0` to dispatch each message immediately. |
...
1. Pair Hermes once with `hermes puerta de enlace setup` and note the connected iLink bot
   account.
2. Have each allowed user send a direct message to that bot/contact.
3. Read the sender/user ID from the puerta de enlace logs or the inbound event payload.
4. Add those IDs to `WEIXIN_ALLOWED_USERS`, then restart the puerta de enlace.
...
:::note
The default group policy is `disabled` for Weixin (unlike WeCom where it defaults to `open`). This is intentional — personal WeChat accounts may be in many groups, and iLink bot identities typically can't receive ordinary WeChat group messages at all. The puerta de enlace logs a `WARNING` at startup if you set `WEIXIN_GROUP_POLICY` to anything other than `disabled`.
:::
...
The adapter receives media attachments from users, downloads them from the WeChat CDN, decrypts them, and caches them locally for agente processing:
...
**Quoted messages:** Media from quoted (replied-to) messages is also extracted, so the agente has contexto about what the user is replying to.
...
No configuración is needed — encryption and decryption happen automatically.
...
## Contexto Token Persistence

The iLink Bot API requires a `contexto_token` to be echoed back with each outbound message for a given peer. The adapter maintains a disk-backed contexto token store:

- Tokens are saved per account+peer to `~/.hermes/weixin/accounts/<account_id>.contexto-tokens.json`
- On startup, previously saved tokens are restored
- Every inbound message updates the stored token for that sender
- Outbound messages automatically include the latest contexto token

This ensures reply continuity even after puerta de enlace restarts.
...
4. The puerta de enlace automatically triggers typing indicators while the agente processes a message
...
Only one Weixin puerta de enlace instance can use a given token at a time. The adapter acquires a scoped lock on startup and releases it on shutdown. If another puerta de enlace is already using the same token, startup fails with an informative error message.
...
| `WEIXIN_ALLOW_ALL_USERS` | — | — | Puerta de enlace-level flag to allow all users (used by setup wizard) |
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `hermes puerta de enlace setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `hermes puerta de enlace setup` |
| `Another local Hermes puerta de enlace is already using this Weixin token` | Stop the other puerta de enlace instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `hermes puerta de enlace setup` to scan a new QR code |
...
| Messages appear duplicated | The adapter deduplicates by message ID. If you see duplicates, check if multiple puerta de enlace instances are running |
| `iLink POST ... HTTP 4xx/5xx` | API error from the iLink service. Check your token validity and network connectivity |
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.hermes/hermes-agente && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
