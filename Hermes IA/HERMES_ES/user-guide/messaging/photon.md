<!-- source: website/docs/guia-usuario/messaging/photon.md -->
...
dedicated number; the complemento supports both, and the free tier is the
...
- **Inbound** — the sidecar consumes the SDK's `app.messages` gRPC
  ...
  agente, reconnecting automatically if the stream drops.
- **Outbound** — replies are loopback POSTs to the sidecar, which calls
  `space.send(...)` on the SDK.

The Python complemento starts, supervises, and shuts down the sidecar
automatically.
...
Either run the unified puerta de enlace wizard and pick **Photon iMessage**:
...
2. **Finds or creates** the `Hermes Agente` project on your account.
...
5. **Prints your assigned iMessage line** — the number you text to reach
   your agente.
6. **Runs `npm install`** inside the complemento's sidecar directory.
...
(device token, panel project id) lives in `~/.hermes/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization modelo as every other Hermes
...
`@Hermes agente` variants. For a custom agente name, set regex patterns:
...
`PHOTON_MENTION_PATTERNS`). This is the same mention-gating modelo the
...
assigned iMessage line Hermes uses. When a Photon token and panel project
are available, `status` refreshes missing number rows from the panel
...
──────────────────────
  device token        : ✓ stored
  panel project   : 3c90c3cc-0d44-4b50-...
...
- **Inbound attachments are metadata-only.** Inbound events carry the
  filename + MIME type; the agente sees a marker but can't yet read the
...
| `PHOTON_DASHBOARD_HOST`   | `app.photon.codes` | Override the panel / device-login host |
