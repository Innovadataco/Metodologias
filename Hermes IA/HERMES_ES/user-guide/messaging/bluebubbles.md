<!-- source: website/docs/guia-usuario/messaging/bluebubbles.md -->
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Hermes uses conservative defaults for `Hermes` and `@Hermes agente` variants.

For a custom agente name, set regex patterns:
...
- **Media:** Images, voice messages, videos, and documents are supported in both directions. Inbound attachments are downloaded and cached locally for the agente to process.
...
### Typing Indicators
Shows "typing..." in the iMessage conversation while the agente is processing. Requires Private API.
...
- Check `hermes logs puerta de enlace` for webhook errors (or `hermes logs -f` to follow in real-time)
