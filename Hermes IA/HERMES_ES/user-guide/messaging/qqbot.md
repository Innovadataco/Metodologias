<!-- source: website/docs/guia-usuario/messaging/qqbot.md -->
...
- Receive messages via a persistent **WebSocket** connection to the QQ Puerta de enlace
- Send text and markdown replies via the **REST API**
- Download and process images, voice messages, and file attachments
- Transcribe voice messages using Tencent's built-in ASR or a configurable STT proveedor
  ...
  qqbot:
    ...
    extra:
      ...
      group_allow_from:
        - "group_openid_1"
      stt:
        proveedor: "zai"          # zai (GLM-ASR), openai (Whisper), etc.
        baseUrl: "https://open.bigmodelo.cn/api/coding/paas/v4"
        apiKey: "your-stt-key"
        modelo: "glm-asr"
```
...
2. **Configured STT proveedor** (fallback) — If QQ's ASR doesn't return text, the adapter calls an OpenAI-compatible STT API:

   - **Zhipu/GLM (zai)**: Default proveedor, uses `glm-asr` modelo
...
2. If using a custom STT proveedor, verify `QQ_STT_API_KEY` is set correctly
3. Check puerta de enlace logs for STT error messages
...
- Check network connectivity to `api.sgroup.qq.com` and the WebSocket puerta de enlace
- Review puerta de enlace logs for detailed error messages and reconnect behavior
