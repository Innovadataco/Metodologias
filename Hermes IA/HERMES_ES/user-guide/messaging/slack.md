<!-- source: website/docs/guia-usuario/messaging/slack.md -->
# Slack

# Slack Setup

Connect Hermes Agente to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
declares every built-in slash command (`/btw`, `/stop`, `/modelo`, …),
...
4. Enter an app name (e.g., "Hermes Agente") and select your workspace
5. Click **Create App**
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Hermes configuración issue.
:::
...
```bash
hermes puerta de enlace setup    # Select Slack when prompted
```

Then start the puerta de enlace:

```bash
hermes puerta de enlace              # Foreground
...
After starting the puerta de enlace, you need to **invite the bot** to any channel where you want it to respond:

```
/invite @Hermes Agente
```
...
Every Hermes command (`/btw`, `/stop`, `/new`, `/modelo`, `/help`, ...)
...
[`COMMAND_REGISTRY`](https://github.com/NousResearch/hermes-agente/blob/main/hermes_cli/commands.py)
...
`!queue`, `!stop`, `!modelo gpt-5.4`, etc. as a regular thread reply —
...
casual messages like `!nice work` pass through to the agente unchanged.
...
Understanding how Hermes behaves in different contextos:

| Contexto | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hermes Agente what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Hermes in `#general` will have separate histories and contextos.

Set to `false` if you want a collaborative mode where the entire channel shares one conversation session. Be aware this means users share contexto growth and token costs, and one user's `/reset` clears the session for everyone.
...
When `true` (the default), incoming audio messages are automatically transcribed using the configured STT proveedor before being processed by the agente.

### Full Example

```yaml
# Global puerta de enlace settings
group_sessions_per_user: true
unauthorized_dm_behavior: "pair"
stt_enabled: true
...
Make sure the bot has been **invited to the channel** (`/invite @Hermes Agente`).

---

## Multi-Workspace Support

Hermes can connect to **multiple Slack workspaces** simultaneously using a single puerta de enlace instance. Each workspace is authenticated independently with its own bot user ID.
...
- Each token is authenticated via `auth.test` on startup. The puerta de enlace maps each `team_id` to its own `WebClient` and `bot_user_id`.
...
- **Incoming:** Voice/audio messages are automatically transcribed using the configured STT proveedor: local `faster-whisper`, Groq Whisper (`GROQ_API_KEY`), or OpenAI Whisper (`VOICE_TOOLS_OPENAI_KEY`)
...
## Per-Channel Habilidad Bindings

Auto-load a habilidad whenever a new session starts in a specific channel or DM. Unlike per-channel prompts (which are injected on every turn), habilidad bindings inject the habilidad content as a user message at **session start** — it becomes part of the conversation history and does not need to be reloaded on subsequent turns.

This is ideal for DMs or channels with a dedicated purpose (flashcards, a domain-specific Q&A bot, a support triage channel, etc.) where you don't want the modelo's own habilidad selector to decide whether to load on every short reply.

```yaml
slack:
  channel_habilidad_bindings:
    # DM channel — always runs in "german-flashcards" mode
    - id: "D0ATH9TQ0G6"
      habilidads:
        - german-flashcards
    # Research channel — preload multiple habilidads in order
    - id: "C01RESEARCH"
      habilidads:
        - arxiv
        - writing-plans
    # Short form: single habilidad as a string
    - id: "C02SUPPORT"
      habilidad: hubspot-on-demand
```
...
- The habilidad is loaded only at session start (new session or after auto-reset). If you change the binding, run `/new` or wait for the session to auto-reset for it to take effect.
- Combine with `channel_prompts` for per-channel tone/constraints on top of the habilidad's instructions.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Hermes Agente` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Hermes Agente` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Hermes Agente`)
...
the puerta de enlace will **deny all messages** by default as a safety measure. Never share your bot tokens —
