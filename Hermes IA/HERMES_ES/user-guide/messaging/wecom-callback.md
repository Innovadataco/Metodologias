<!-- source: website/docs/guia-usuario/messaging/wecom-callback.md -->
# guia-usuario/messaging/wecom-callback.md

# WeCom Callback (Self-Built App)

Connect Hermes to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook modelo.
...
> Run `hermes puerta de enlace setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Hermes decrypts the message, queues it for the agente
4. Immediately acknowledges (silent — nothing displayed to the user)
5. The agente processes the request (typically 3–30 minutes)
...
4. Note the **Agente ID** from the app's overview page
...
(Use `hermes puerta de enlace start` only after `hermes puerta de enlace install` has registered the systemd/launchd service.)
...
| `agente_id` | — | Agente ID of the self-built app (required) |
...
- **No streaming** — replies arrive as complete messages after the agente finishes
- **No typing indicators** — the callback modelo doesn't support typing status
- **Text only** — currently supports text messages for input; image/file/voice input not yet implemented. The agente is aware of outbound media capabilities via the WeCom platform hint (images, documents, video, voice).
- **Response latency** — agente sessions take 3–30 minutes; users see the reply when processing completes
...
fixing, restart `hermes puerta de enlace run`.

**Callback URL not reachable / verification step fails.**
WeCom hits the public URL you registered. Confirm:
1. Your reverse proxy / tunnel forwards `/wecom/callback` to the puerta de enlace's port.
...
Check `hermes puerta de enlace run` logs for the bound host/port. If the adapter bound to
