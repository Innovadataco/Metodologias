<!-- source: website/docs/guia-usuario/messaging/matrix.md -->
# Matrix

# Matrix Setup

Hermes Agente integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Hermes Agente pipeline (including herramienta use, memoria, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).
...
| **Threads** | Hermes supports Matrix threads (MSC3440). If you reply in a thread, Hermes keeps the thread contexto isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
...
| **Interactive controls** | Dangerous-command approval and `/modelo` selection can use Matrix reactions. Approval reactions can be limited to the user who requested the action. |
| **Thinking and herramienta activity** | Matrix uses threaded, editable thinking/herramienta-activity panes when puerta de enlace progress is enabled, so updates do not flood the main room timeline. |
...
### Mention and Threading Configuración
...
matrix:
  ...
  allowed_users:                  # Matrix users allowed to trigger agente turns
    - "@alice:matrix.org"
  allowed_rooms:                  # Matrix rooms allowed to trigger agente turns
    - "!abc123:matrix.org"
...
MATRIX_SESSION_SCOPE=room       # recommended for stable project-room contexto
...
and a Matrix room-boundary note in the agente prompt. `/status` also shows the
...
1. Use the admin API or registration herramienta to create a new user:
...
Hermes Agente uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.

To find yours:

1. Open [Element](https://app.element.io) (or your preferred Matrix client).
2. Click your avatar → **Settings**.
3. Your User ID is displayed at the top of the perfil (e.g., `@alice:matrix.org`).
...
## Step 4: Configure Hermes Agente
...
room can trigger an agente turn. If `MATRIX_ALLOWED_ROOMS` is unset, any room the
bot joins can trigger an agente turn. A locked-down deployment should set both:
...
allowlists tight, prefer DMs or private rooms for herramienta-heavy work, and avoid
...
- `group_sessions_per_user: true` keeps each participant's contexto isolated inside shared rooms

### Start the Puerta de enlace

Once configured, start the Matrix puerta de enlace:
...
cd ~/.hermes/hermes-agente && uv pip install -e ".[matrix]"
```
...
### Matrix Herramientas and Controls

In Matrix conversations, Hermes exposes Matrix-specific herramientas to the agente:
...
These herramientas are scoped to Matrix contextos and are not available in non-Matrix herramientasets. Admin-style herramientas are disabled by default: redaction requires `MATRIX_TOOLS_ALLOW_REDACTION=true`, invites require `MATRIX_TOOLS_ALLOW_INVITES=true`, and room creation requires `MATRIX_TOOLS_ALLOW_ROOM_CREATE=true`. Public room creation also requires `MATRIX_ALLOW_PUBLIC_ROOMS=true`.
Matrix herramientas are limited to the current Matrix room by default. Explicit
...
set, Matrix herramientas may only target those rooms.
...
- ♾️ approve always
- ❌ deny
- number reactions for `/modelo` choices

Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/modelo picker prompt. The default is requester-bound when Hermes knows who requested the action.

### Media Limits

Hermes uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread contexto across the batch.
...
```bash
docker compose -f tests/e2e/matrix_synapse_puerta de enlace/docker-compose.yml up -d
HERMES_MATRIX_SYNAPSE_INTEGRATION=1 \
  scripts/run_tests.sh -m "integration and matrix_synapse" \
  tests/e2e/matrix_synapse_puerta de enlace/test_puerta de enlace.py
docker compose -f tests/e2e/matrix_synapse_puerta de enlace/docker-compose.yml down -v
```
...
Hermes supports the same puerta de enlace commands in Matrix that it supports on other
messaging platforms, including `/commands`, `/modelo`, `/stop`, `/queue`,
...
!commands
!modelo
!modelo gpt-5.5 --proveedor openrouter
!queue continue with the next task
!stop
```

Hermes only normalizes `!command` when the command is known to the puerta de enlace, a
registered complemento command, or an installed habilidad command. Ordinary exclamations
...
**Fix**: Invite the bot to the room — it auto-joins on invite. Verify your User ID is in `MATRIX_ALLOWED_USERS` (use the full `@user:server` format) and the room ID is in `MATRIX_ALLOWED_ROOMS` if that allowlist is configured. In rooms, mention the bot or add the room to `MATRIX_FREE_RESPONSE_ROOMS`. Restart the puerta de enlace.

### Bot joins rooms but silently drops every message (clock skew)

**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/hermes-agente/issues/12614).

**Symptom**: Puerta de enlace log shows `Matrix: dropped N live events as 'too old' more than 30s after startup`.
...
3. In your Matrix client (Element), go to the bot's perfil -> Sessions -> verify/trust the bot's device.
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "Hermes Agente"
     }'
   ```
...
5. **Restart the puerta de enlace**:

   ```bash
   hermes puerta de enlace run
   ```
...
Matrix E2EE requires `libolm`, which doesn't compile on macOS ARM64 (Apple Silicon). The `hermes-agente[matrix]` extra is gated to Linux only. If you're on macOS, proxy mode lets you run E2EE in a Docker container on a Linux VM while the actual agente runs natively on macOS with full access to your local files, memoria, and habilidads.
...
services:
  hermes-matrix:
    ...
    environment:
      ...
      # Proxy mode — forward to host agente
...
RUN cd ~/.hermes/hermes-agente && uv pip install -e ".[matrix]"

CMD ["hermes", "puerta de enlace"]
```

That's the entire container. No API keys for OpenRouter, Anthropic, or any inference proveedor.

### Step 3: Start Both

1. Start the host puerta de enlace first:
   ```bash
   hermes puerta de enlace
   ```
...
Proxy mode is configured on the **container side** (the thin puerta de enlace):

| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Hermes API server (e.g., `http://192.168.1.100:8642`) |
| `GATEWAY_PROXY_KEY` | Bearer token for autenticación (must match `API_SERVER_KEY` on the host) |
| `puerta de enlace.proxy_url` | Same as `GATEWAY_PROXY_URL` but in `config.yaml` |
...
Proxy mode is not limited to Matrix. Any platform adapter can use it — set `GATEWAY_PROXY_URL` on any puerta de enlace instance and it will forward to the remote agente instead of running one locally. This is useful for any deployment where the platform adapter needs to run in a different environment from the agente (network isolation, E2EE requirements, resource constraints).

:::tip
Session continuity is maintained via the `X-Hermes-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agente.
:::

:::note
**Limitations (v1):** Herramienta progress messages from the remote agente are not relayed back — the user sees the streamed final response only, not individual herramienta calls. Dangerous command approval prompts are handled on the host side, not relayed to the Matrix user. These can be addressed in future updates.
:::
...
messages still fail after a puerta de enlace restart, verify handlers are registered before
the first sync and check logs for `sync event dispatch error`.

### Sync issues / bot falls behind

**Cause**: Long-running herramienta executions can delay the sync loop, or the homeserver is slow.
...
**Cause**: The Hermes puerta de enlace isn't running, or it failed to connect.

**Fix**: Check that `hermes puerta de enlace` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.hermes/.env` and restart the puerta de enlace. Use the full `@user:server` format.
