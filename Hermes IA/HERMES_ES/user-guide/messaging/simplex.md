<!-- source: website/docs/guia-usuario/messaging/simplex.md -->
...
> Run `hermes puerta de enlace setup` and pick **SimpleX** for a guided walk-through.
...
After starting the daemon, open a conversation with your agente contact. The numeric `contactId` appears in session logs or via `hermes send_message action=list`. If you'd rather use the display name shown in the SimpleX UI, that works too — `SIMPLEX_ALLOWED_USERS` accepts either form.
...
Agente replies can also embed `MEDIA:/path/to/file` tags in plain text —
