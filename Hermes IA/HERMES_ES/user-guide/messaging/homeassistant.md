<!-- source: website/docs/guia-usuario/messaging/homeassistant.md -->
# guia-usuario/messaging/homeassistant

# Home Assistant Integration

Hermes Agente integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:

1. **Puerta de enlace platform** — subscribes to real-time state changes via WebSocket and responds to events
2. **Smart home herramientas** — four LLM-callable herramientas for querying and controlling devices via the REST API
...
2. Go to your **Perfil** (click your name in the sidebar)
3. Scroll to **Long-Lived Access Tokens**
4. Click **Create Token**, give it a name like "Hermes Agente"
5. Copy the token
...
:::info
The `homeassistant` herramientaset is automatically enabled when `HASS_TOKEN` is set. Both the puerta de enlace platform and the device control herramientas activate from this single token.
:::
...
Hermes Agente registers four herramientas for smart home control:
...
## Puerta de enlace Platform: Real-Time Events

The Home Assistant puerta de enlace adapter connects via WebSocket and subscribes to `state_changed` events. When a device state changes and matches your filters, it's forwarded to the agente as a message.

### Event Filtering

:::warning Required Configuración
...
Configure which events the agente sees in `~/.hermes/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agente are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Hermes Agente".
...
The Home Assistant herramientas enforce security restrictions:
...
### Reactive Automation (via Puerta de enlace Events)

When connected as a puerta de enlace platform, the agente can react to events:

```
[Home Assistant] Front Door: triggered (was cleared)

Agente automatically:
...
from `config.yaml`. Double-check the file lives under the active Hermes perfil
home and that there's no stray quoting around the URL/token. Restart the puerta de enlace
after editing — env changes are only applied on process start.


**REST auth failing (`401 Unauthorized`).**
The token must be a *Long-Lived Access Token* created from your HA user perfil
page (**Perfil → Security → Long-lived access tokens**). Short-lived UI
