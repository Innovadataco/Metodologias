<!-- source: website/docs/guia-usuario/messaging/mattermost.md -->
# Mattermost

# Mattermost Setup

Hermes Agente integrates with Mattermost as a bot, letting you chat with your AI assistant through direct messages or team channels. Mattermost is a self-hosted, open-source Slack alternative — you run it on your own infrastructure, keeping full control of your data. The bot connects via Mattermost's REST API (v4) and WebSocket for real-time events, processes messages through the Hermes Agente pipeline (including herramienta use, memoria, and reasoning), and responds in real time. It supports text, file attachments, images, and slash commands.
...
| **Threads** | If `MATTERMOST_REPLY_MODE=thread`, Hermes replies in a thread under your message. Thread contexto stays isolated from the parent channel. |
...
3. Fill in the details:
   - **Username**: e.g., `hermes`
   - **Display Name**: e.g., `Hermes Agente`
...
:::tip
You can also use a **personal access token** instead of a bot account. Go to **Perfil** → **Security** → **Personal Access Tokens** → **Create Token**. This is useful if you want Hermes to post as your own user rather than a separate bot user.
:::
...
Hermes Agente uses your Mattermost User ID to control who can interact with the bot. To find it:

1. Click your **avatar** (top-left corner) → **Perfil**.
2. Your User ID is displayed in the perfil dialog — click it to copy.
...
## Step 5: Configure Hermes Agente
...
- `group_sessions_per_user: true` keeps each participant's contexto isolated inside shared channels and threads

### Start the Puerta de enlace

Once configured, start the Mattermost puerta de enlace:
...
**Fix**: Add the bot to the channel (channel name → Add Members → search for the bot). Verify your User ID is in `MATTERMOST_ALLOWED_USERS`. Restart the puerta de enlace.
...
**Fix**: The adapter automatically reconnects with exponential backoff (2s → 60s). Check your server's WebSocket configuración — reverse proxies (nginx, Apache) need WebSocket upgrade headers configured. Verify no firewall is blocking WebSocket connections on your Mattermost server.
...
**Fix**: Check that `hermes puerta de enlace` is running. Look at the terminal output for error messages. Common issues: wrong URL, expired token, Mattermost server unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATTERMOST_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATTERMOST_ALLOWED_USERS` in `~/.hermes/.env` and restart the puerta de enlace. Remember: the User ID is a 26-character alphanumeric string, not your `@username`.
...
:::warning
Always set `MATTERMOST_ALLOWED_USERS` to restrict who can interact with the bot. Without it, the puerta de enlace denies all users by default as a safety measure. Only add User IDs of people you trust — authorized users have full access to the agente's capabilities, including herramienta use and system access.
:::
...
- **No extra dependencies**: The adapter uses `aiohttp` for HTTP and WebSocket, which is already included with Hermes Agente.
