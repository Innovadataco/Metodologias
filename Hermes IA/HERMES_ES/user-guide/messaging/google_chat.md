<!-- source: website/docs/guia-usuario/messaging/google_chat.md -->
# Google Chat

# Google Chat Setup

Connect Hermes Agente to Google Chat as a bot. The integration uses Cloud Pub/Sub
...
> Run `hermes puerta de enlace setup` and pick **Google Chat** for a guided walk-through.
...
| **Autenticación** | Service Account JSON with `roles/pubsub.subscriber` on the subscription |
...
Go to **APIs & Services → Google Chat API → Configuración**.
...
| Inline images via URL | Interactive Card v2 buttons (v1 of this puerta de enlace) |
| Native file attachments (after `/setup-files` — see Step 10) | Native voice notes / circular video notes |

The agente's system prompt includes a Google Chat–specific hint so it knows these
limits and avoids formatting that won't render.

Message size limit: 4000 characters per message. Longer agente responses are
...
> This method doesn't support app autenticación with a service account.
...
4. Register the client with Hermes (run under the perfil you want it scoped to):

```bash
# Default perfil:
python -m complementos.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json

# A named perfil gets its own separate registration:
hermes -p <perfil> python -m complementos.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active perfil's Hermes home (e.g.
`~/.hermes/google_chat_user_client_secret.json` for the default perfil). The
client secret is **perfil-scoped, not shared across perfils** — each perfil
registers its own. This is deliberate: perfils are isolated auth boundaries, so
two perfils can point at different Google OAuth apps / accounts. Register it
once per perfil that needs Google Chat attachment delivery.
...
3. Check `hermes puerta de enlace` logs for `[GoogleChat] Connected`. If you see
   `[GoogleChat] Config validation failed`, the error message tells you which
   env var to fix.

**Bot replies but an error message appears instead of the agente's answer.**
...
agente produces long streaming responses that exceed that, the adapter retries
...
the next file request uploads natively without a puerta de enlace restart.

**`/setup-files start` says "No client credentials stored."**

The one-time setup wasn't done *for this perfil* (the client secret is
perfil-scoped, so a registration under one perfil won't be seen by another).
From a terminal, run it under the perfil the puerta de enlace uses:
...
- **Redaction**: Service Account emails, subscription paths, and topic paths
  are stripped from log output by `agente/redact.py`. The debug envelope dump
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  permissions are the protection — same modelo as the SA key file). Each
