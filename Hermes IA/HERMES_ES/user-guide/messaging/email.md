<!-- source: website/docs/guia-usuario/messaging/email.md -->
# Email

# Email Setup

Hermes can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agente's address and it replies in-thread — no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any proveedor that supports IMAP/SMTP.

:::info Puerta de enlace adapter only: no external dependencies
This page covers the Email puerta de enlace adapter, which uses Python's built-in `imaplib`, `smtplib`, and `email` modules. No additional packages or external services are required for this puerta de enlace path.
:::

This is separate from the bundled [Himalaya email habilidad](/docs/guia-usuario/habilidads/bundled/email/email-himalaya), which lets the agente manage email through terminal commands and requires the external `himalaya` CLI plus a Himalaya config file.

| Use case | What to configure | External dependency |
|---|---|---|
| Let people email the Hermes agente and receive replies | Email puerta de enlace adapter on this page | None beyond an IMAP/SMTP email account |
| Let the agente inspect, compose, move, and manage mailbox messages from terminal herramientas | Himalaya email habilidad | `himalaya` CLI and `~/.config/himalaya/config.toml` |

---

## Prerequisites

- **A dedicated email account** for your Hermes agente (don't use your personal email)
- **IMAP enabled** on the email account
- **An app password** if using Gmail or another proveedor with 2FA

### Gmail Setup

1. Enable 2-Factor Autenticación on your Google Account
...
Most email proveedors support IMAP/SMTP. Check your proveedor's documentation for:
...
## Step 2: Start the Puerta de enlace
...
- **Subject line** is included as contexto (e.g., `[Subject: Deploy to production]`)
- **Reply emails** (subject starting with `Re:`) skip the subject prefix — the thread contexto is already established
- **Attachments** are cached locally:
  - Images (JPEG, PNG, GIF, WebP) → available to the vision herramienta
...
- **Message-ID** generated with the agente's domain
- Responses are sent as plain text (UTF-8)

### File Attachments

The agente can send file attachments in replies. Include `MEDIA:/path/to/file` in the response and the file is attached to the outgoing email.
...
| **Messages not received** | Check `EMAIL_ALLOWED_USERS` includes the sender's email. Check spam folder — some proveedors flag automated replies. |
| **"Autenticación failed"** | For Gmail, you must use an App Password, not your regular password. Ensure 2FA is enabled first. |
| **Duplicate replies** | Ensure only one puerta de enlace instance is running. Check `hermes puerta de enlace status`. |
...
:::warning
**Use a dedicated email account.** Don't use your personal email — the agente stores the password in `.env` and has full inbox access via IMAP.
:::

- Use **App Passwords** instead of your main password (required for Gmail with 2FA)
- Set `EMAIL_ALLOWED_USERS` to restrict who can interact with the agente
...
|----------|----------|---------|-------------|
| `EMAIL_ADDRESS` | Yes | — | Agente's email address |
