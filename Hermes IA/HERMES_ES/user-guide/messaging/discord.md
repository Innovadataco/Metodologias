<!-- source: website/docs/guia-usuario/messaging/discord.md -->
# Discord

# Discord Setup

Hermes Agente integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the Hermes Agente pipeline (including herramienta use, memoria, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.
...
### Discord Puerta de enlace Modelo

Hermes on Discord is not a webhook that replies statelessly. It runs through the full messaging puerta de enlace, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal Hermes agente execution, including herramientas, memoria, and slash commands
...
- users share contexto growth and token costs
- one person's long herramienta-heavy task can bloat everyone else's contexto
- one person's in-flight run can interrupt another person's follow-up in the same room

### Interrupts and Concurrency

Hermes tracks running agentes by session key.
...
- the whole room shares one running-agente slot for that channel/thread
...
3. Enter a name for your application (e.g., "Hermes Agente") and accept the Developer Terms of Service.
4. Click **Create**.
...
## Step 3: Enable Privileged Puerta de enlace Intents
...
On the **Bot** page, scroll down to **Privileged Puerta de enlace Intents**. You'll see three toggles:
...
If your bot is online but never responds to messages, the **Message Content Intent** is almost certainly disabled. Go back to the [Developer Portal](https://discord.com/developers/applications), select your application → Bot → Privileged Puerta de enlace Intents, and make sure **Message Content Intent** is toggled ON. Click **Save Changes**.
:::
...
The bot token is the credential Hermes Agente uses to log in as your bot. Still on the **Bot** page:
...
2. Under **Installation Contextos**, enable **Guild Install**.
...
- **Read Message History** — maintain conversation contexto
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Hermes puerta de enlace).

## Step 7: Find Your Discord User ID

Hermes Agente uses your Discord User ID to control who can interact with the bot. To find it:

1. Open Discord (desktop or web app).
2. Go to **Settings** → **Advanced** → toggle **Developer Mode** to **ON**.
3. Close settings.
4. Right-click your own username (in a message, the member list, or your perfil) → **Copy User ID**.
...
## Step 8: Configure Hermes Agente
...
Then start the puerta de enlace:
...
:::tip
You can run `hermes puerta de enlace` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::
...
| `DISCORD_ALLOWED_USERS` | **Yes** | — | Comma-separated Discord user IDs allowed to interact with the bot. Without this **or** `DISCORD_ALLOWED_ROLES`, the puerta de enlace denies all users. |
...
| `DISCORD_HISTORY_BACKFILL` | No | `true` | When `true`, prepend recent channel scrollback (since the bot's last response) to the user message when the bot is mentioned. Recovers contexto the bot would otherwise miss with `require_mention`. Skipped in DMs and free-response channels. Set to `false` to disable. |
...
| `DISCORD_ALLOW_ANY_ATTACHMENT` | No | `false` | When `true`, the bot accepts attachments of any file type (not just the built-in PDF/text/zip/office allowlist). Unknown types are cached to disk and surfaced to the agente as a local path with `application/octet-stream` MIME so it can inspect them with `terminal` / `read_file` / `ffprobe` / etc. |
| `DISCORD_MAX_ATTACHMENT_BYTES` | No | `33554432` | Maximum bytes per attachment the puerta de enlace will download and cache. Default 32 MiB. Set to `0` for no cap (attachments are held in memoria while being written, so unlimited carries a real memoria cost). |
...
# Session isolation (applies to all puerta de enlace platforms, not just Discord)
...
Per-user sessions (`group_sessions_per_user: true`, the default) also benefit: a user's session is missing the contexto posted by other channel participants and the user's own messages from before they tagged the bot. Backfill fills both gaps.
...
Maximum number of messages to scan backwards when recovering channel contexto. In practice the scan usually stops much earlier — at the bot's own last message in the channel, which is the natural boundary between turns. This limit is a safety cap for cold starts and long gaps where no prior bot message exists in recent history.
...
This is a global puerta de enlace setting (not Discord-specific) that controls whether users in the same channel get isolated session histories.

When `true`: Alice and Bob talking in `#research` each have their own separate conversation with Hermes. When `false`: the entire channel shares one conversation transcript and one running-agente slot.

```yaml
group_sessions_per_user: true
```

See the [Session Modelo](#session-modelo-in-discord) section above for the full implications of each mode.

#### `display.herramienta_progress`

**Type:** string — **Default:** `"all"` — **Values:** `off`, `new`, `all`, `verbose`

Controls whether the bot sends progress messages in the chat while processing (e.g., "Reading file...", "Running terminal command..."). This is a global puerta de enlace setting that applies to all platforms.
...
- `new` — only show the first herramienta call per turn
- `all` — show all herramienta calls (truncated to 40 characters in puerta de enlace messages)
- `verbose` — show full herramienta call details (can produce long messages)

#### `display.herramienta_progress_command`

**Type:** boolean — **Default:** `false`

When enabled, makes the `/verbose` slash command available in the puerta de enlace, letting you cycle through herramienta progress modes (`off → new → all → verbose → off`) without editing config.yaml.

```yaml
display:
  herramienta_progress_command: true
```
...
- A user in `allow_admin_from` for a scope (DM or server channel) can run **every** registered slash command — built-in AND complemento-registered — through the live command registry.
...
Send `/modelo` with no arguments in a Discord channel to open a dropdown-based modelo picker:

1. **Proveedor selection** — a Select dropdown showing available proveedors (up to 25).
2. **Modelo selection** — a second dropdown with modelos for the chosen proveedor (up to 25).

The picker times out after 120 seconds. Only authorized users (those in `DISCORD_ALLOWED_USERS`) can interact with it. If you know the modelo name, type `/modelo <name>` directly.

## Native Slash Commands for Habilidads

Hermes automatically registers installed habilidads as **native Discord Application Commands**. This means habilidads appear in Discord's autocomplete `/` menu alongside built-in commands.

- Each habilidad becomes a Discord slash command (e.g., `/code-review`, `/ascii-art`)
- Habilidads accept an optional `args` string parameter
- Discord has a limit of 100 application commands per bot — if you have more habilidads than available slots, extra habilidads are skipped with a warning in the logs
- Habilidads are registered during bot startup alongside built-in commands like `/modelo`, `/reset`, and `/background`

No extra configuración is needed — any habilidad installed via `hermes habilidads install` is automatically registered as a Discord slash command on the next puerta de enlace restart.

### Disabling Slash Command Registration

If you run multiple Hermes puerta de enlaces against the same Discord application (e.g. staging + production), only one of them should own the global slash-command registration — otherwise the last startup wins and the registrations flap. Turn slash registration off on the "follower" puerta de enlace:
...
Leaving this at `true` on the "primary" puerta de enlace keeps the normal behavior — global `/`-menu commands for built-ins and installed habilidads.

## Sending Media (`send_message` + `MEDIA:` tags)

The Discord adapter supports native file uploads for every common media type via the `send_message` herramienta and inline `MEDIA:/path/to/file` tags emitted by the agente:
...
Any file type a user uploads is accepted. Authorization to message the agente is the gate — not the file extension. Every upload is downloaded, cached under `~/.hermes/cache/documents/`, and surfaced to the agente as a `DOCUMENT`-typed message event so it can inspect the file with `terminal` (`ffprobe`, `unzip`, `file`, `strings`, etc.) or `read_file`.
...
- Small UTF-8-decodable files (text, code, config, HTML, CSS, JSON, YAML, ...) have their contents auto-injected into the prompt up to 100 KiB. Binary files that can't be decoded are surfaced as a path-pointing contexto note only (auto-translated for Docker/Modal sandboxed terminals via `to_agente_visible_cache_path`), so they don't blow up the contexto window.
...
discord:
  ...
  # The whole file is held in memoria while being cached, so unlimited
  # uploads carry a real memoria cost.
...
:::warning Memoria cost of unlimited
Disabling the size cap (`max_attachment_bytes: 0`) means a user can drop a multi-GB file on the bot and the puerta de enlace will dutifully buffer it through memoria while caching to disk. Only set this in trusted single-user installs. For shared bots, keep the default 32 MiB or raise it conservatively.
:::

## Interactive Prompts (clarify)

When the agente calls the `clarify` herramienta — to ask which approach you prefer, get post-task feedback, or check before a non-trivial decision — Discord renders the question with **one button per choice**:
...
The buttons disable themselves once a choice is made so duplicate clicks don't double-resolve the prompt. Configure the response timeout via `agente.clarify_timeout` in `~/.hermes/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agente unblocks with a sentinel message and adapts rather than hanging.
...
Hermes Agente supports Discord voice messages:

- **Incoming voice messages** are automatically transcribed using the configured STT proveedor: local `faster-whisper` (no key), Groq Whisper (`GROQ_API_KEY`), or OpenAI Whisper (`VOICE_TOOLS_OPENAI_KEY`).
- **Text-to-speech**: Use `/voice tts` to have the bot send spoken audio responses alongside text replies.
- **Discord voice channels**: Hermes can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/guia-usuario/features/voice-mode)
- [Use Voice Mode with Hermes](/guides/use-voice-mode-with-hermes)

### Voice Channel Audio Effects (ambient + verbal acks)

When the bot is in a voice channel, you can give it a more conversational feel: a short verbal acknowledgement ("let me look into that") before it starts working, and a subtle ambient "thinking" bed that plays underneath while herramientas run — the speech ducks the ambient down and swells it back when finished, similar to Grok voice mode.
...
discord:
  voice_fx:
    ...
    ambient_enabled: true  # idle "thinking" bed while herramientas run
    ...
    ack_enabled: true      # speak a short phrase before the first herramienta call of a turn
...
Notes:
- The acknowledgement fires at most once per turn, only when the bot is in a voice channel and the mixer is active. It uses your configured TTS proveedor.
...
Discord forum channels (type 15) don't accept direct messages — every post in a forum must be a thread. Hermes auto-detects forum channels and creates a new thread post whenever it needs to send there, so `send_message`, TTS, images, voice messages, and file attachments all work without special handling from the agente.
...
Refreshing the directory (`/channels refresh` on platforms that expose it, or a puerta de enlace restart) populates the cache with any forum channels created after the bot started.
...
**Fix**: Go to [Developer Portal](https://discord.com/developers/applications) → your app → Bot → Privileged Puerta de enlace Intents → enable **Message Content Intent** → Save Changes. Restart the puerta de enlace.

### "Disallowed Intents" error on startup

**Cause**: Your code requests intents that aren't enabled in the Developer Portal.

**Fix**: Enable all three Privileged Puerta de enlace Intents (Presence, Server Members, Message Content) in the Bot settings, then restart.
...
**Cause**: The Hermes puerta de enlace isn't running, or the token is incorrect.

**Fix**: Check that `hermes puerta de enlace` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.hermes/.env` and restart the puerta de enlace.

### People in the same channel are sharing contexto unexpectedly

**Cause**: `group_sessions_per_user` is disabled, or the platform cannot provide a user ID for the messages in that contexto.

**Fix**: Set this in `~/.hermes/config.yaml` and restart the puerta de enlace:
...
:::warning
Always set `DISCORD_ALLOWED_USERS` (or `DISCORD_ALLOWED_ROLES`) to restrict who can interact with the bot. Without either, the puerta de enlace denies all users by default as a safety measure. Only authorize people you trust — authorized users have full access to the agente's capabilities, including herramienta use and system access.
:::

### Role-Based Access Control

For servers where access is managed by roles instead of individual user lists (moderator teams, support staff, internal herramientaing), use `DISCORD_ALLOWED_ROLES` — a comma-separated list of role IDs. Any member with one of those roles is authorized.
...
This is the preferred pattern when the moderation team churns — new moderators get access the moment the role is granted, with no `.env` edit or puerta de enlace restart.
...
For more information on securing your Hermes Agente deployment, see the [Security Guide](../security.md).
