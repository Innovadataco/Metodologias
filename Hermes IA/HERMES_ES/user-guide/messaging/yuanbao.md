<!-- source: website/docs/guia-usuario/messaging/yuanbao.md -->
# Yuanbao

# Yuanbao

Connect Hermes to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket puerta de enlace for real-time message delivery and supports both direct (C2C) and group conversations.

:::info
Yuanbao is an enterprise messaging platform primarily used within Tencent and enterprise environments. It uses WebSocket for real-time communication, HMAC-based autenticación, and supports rich media including images, files, and voice messages.
:::
...
3. Save the configuración automatically
...
### 4. Start the Puerta de enlace

```bash
hermes puerta de enlace
```

The adapter will connect to the Yuanbao WebSocket puerta de enlace, authenticate using HMAC signatures, and begin processing messages.

## Características

- **WebSocket puerta de enlace** — real-time bidirectional communication
- **HMAC autenticación** — secure request signing with APP_ID/APP_SECRET
...
- **Typing indicators** — shows "typing…" status while the agente processes
...
- **Slow-response notification** — sends a waiting message when the agente takes longer than expected
...
| `/modelo [proveedor:modelo]` | Show or change the modelo |
...
**Cause**: Autenticación failed during WebSocket handshake.
...
4. Review puerta de enlace logs: `tail -f ~/.hermes/logs/puerta de enlace.log`
...
4. Check COS bucket configuración with platform admin
...
**Fix**:
1. Check puerta de enlace logs for error patterns
...
```bash
HERMES_LOG_LEVEL=debug hermes puerta de enlace
```
...
- [Messaging Puerta de enlace Overview](./index.md)
- [Slash Commands Reference](/reference/slash-commands)
- [Cron Jobs](/guia-usuario/features/cron)
- [Background Sessions](/guia-usuario/cli#background-sessions)
