...
Start the puerta de enlace: `hermes puerta de enlace run`. The listener exposes:
...
Most settings also have an equivalent env var (`MSGRAPH_WEBHOOK_*`) that merges into the config at puerta de enlace startup (the exception is `host`, which is config-only — see the note above) — see the [environment variables reference](/reference/environment-variables#microsoft-graph-teams-meetings).
...
- [Microsoft Teams bot setup](/guia-usuario/messaging/teams) — the different platform that lets users chat with Hermes in Teams
