<!-- source: website/docs/guia-usuario/messaging/line.md -->
# LINE

# LINE Setup

Run Hermes Agente as a [LINE](https://line.me/) bot via the official LINE Messaging API. The adapter lives as a bundled platform complemento under `complementos/platforms/line/` — no core edits, just enable it like any other platform.

LINE is the dominant messaging app in Japan, Taiwan, and Thailand. If your users live there, this is how they reach you.

> Run `hermes puerta de enlace setup` and pick **LINE** for a guided walk-through.
...
2. Create a Proveedor, then under it a **Messaging API** channel.
...
That's enough — the bundled-complemento scan in `puerta de enlace/config.py` automatically picks up `complementos/platforms/line/`. No `Platform.LINE` enum edit, no `_create_adapter` registration.
...
## Step 5: Run the puerta de enlace

```bash
hermes puerta de enlace
```

The agente log shows:
...
Cron jobs with `deliver: line` route to `LINE_HOME_CHANNEL`. The adapter ships a standalone Push-only sender so cron jobs work even when cron runs in a separate process from the puerta de enlace.
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.hermes/logs/puerta de enlace.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
...
**Postback button never appears.** Either the LLM responded faster than `LINE_SLOW_RESPONSE_THRESHOLD`, or another bubble (herramienta-progress, streaming) consumed the reply token first. See the suppression block under "Slow LLM responses".

**"already in use by another perfil".** The same channel access token is bound to another running Hermes perfil. Stop the other puerta de enlace or use a separate channel.
