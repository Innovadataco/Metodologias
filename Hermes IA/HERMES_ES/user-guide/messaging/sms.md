<!-- source: website/docs/guia-usuario/messaging/sms.md -->
# SMS (Twilio)
...
The SMS puerta de enlace shares credentials with the optional [telephony habilidad](/reference/habilidads-catalog). If you've already set up Twilio for voice calls or one-off SMS, the puerta de enlace works with the same `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, and `TWILIO_PHONE_NUMBER`.
:::
...
- **aiohttp** — `cd ~/.hermes/hermes-agente && uv pip install -e ".[sms]"`
...
2. Copy your **Account SID** and **Auth Token** from the panel
...
**The puerta de enlace denies all users by default.** Configure an allowlist:
...
3. Check Hermes puerta de enlace logs for Twilio API errors
