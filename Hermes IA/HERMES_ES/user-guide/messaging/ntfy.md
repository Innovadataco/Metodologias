<!-- source: website/docs/guia-usuario/messaging/ntfy.md -->
...
ntfy makes a great lightweight push channel for Hermes: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agente, get the response back on your phone.

> Run `hermes puerta de enlace setup` and pick **ntfy** for a guided walk-through.
...
## Identity modelo — read this before deploying
...
This means **anyone who knows the topic can talk to the agente**. To make that a real trust boundary:
...
## Quick start — talk to your agente from your phone
...
3. On the host:
   ...
   ```
4. From the ntfy app, send a message to the topic. The agente's reply lands as a push notification.
...
This works even when the cron runs out-of-process from the puerta de enlace — the complemento registers a `standalone_sender_fn` that opens its own HTTP connection.
...
- **No native user identity**: see the identity-modelo section above.

## Troubleshooting

**Auth failure / 401** — `NTFY_TOKEN` is wrong, or the token doesn't have publish/subscribe rights on this topic. The adapter halts its reconnect loop on 401 and the puerta de enlace runtime status will show `fatal: ntfy_unauthorized`. Fix the token and restart the puerta de enlace.
...
**Connected but no messages** — Check that `NTFY_ALLOWED_USERS` includes the topic name itself. With ntfy's identity modelo, the topic IS the user; leaving the allowlist empty rejects everything.
