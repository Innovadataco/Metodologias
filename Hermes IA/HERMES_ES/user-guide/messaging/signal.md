<!-- source: website/docs/guia-usuario/messaging/signal.md -->
# Signal
...
Signal is the most privacy-focused mainstream messenger — end-to-end encrypted by default, open-source protocol, minimal metadata collection. This makes it ideal for security-sensitive agente workflows.
...
signal-cli link -n "HermesAgente"
```
...
**Outgoing** (agente → user):

The agente can send media files via `MEDIA:` tags in responses. The following delivery methods are supported:
...
**Reactions.** The agente can react to messages via the standard reaction API; reactions surface in Signal as emoji reactions on the referenced message rather than as extra text.
...
Signal does not support editing already-sent messages. Hermes therefore suppresses puerta de enlace herramienta-progress bubbles on Signal, even when `/verbose` is enabled and saves a non-`off` mode for the platform.

You can still see herramienta activity in the CLI, and final Signal replies can include normal assistant output. If you need live per-herramienta progress in chat, use a messaging platform with message editing support.

### Phone Number Redaction

All phone numbers are automatically redacted in logs:
- `+15551234567` → `+155****4567`
- This applies to both Hermes puerta de enlace logs and the global redaction system
...
**No extra configuración needed.** This works automatically as long as `SIGNAL_ACCOUNT` matches your phone number.
...
| **Bot responds to no one** | Configure `SIGNAL_ALLOWED_USERS`, use DM pairing, or explicitly allow all users through puerta de enlace policy if you want broader access. |
...
:::warning
**Always configure access controls.** The bot has terminal access by default. Without `SIGNAL_ALLOWED_USERS` or DM pairing, the puerta de enlace denies all incoming messages as a safety measure.
:::
