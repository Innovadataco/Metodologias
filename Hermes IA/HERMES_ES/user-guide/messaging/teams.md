<!-- source: website/docs/guia-usuario/messaging/teams.md -->
# Microsoft Teams

# Microsoft Teams Setup

Connect Hermes Agente to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).

Need meeting summaries from Microsoft Graph events rather than normal bot conversations? Use the dedicated setup page: [Teams Meetings](/guia-usuario/messaging/teams-meetings).

> Run `hermes puerta de enlace setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams cannot deliver messages to `localhost`. For local development, use any tunnel herramienta to get a public HTTPS URL. The default port is `3978` — change it with `TEAMS_PORT` if needed.
...
## Step 5: Start the Puerta de enlace

```bash
HERMES_UID=$(id -u) HERMES_GID=$(id -g) docker compose up -d puerta de enlace
```

This starts the puerta de enlace. The default webhook port is `3978` (override with `TEAMS_PORT`). Check that it's running:
...
When the agente needs to run a potentially dangerous command, it sends an Adaptive Card with four buttons instead of asking you to type `/approve`:
...
When the [Teams meeting pipeline complemento](/guia-usuario/messaging/msgraph-webhook) is enabled, this adapter also handles outbound delivery of meeting summaries — one Teams integration surface, not two. After a meeting's transcript is summarized, the writer posts the summary into your chosen Teams target.
...
platforms:
  teams:
    ...
    extra:
      ...
      # Meeting summary delivery (only used when the teams_pipeline complemento is enabled)
...
If the `teams_pipeline` complemento is **not** enabled, these settings are inert — they only wire up when the pipeline runtime binds to the Graph webhook ingress.
...
| `No inference proveedor configured` | Check that `ANTHROPIC_API_KEY` (or another proveedor key) is set in `~/.hermes/.env` |
...
- [Teams Meetings](/guia-usuario/messaging/teams-meetings)
