<!-- source: website/docs/guia-usuario/messaging/whatsapp-cloud.md -->
...
- The Hermes puerta de enlace needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
...
The wizard walks you through every credential, validates each one as you paste it (catches the #1 setup trap — pasting a phone number into the Phone Number ID field), and prints exact follow-up instructions for the parts that need to happen outside the wizard (starting cloudflared, configuring Meta's webhook panel).
...
3. **A way to expose a local port to the public internet** with HTTPS.  Cloudflare Tunnel (`cloudflared`) is recommended — free, no port forwarding, no domain required.  ngrok, your own domain with a reverse proxy + TLS, or a VPS with the puerta de enlace directly bound to a public IP all work too.
...
You'll need these values from the panel — the wizard prompts for them in this order:

| Value | Where in panel | Field shape | Notes |
|---|---|---|---|
| **Phone Number ID** | App Panel → WhatsApp → API Setup → below the "From" dropdown | Numeric, 15-17 digits | **NOT** the phone number itself. The #1 setup mistake is pasting the actual phone number here. |
| **Access Token** | App Panel → WhatsApp → API Setup → "Generate access token" | Starts with `EAA`, 100+ chars | Temp tokens last 24h — see "Permanent token" below for production. |
| **App Secret** | App Panel → Settings → Basic → click "Show" next to App secret | 32-character lowercase hex | Used to verify incoming webhook signatures.  Without it, inbound delivery is refused with 503. |
| **App ID** (optional) | App Panel → Settings → Basic | Numeric, 15-16 digits | Not required for messaging, useful for analytics. |
| **WABA ID** (optional) | App Panel → WhatsApp → API Setup → near the top | Numeric, 15+ digits | Not required for messaging, useful for analytics. |
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.hermes/.env` → restart the puerta de enlace.
...
The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Hermes puerta de enlace has to be reachable from Meta's servers.  Three common ways:

### Cloudflare Tunnel (recommended)

Free, no port forwarding, works on Windows / macOS / Linux.  Runs as a separate process alongside the puerta de enlace.
...
3. Start the Hermes puerta de enlace: `hermes puerta de enlace`.
4. In the Meta App Panel → **WhatsApp → Configuración** (or **Use cases → Customize → Configuración** depending on UI version) → click **Edit** on the Webhook section.
5. Fill in:
   - **Callback URL**: `https://abc123.trycloudflare.com/whatsapp/webhook`
   - **Verify Token**: the string from step 2 (must match exactly)
6. Click **Verify and save**.  Meta hits your URL with a GET request, the puerta de enlace echoes back the challenge, and Meta marks the webhook as verified.
...
1. App Panel → WhatsApp → API Setup → **To** dropdown.
...
In addition to Meta's recipient whitelist, Hermes has its own per-platform allowlist that controls **which incoming messages the agente processes**.  Add to `~/.hermes/.env`:
...
## Polishing your bot's WhatsApp perfil

WhatsApp displays a **name and perfil picture** for your bot in the chat header and contact list. These can't be set via the Cloud API — they live in Meta's Business Manager.
...
| **Perfil picture** | Top of the phone-number page | Square image, ≥640×640px recommended. Updates immediately. |
| **About / description / website / email / hours / category** | "Edit perfil" button | These appear in the info pane when a user taps the bot's name. Cosmetic. |
...
- **Text messages** — passed straight to the agente.
- **Images** — auto-downloaded and attached to the agente's input. Modelos with native vision (Claude, GPT-4o, Gemini, etc.) read the image directly; non-vision modelos receive an auto-generated text description.
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT proveedor (local faster-whisper, OpenAI/Nous, Groq, etc.), then handed to the agente as text.
- **Documents** — auto-downloaded. Small text-readable files (`.txt`, `.md`, `.json`, `.py`, `.csv`, etc.) up to 100KB get inlined into the agente's input so it can read them without a herramienta call. Larger files are cached locally for the agente's other herramientas to access.
...
- **Reply contexto** — when the user replies to a previous bot message, the agente sees the original message as contexto.
...
- **Images** — agente-generated images and local image files both supported, delivered as native photo attachments.
...
When the agente invokes any of these flows, Hermes uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:

- **`clarify` herramienta** — multi-choice questions render as quick-reply buttons (1–3 choices) or a tap-to-open list sheet (4+ choices). Picking "✏️ Other" lets the user type a free-form answer that the agente receives as the resolution.
- **Dangerous-command approvals** — when the agente's terminal/code execution hits a gated command, the user sees `✅ Approve` / `❌ Deny` buttons instead of needing to type `/approve` or `/deny`.
...
- Your message shows **blue double-checkmarks** as soon as the puerta de enlace receives it.
- The bot's name in your WhatsApp chat shows **"typing…"** while the agente is preparing a reply.
...
- **Without ffmpeg** — outbound TTS arrives as an MP3 audio attachment. Plays fine, just doesn't look like a voice note. A one-time warning fires in the puerta de enlace log so you know.

You can check whether the puerta de enlace found ffmpeg via the health endpoint:
...
Hermes warns the agente about this window in its system prompt, so the modelo knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Hermes. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agente/issues) — it's planned but waiting on a clear demand signal.
...
### Setup verification fails ("URL couldn't be validated") in Meta panel

Almost always one of:

- **Tunnel URL is wrong or stale** — cloudflared quick tunnels rotate.  Get a fresh URL and update both `.env` and Meta's panel.
- **Verify token mismatch** — the token in `~/.hermes/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's panel.  Run the curl probe above to confirm the puerta de enlace's verify handshake works locally first.
- **Puerta de enlace not running** — check `hermes puerta de enlace` is up.
...
### `graph error 190`: Autenticación Error
...
### Bot replies appear as raw JSON / herramienta-call leakage

Common cause: the herramientaset configured for `whatsapp_cloud` is missing the herramientas the agente wants to call.  Check `hermes herramientas list` and verify the platform is using `hermes-whatsapp` (the default Cloud adapter herramientaset, same as Baileys).

If the modelo emits herramienta-call-shaped text instead of a structured call, it usually means the herramientaset was effectively empty.  See `hermes_cli/platforms.py` for the platform → default herramientaset mapping.

### STT (voice note transcription) returns empty / "could not transcribe"

The default `stt.proveedor: local` requires `pip install faster-whisper`.  If you're a Nous subscriber, you can route STT through Meta's managed audio puerta de enlace instead:

```bash
hermes config set stt.proveedor openai
hermes config set stt.use_puerta de enlace true
...
- **The webhook endpoint accepts only signed requests when `WHATSAPP_CLOUD_APP_SECRET` is set** — leave it set even in development.  Without it, the puerta de enlace refuses inbound delivery with HTTP 503.
