<!-- source: website/docs/guia-usuario/messaging/feishu.md -->
# Feishu / Lark

# Feishu / Lark Setup

Hermes Agente integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agente in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal puerta de enlace flow.
...
- `webhook` — useful when you want Feishu/Lark to push events into your puerta de enlace over HTTP
...
5. Run `hermes puerta de enlace setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
2. In the **Event Configuración** section, subscribe to:
...
## Step 4: Start the Puerta de enlace
...
This key is found in the **Event Subscriptions** section of your Feishu app configuración. When set, the adapter verifies every webhook request using the signature algorithm:
...
An additional layer of autenticación that checks the `token` field inside webhook payloads:
...
Puerta de enlace-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `hermes update --puerta de enlace` needs confirmation, the adapter records the selected answer in Hermes's `.update_response` file and replaces the card inline with a resolved state.

Card action events are dispatched with `MessageType.COMMAND`, so they flow through the normal command processing pipeline.

This is also how **command approval** works — when the agente needs to run a dangerous command, it sends an interactive card with Allow Once / Session / Always / Deny buttons. The user clicks a button, and the card action callback delivers the approval decision back to the agente.

### Required Feishu App Configuración

Interactive cards require **three** configuración steps in the Feishu Developer Console. Missing any of them causes error **200340** when users click card buttons.
...
- Runs the agente with the `feishu_doc` + `feishu_drive` herramientasets scoped to that single comment session.
- Chunks replies at 4000 chars and posts them back as threaded replies.
- Caches per-document sessions for 1 hour with a 50-message cap so follow-up comments on the same doc keep contexto.
...
Rules live in `~/.hermes/feishu_comment_rules.json` (pairing grants in `~/.hermes/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the puerta de enlace.

CLI:

```bash
# Inspect current rules and pairing state
python -m puerta de enlace.platforms.feishu_comment_rules status

# Simulate an access check for a specific doc + user
python -m puerta de enlace.platforms.feishu_comment_rules check <fileType:fileToken> <user_open_id>

# Manage pairing grants at runtime
python -m puerta de enlace.platforms.feishu_comment_rules pairing list
python -m puerta de enlace.platforms.feishu_comment_rules pairing add <user_open_id>
python -m puerta de enlace.platforms.feishu_comment_rules pairing remove <user_open_id>
```
...
You can invite the Hermes Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, Hermes can automatically start an agente turn that attempts to join the meeting.
...
- If the inviter is authorized by the normal puerta de enlace allowlist or pairing policy, the agente receives the meeting number and tries to join automatically.
- If the invite is malformed, or the agente cannot join, Hermes drops the event or replies to the inviter with a concise explanation.
...
- Ensure the puerta de enlace user allowlist or pairing policy authorizes the inviter. Meeting invitations do not bypass normal puerta de enlace access checks.
...
For small text-based documents (.txt, .md), the file content is automatically injected into the message text so the agente can read it directly without needing herramientas.
...
While the agente is working, the bot shows a `Typing` reaction on your message. It's cleared when the reply arrives, or replaced with `CrossMark` if processing failed.

Set `FEISHU_REACTIONS=false` to turn it off.

## Burst Protection and Batching

The adapter includes debouncing for rapid message bursts to avoid overwhelming the agente:
...
- **Tracking cap:** Up to 4096 unique keys tracked (prevents unbounded memoria growth)
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `hermes puerta de enlace setup` |
| `Another local Hermes puerta de enlace is already using this Feishu app_id` | Only one Hermes instance can use the same app_id at a time. Stop the other puerta de enlace first. |
...
| Error 200340 when clicking approval buttons | Enable **Interactive Card** capability and configure **Card Request URL** in the Feishu Developer Console. See [Required Feishu App Configuración](#required-feishu-app-configuración) above. |
| `Webhook rate limit exceeded` | More than 120 requests/minute from the same IP. This is usually a misconfiguración or loop. |

## Herramientaset

Feishu / Lark uses the `hermes-feishu` platform preset, which includes the same core herramientas as Telegram and other puerta de enlace-based messaging platforms.
