<!-- source: website/docs/guia-usuario/messaging/webhooks.md -->
# Webhooks

# Webhooks

Receive events from external services (GitHub, GitLab, JIRA, Stripe, etc.) and trigger Hermes agente runs automatically. The webhook adapter runs an HTTP server that accepts POST requests, validates HMAC signatures, transforms payloads into agente prompts, and routes responses back to the source or to another configured platform.

The agente processes the event and can respond by posting comments on PRs, sending messages to Telegram/Discord, or logging the result.
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Hermes Agente — Webhooks Tutorial"
...
1. Enable via `hermes puerta de enlace setup` or environment variables
...
Once the puerta de enlace is running:
...
| `habilidads` | No | List of habilidad names to load for the agente run. |
| `deliver` | No | Where to send the response: `github_comment`, `telegram`, `discord`, `slack`, `signal`, `sms`, `whatsapp`, `matrix`, `mattermost`, `homeassistant`, `email`, `dingtalk`, `feishu`, `wecom`, `weixin`, `bluebubbles`, `qqbot`, or `log` (default). |
| `deliver_extra` | No | Additional delivery config — keys depend on `deliver` type (e.g. `repo`, `pr_number`, `chat_id`). Values support the same `{dot.notation}` templates as `prompt`. |
| `deliver_only` | No | If `true`, skip the agente entirely — the rendered `prompt` template becomes the literal message that gets delivered. Zero LLM cost, sub-second delivery. See [Direct Delivery Mode](#direct-delivery-mode) for use cases. Requires `deliver` to be a real target (not `log`). |
...
- `{__raw__}` — special token that dumps the **entire payload** as indented JSON (truncated at 4000 characters). Useful for monitoring alerts or generic webhooks where the agente needs the full contexto.
...
GitLab webhooks work similarly but use a different autenticación mechanism. GitLab sends the secret as a plain `X-Gitlab-Token` header (exact string match, not HMAC).
...
The `deliver` field controls where the agente's response goes after processing the webhook event.

| Deliver Type | Description |
|-------------|-------------|
| `log` | Logs the response to the puerta de enlace log output. This is the default and is useful for testing. |
| `github_comment` | Posts the response as a PR/issue comment via the `gh` CLI. Requires `deliver_extra.repo` and `deliver_extra.pr_number`. The `gh` CLI must be installed and authenticated on the puerta de enlace host (`gh auth login`). |
...
For cross-platform delivery, the target platform must also be enabled and connected in the puerta de enlace. If no `chat_id` is provided in `deliver_extra`, the response is sent to that platform's configured home channel.

---

## Direct Delivery Mode {#direct-delivery-mode}

By default, every webhook POST triggers an agente run — the payload becomes a prompt, the agente processes it, and the agente's response is delivered. This costs LLM tokens on every event.

For use cases where you just want to **push a plain notification** — no reasoning, no agente loop, just deliver the message — set `deliver_only: true` on the route. The rendered `prompt` template becomes the literal message body, and the adapter dispatches it directly to the configured delivery target.
...
- **Inter-agente pings** — Agente A notifies Agente B's user that a long-running task finished
- **Background job completion** — Cron job finishes → post result to Slack

Benefits:

- **Zero LLM tokens** — the agente is never invoked
- **Sub-second delivery** — a single adapter call, no reasoning loop
- **Same security as agente mode** — HMAC auth, rate limits, idempotency, and body-size limits all still apply
...
| `502 Bad Puerta de enlace` | Target adapter rejected the message or raised. The error is logged server-side; the response body is a generic `Delivery failed` to avoid leaking adapter internals. |

### Configuración gotchas
...
- The `habilidads` field is ignored in direct delivery mode (no agente runs, so there's nothing to inject habilidads into).
- Template rendering uses the same `{dot.notation}` syntax as agente mode, including the `{__raw__}` token.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `hermes webhook` CLI command. This is especially useful when the agente itself needs to set up event-driven triggers.
...
- Dynamic subscriptions use the same route format and capabilities as static routes (events, prompt templates, habilidads, delivery)
- No puerta de enlace restart required — subscribe and it's immediately live

### Agente-driven subscriptions

The agente can create subscriptions via the terminal herramienta when guided by the `webhook-subscriptions` habilidad. Ask the agente to "set up a webhook for GitHub issues" and it will run the appropriate `hermes webhook subscribe` command.
...
`INSECURE_NO_AUTH` is only accepted when the puerta de enlace is bound to a loopback host (`127.0.0.1`, `localhost`, `::1`). If it is combined with a non-loopback bind such as `0.0.0.0` or a LAN IP, the adapter refuses to start — this prevents accidentally exposing an unauthenticated endpoint on a public interface.
...
Delivery IDs (from `X-GitHub-Delivery`, `X-Request-ID`, or a timestamp fallback) are cached for **1 hour**. Duplicate deliveries (e.g. webhook retries) are silently skipped with a `200` response, preventing duplicate agente runs.
...
:::warning
Webhook payloads contain attacker-controlled data — PR titles, commit messages, issue descriptions, etc. can all contain malicious instructions. Run the puerta de enlace in a sandboxed environment (Docker, VM) when exposed to the internet. Consider using the Docker or SSH terminal backend for isolation.
:::
...
- Check puerta de enlace logs for `Invalid signature` warnings
...
### Agente not responding

- Run the puerta de enlace in foreground to see logs: `hermes puerta de enlace run`
...
- Run `gh auth login` on the puerta de enlace host
