<!-- source: website/docs/guia-usuario/messaging/telegram.md -->
# Telegram

# Telegram Setup

Hermes Agente integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agente from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agente in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.

## Step 1: Create a Bot via BotFather

Every Telegram bot requires an API token issued by [@BotFather](https://t.me/BotFather), Telegram's official bot management herramienta.

1. Open Telegram and search for **@BotFather**, or visit [t.me/BotFather](https://t.me/BotFather)
2. Send `/newbot`
3. Choose a **display name** (e.g., "Hermes Agente") — this can be anything
...
| `/setabouttext` | Short text on the bot's perfil page |
...
when the puerta de enlace connects and **Offline** on a clean shutdown:
...
- The short description is **global** to the bot (visible to all users), not
  per-chat. Users see it on the bot's perfil page, not as a live badge inside
  an open chat.
- Only a **clean** puerta de enlace shutdown (`/stop`, `disconnect`) writes "Offline".
  A hard crash leaves the last-known status — the inherent limitation of a
  perfil-text indicator.
- Off by default, since it mutates the bot's global perfil.

### Command menu priority and cap (Optional)

Hermes registers its command menu automatically when the Telegram puerta de enlace starts. The menu is built from the central slash-command registry plus eligible complemento/habilidad commands, then capped so Telegram accepts the payload reliably. The default cap is 60 commands — enough to keep all built-in commands plus common habilidad commands visible.

If you have local or complemento commands that should stay visible in Telegram's `/` picker, prioritize them in `~/.hermes/config.yaml`:
...
With this mode enabled, unmentioned group messages from explicitly allowlisted chats/topics are appended to the shared chat/topic session transcript as observed contexto, but they do not dispatch the agente. `allowed_chats` gates where the bot responds; `group_allowed_chats` authorizes the shared group session used for observed contexto, so use the same chat IDs for this mode. A later `@botname` mention, reply to the bot, or configured mention pattern in that same allowlisted chat/topic can use that observed contexto. The triggered message is also tagged with `[nickname|user_id]` and gets a per-turn safety prompt so the modelo treats prior observed lines as contexto, not instructions addressed to the bot.
...
This requires Telegram to deliver ordinary group messages to the puerta de enlace, so disable BotFather privacy mode or promote the bot to group admin as described above.

## Step 4: Find Your User ID

Hermes Agente uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
Select **Telegram** when prompted. The wizard asks for your bot token and allowed user IDs, then writes the configuración for you.
...
sent by the **puerta de enlace process**, not from inside the container. That means the
final `MEDIA:/...` path must be readable on the host where the puerta de enlace is
running.

Common pitfall:

- the agente writes a file inside Docker to `/workspace/report.txt`
- the modelo emits `MEDIA:/workspace/report.txt`
...
The puerta de enlace extracts `MEDIA:/path/to/file` tags from agente replies and ships the referenced file as a platform-native attachment. Supported extensions across all puerta de enlace platforms:
...
|---|---|---|
| Direction | Puerta de enlace → Telegram (outbound) | Telegram → Puerta de enlace (inbound) |
...
| `TELEGRAM_WEBHOOK_SECRET` | **Yes** (when `TELEGRAM_WEBHOOK_URL` is set) | Secret token that Telegram echoes in every webhook request for verification. The puerta de enlace refuses to start without it — see [GHSA-3vpc-7q5r-276h](https://github.com/NousResearch/hermes-agente/security/advisories/GHSA-3vpc-7q5r-276h). Generate with `openssl rand -hex 32`. |
| `TELEGRAM_WEBHOOK_PORT` | No | Local port the webhook server listens on (default: `8443`). |

When `TELEGRAM_WEBHOOK_URL` is set, the puerta de enlace starts an HTTP webhook server instead of polling. When unset, polling mode is used — no behavior change from previous versions.
...
The puerta de enlace log should show: `[telegram] Connected to Telegram (webhook mode)`.
...
The proxy applies to both the main Telegram connection and the fallback IP transport. If no Telegram-specific proxy is set, the puerta de enlace falls back to `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY` (or macOS system proxy auto-detection).
...
Voice messages you send on Telegram are automatically transcribed by Hermes's configured STT proveedor and injected as text into the conversation.
...
#### Skipping STT: pass the raw audio file to the agente

If you'd rather have the **agente itself** handle audio — for diarization, a custom transcription herramienta, or just archiving the recording — set `stt.enabled: false` in `~/.hermes/config.yaml`:
...
With STT disabled, the puerta de enlace still downloads the voice/audio attachment into Hermes's audio cache, but **does not transcribe it**. The agente receives the message with a marker like:

```
[The user sent a voice message: /home/<user>/.hermes/cache/audio/<hash>.ogg]
```

Your herramientas or habilidads can then read that path directly (e.g., hand it off to a local diarization pipeline, a richer transcription modelo, or upload it to long-term storage). The file extension reflects the original format Telegram delivered (`.ogg` for voice notes, `.mp3`/`.m4a`/etc. for audio attachments).
...
When the agente generates audio via TTS, it's delivered as native Telegram **voice bubbles** — the round, inline-playable kind.

- **OpenAI and ElevenLabs** produce Opus natively — no extra setup needed
- **Edge TTS** (the default free proveedor) outputs MP3 and requires **ffmpeg** to convert to Opus:
...
Configure the TTS proveedor in your `config.yaml` under the `tts.proveedor` key.
...
- Uploading large videos for vision-herramienta processing
...
Restart the puerta de enlace and look for a confirmation log line:

```bash
hermes puerta de enlace restart
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.hermes/logs/puerta de enlace.log | tail
```
...
If `local_mode: true` is set but Hermes can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server — which in `--local` mode responds with `404 Not Found`. The symptom shows up in `puerta de enlace.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the Hermes host as the user the puerta de enlace runs as, and confirm a single file is `cat`-able without a permission error.

### Step 6: Test it

Send the bot a voice note or audio file that's bigger than 20 MB. Tail the puerta de enlace log:

```bash
tail -f ~/.hermes/logs/puerta de enlace.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.hermes/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agente's inbound message for downstream processing.

## Group Chat Usage

Hermes Agente works in Telegram group chats with a few considerations:
...
- In groups with multiple Hermes bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot perfils process it; other Hermes bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
...
If you run several Hermes perfils in the same Telegram group, create one Telegram bot token per perfil and start one puerta de enlace per perfil. Do not reuse the same bot token in multiple running puerta de enlaces; Telegram will reject concurrent polling for the same token.
...
To operate several perfils, run the puerta de enlace command once per perfil. For example:
...
# named perfils
hermes -p research puerta de enlace start
hermes -p research puerta de enlace status
hermes -p research puerta de enlace stop
```

For a small fixed fleet, use a shell loop or script that calls `hermes puerta de enlace <action>` for the default perfil and `hermes -p <perfil> puerta de enlace <action>` for each named perfil. This is more reliable than assuming a single process-level command controls every named perfil on every service manager.
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   Hermes perfil uses a unique bot token and keep `exclusive_bot_mentions`
...
### Example group trigger configuración
...
- Invalid regex patterns are ignored with a warning in the puerta de enlace logs rather than crashing the bot
...
If you work on several long-running projects, topics keep their contexto separate:
...
Each topic gets its own conversation session, history, and contexto — completely isolated from the others.
  ...
  telegram:
    extra:
      ...
      - chat_id: 123456789        # Your Telegram user ID
        ...
        - name: Research
          icon_color: 16766590
          habilidad: arxiv              # Auto-load a habilidad in this topic
```
...
| `habilidad` | No | Habilidad to auto-load on new sessions in this topic |
| `thread_id` | No | Auto-populated after topic creation — don't set manually |

### How it works

1. On puerta de enlace startup, Hermes calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
2. The `thread_id` is saved back to `config.yaml` automatically — subsequent restarts skip the API call
3. Each topic maps to an isolated session key: `agente:main:telegram:dm:{chat_id}:{thread_id}`
4. Messages in each topic have their own conversation history, memoria flush, and contexto window
...
Topics with a `habilidad` field automatically load that habilidad when a new session starts in the topic. This works exactly like typing `/habilidad-name` at the start of a conversation — the habilidad content is injected into the first message, and subsequent messages see it in the conversation history.

For example, a topic with `habilidad: arxiv` will have the arxiv habilidad pre-loaded whenever its session resets (due to idle timeout, daily reset, or manual `/reset`).

:::tip
Topics created outside of the config (e.g., by manually calling the Telegram API) are discovered automatically when a `forum_topic_created` service message arrives. You can also add topics to the config while the puerta de enlace is running — they'll be picked up on the next cache miss.
:::
...
| Primary use case | Permanent workspaces with optional habilidad binding | Ad-hoc parallel sessions |
...
Every topic gets its own conversation history, modelo state, herramienta execution, and session ID. The isolation key is `agente:main:telegram:dm:{chat_id}:{thread_id}` — identical to the config-driven DM topics isolation.
...
Hermes confirms with the session title and replays the last assistant message for contexto.
...
- The topic-mode SQLite migration is **opt-in**: it runs on the first `/topic` call, never on puerta de enlace startup. Until a user runs `/topic` in this perfil, `state.db` is unchanged
...
If you downgrade to a Hermes version that predates `/topic`, the feature simply stops working — the `telegram_dm_topic_mode` and `telegram_dm_topic_bindings` tables remain in `state.db` but are ignored by older code. DMs revert to the native per-thread isolation (each `message_thread_id` still gets its own session via `build_session_key`), so your existing Telegram topics keep working as parallel sessions. The root DM is no longer a lobby — messages there go into the agente like they used to. Re-upgrading reactivates multi-session mode exactly where it was.

## Group Forum Topic Habilidad Binding

Supergroups with **Topics mode** enabled (also called "forum topics") already get session isolation per topic — each `thread_id` maps to its own conversation. But you may want to **auto-load a habilidad** when messages arrive in a specific group topic, just like DM topic habilidad binding works.

### Use case

A team supergroup with forum topics for different workstreams:

- **Engineering** topic → auto-loads the `software-development` habilidad
- **Research** topic → auto-loads the `arxiv` habilidad
- **General** topic → no habilidad, general-purpose assistant
  ...
  telegram:
    extra:
      ...
      - chat_id: -1001234567890       # Supergroup ID
        ...
        - name: Engineering
          thread_id: 5
          habilidad: software-development
        - name: Research
          thread_id: 12
          habilidad: arxiv
        - name: General
          thread_id: 1
          # No habilidad — general purpose
```
...
2. If a matching entry has a `habilidad` field, that habilidad is auto-loaded for the session — identical to DM topic habilidad binding
3. Topics without a `habilidad` key get session isolation only (existing behavior, unchanged)
4. Unmapped `thread_id` values or `chat_id` values fall through silently — no error, no habilidad
...
### Streaming transport (`puerta de enlace.streaming.transport`)

When streaming is enabled (`puerta de enlace.streaming.enabled: true`), Hermes picks one of four transports:
...
**What you'll see in DMs with `edit` (default)** — the puerta de enlace sends a normal preview message and progressively updates it via `editMessageText`, avoiding Telegram's draft-preview collapse/rollback effect.
...
**What about groups, supergroups, forum topics?** Telegram restricts `sendMessageDraft` to private chats (DMs). The puerta de enlace transparently falls back to the edit-based path for everything else — same UX as before.
...
**Link previews.** Telegram auto-generates link previews for URLs in bot messages. If you'd rather suppress those (long `/herramientas` output, agente reply that mentions ten links, etc.):
  ...
  platforms:
    telegram:
      extra:
        ...
        # NEW — admins get all slash commands (built-in + complemento)
        allow_admin_from:
          - "123456789"
...
- A user listed in `allow_admin_from` for a scope (DM or group) can run **every** registered slash command — built-in commands AND complemento-registered ones — through the live registry.
...
- Plain chat (non-slash messages) is unaffected. Non-admin users can still talk to the agente normally, they just can't trigger arbitrary commands.
...
## Interactive Modelo Picker

When you send `/modelo` with no arguments in a Telegram chat, Hermes shows an interactive inline keyboard for switching modelos:

1. **Proveedor selection** — buttons showing each available proveedor with modelo counts (e.g., "OpenAI (15)", "✓ Anthropic (12)" for the current proveedor).
2. **Modelo selection** — paginated modelo list with **Prev**/**Next** navigation, a **Back** button to return to proveedors, and **Cancel**.

The current modelo and proveedor are displayed at the top. All navigation happens by editing the same message in-place (no chat clutter).

:::tip
If you know the exact modelo name, type `/modelo <name>` directly to skip the picker. You can also type `/modelo <name> --global` to persist the change across sessions.
:::
...
Set the proxy in your environment before starting the puerta de enlace:
...
The proxy applies to both the primary transport and all fallback IP transports. No additional Hermes configuración is needed — if the environment variable is set, it's used automatically.
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `hermes puerta de enlace` logs for errors. |
...
When the agente tries to run a potentially dangerous command, it asks you for approval in the chat:
...
When the agente calls the `clarify` herramienta — to ask which approach you prefer, get post-task feedback, or check before a non-trivial decision — Telegram renders the question with **inline keyboard buttons**:

> ❓ Which framework should I use for the panel?
>
...
Configure the response timeout via `agente.clarify_timeout` in `~/.hermes/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agente unblocks with a sentinel message and adapts rather than hanging.

## Push notification volume

Telegram fires a push notification on every message the bot sends. For long agente turns that emit herramienta-progress bubbles, streaming updates, and status callbacks, this gets noisy fast. The Telegram adapter has two notification modes:

| Mode | Behavior |
|------|----------|
| `important` (default) | Only **final responses**, **approval prompts**, and **slash-command confirmations** ring. Herramienta progress, streaming chunks, and status messages are delivered with `disable_notification=true`. |
| `all` | Every outgoing message fires a push notification. Legacy behavior; opt in if you genuinely want to hear about every herramienta call. |
...
The Telegram adapter routes recurring agente status callbacks (e.g. "Compressing contexto…", "Calling herramienta…") through `send_or_update_status()`, which keeps a `{(chat_id, status_key) → message_id}` cache and **edits the existing bubble** on subsequent emits instead of appending a new one each time. Distinct `status_key` values get their own messages; distinct chats never collide. If the edit fails (e.g. the user deleted the message, or it's older than Telegram allows for edits), the cache entry is dropped and the next emit posts a fresh message and re-caches its ID. No config required — this is the default Telegram behavior. Other adapters that don't implement `send_or_update_status` fall through to plain `send()` unchanged.

## Pin incoming user message during agente turn

When a user sends a message that triggers an agente turn, the Telegram adapter pins that incoming message for the duration of the turn and unpins it when the response is finished — a lightweight visual indicator that the bot is actively working on the message rather than ignoring it. The pin uses `disable_notification=true` to avoid extra pings. No config required.

## Security

:::warning
Always set `TELEGRAM_ALLOWED_USERS` to restrict who can interact with your bot. Without it, the puerta de enlace denies all users by default as a safety measure.
:::

Never share your bot token publicly. If compromised, revoke it immediately via BotFather's `/revoke` command.

For more details, see the [Security documentation](/guia-usuario/security). You can also use [DM pairing](/guia-usuario/messaging#dm-pairing-alternative-to-allowlists) for a more dynamic approach to user authorization.
