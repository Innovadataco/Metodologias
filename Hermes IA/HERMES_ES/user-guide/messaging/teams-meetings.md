<!-- source: website/docs/guia-usuario/messaging/teams-meetings.md -->
# Teams Meetings
...
> Run `hermes puerta de enlace setup` and pick **Teams Meetings** for a guided walk-through.

This page focuses on setup and enablement:
- Graph credentials
- webhook listener configuración
...
Operator actions stay in the CLI (the `teams-pipeline` subcommand is registered by the `teams_pipeline` complemento — enable it via `hermes complementos enable teams_pipeline` or set `complementos.enabled: [teams_pipeline]` in `config.yaml`):
...
- the existing [Microsoft Teams bot setup](/guia-usuario/messaging/teams) if you want Teams outbound delivery
...
The webhook listener is a puerta de enlace platform named `msgraph_webhook`. At minimum, enable it and set a client state value:
...
The pipeline supports two Teams summary-delivery modes inside the existing Teams complemento.
...
Or, if you run Hermes in Docker, start the puerta de enlace the same way you already do for your deployment.
...
Use the complemento CLI to create and inspect subscriptions.
...
- [Microsoft Teams bot setup](/guia-usuario/messaging/teams)
