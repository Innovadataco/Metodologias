<!-- source: website/docs/guia-usuario/messaging/whatsapp.md -->
# WhatsApp
...
> Run `hermes puerta de enlace setup` and pick **WhatsApp** for a guided walk-through.
...
| **Personal self-chat** | Use your own WhatsApp. You message yourself to talk to the agente. | Quick setup, single user, testing |
...
[DM pairing system](/guia-usuario/security#dm-pairing-system).
:::
...
The puerta de enlace starts the WhatsApp bridge automatically using the saved session.
...
errors in the puerta de enlace logs. To fix it:

```bash
hermes whatsapp
```

This generates a fresh QR code. Scan it again and the session is re-established. The puerta de enlace
...
- **Incoming:** Voice messages (`.ogg` opus) are automatically transcribed using the configured STT proveedor: local `faster-whisper`, Groq Whisper (`GROQ_API_KEY`), or OpenAI Whisper (`VOICE_TOOLS_OPENAI_KEY`)
- **Outgoing:** TTS responses are sent as MP3 audio file attachments
- Agente responses are prefixed with "⚕ **Hermes Agente**" by default. You can customize or disable this in `config.yaml`:
...
Long responses are automatically split into multiple messages at **4,096 characters** per chunk (WhatsApp's practical display limit). You don't need to configure anything — the puerta de enlace handles splitting and sends chunks sequentially.
...
| **Bridge crashes or reconnect loops** | Restart the puerta de enlace, update Hermes, and re-pair if the session was invalidated by a WhatsApp protocol change. |
| **Bot stops working after WhatsApp update** | Update Hermes to get the latest bridge version, then re-pair. |
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `hermes puerta de enlace install` to re-snapshot your current PATH into the plist, then `hermes puerta de enlace start`. See the [Puerta de enlace Service docs](./index.md#macos-launchd) for details. |
| **Messages not being received** | Verify `WHATSAPP_ALLOWED_USERS` includes the sender's number (with country code, no `+` or spaces), or set it to `*` to allow everyone. Set `WHATSAPP_DEBUG=true` in `.env` and restart the puerta de enlace to see raw message events in `bridge.log`. |
...
`WHATSAPP_ALLOW_ALL_USERS=true`. Without any of these, the puerta de enlace **denies all incoming
