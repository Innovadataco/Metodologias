<!-- source: website/docs/guia-usuario/habilidads/google-workspace.md -->
# Google Workspace — Gmail, Calendar, Drive, Sheets & Docs

# Google Workspace Habilidad
...
**Habilidad path:** `habilidads/productivity/google-workspace/`

## Setup

The setup is fully agente-driven — ask Hermes to set up Google Workspace and it walks you through each step. The flow:
...
If you only need email (no Calendar/Drive/Sheets), use the **himalaya** habilidad instead — it works with a Gmail App Password and takes 2 minutes. No Google Cloud project needed.
:::
...
$GAPI gmail send --to user@example.com --subject "Hello" \
  --from '"Research Agente" <user@example.com>' --body "Message text"
...
The `--from` flag lets you customize the sender display name on outgoing emails. This is useful when multiple agentes share the same Gmail account but you want recipients to see different names:

```bash
# Agente 1
$GAPI gmail send --to client@co.com --subject "Research Summary" \
  --from '"Research Agente" <shared@company.com>' --body "..."
...
**How it works:** The `--from` value is set as the RFC 5322 `From` header on the MIME message. Gmail allows customizing the display name on your own authenticated email address without any additional configuración. Recipients see the custom display name (e.g. "Research Agente") while the email address stays the same.
