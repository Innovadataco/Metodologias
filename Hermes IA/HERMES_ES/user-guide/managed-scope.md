<!-- source: website/docs/guia-usuario/managed-scope.md -->
# Managed Scope

# Managed Scope

**Managed scope** lets an administrator push a baseline of configuración and
secrets that a standard (non-root) user **cannot override**. It is intended for
fleet/org deployments where IT needs to pin, for example, the modelo proveedor, a
...
means "no managed scope," and configuración resolves exactly as it does without
the feature.
...
Merging is **leaf-level**: pinning `modelo.default` does not freeze the rest of
`modelo.*`. A managed `config.yaml` of:

```yaml
modelo:
  default: org/standard-modelo
```

forces `modelo.default` for every user while leaving `modelo.fallback` (and every
...
```bash
$ hermes config set modelo.default my/modelo
Cannot set 'modelo.default': it is managed by your administrator
...
## Security modelo and limitations (v1)
...
- **The agente's own herramientas are not hard-blocked from a managed *env* value.** A
  managed environment variable is applied at startup, but nothing stops the
  agente from setting a different value inside its own subprocess shell. v1 is a
...
- A hard boundary that the agente itself cannot escape.
