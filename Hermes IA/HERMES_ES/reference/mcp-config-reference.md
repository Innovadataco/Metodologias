...
mcp_servers:
  <server_name>:
    ...
    timeout: 120
    connect_timeout: 60
    supports_parallel_herramienta_calls: false
    herramientas:
      include: []
      exclude: []
      resources: true
...
| `timeout` | number | both | Herramienta call timeout in seconds (default: `300`) |
| `connect_timeout` | number | both | Initial connection timeout in seconds (default: `60`) |
| `supports_parallel_herramienta_calls` | bool | both | Allow herramientas from this server to run concurrently |
| `herramientas` | mapping | both | Filtering and utility-herramienta policy |
| `auth` | string | HTTP | Autenticación method. Set to `oauth` to enable OAuth 2.1 with PKCE |
| `sampling` | mapping | both | Server-initiated LLM request policy (see MCP guide) |

## `herramientas` policy keys

| Key | Type | Meaning |
|---|---|---|
| `include` | string or list | Whitelist server-native MCP herramientas |
| `exclude` | string or list | Blacklist server-native MCP herramientas |
...
If `include` is set, only those server-native MCP herramientas are registered.
...
If `exclude` is set and `include` is not, every server-native MCP herramienta except those names is registered.
...
Even when `resources: true` or `prompts: true`, Hermes only registers those utility herramientas if the MCP session actually exposes the corresponding capability.
...
- no discovery
- no herramienta registration
- config remains in place for later reuse

## Empty result behavior

If filtering removes all server-native herramientas and no utility herramientas are registered, Hermes does not create an empty MCP runtime herramientaset for that server.
...
Server-native MCP herramientas become:

```text
mcp_<server>_<herramienta>
```
...
Utility herramientas follow the same prefixing pattern:
...
Hyphens (`-`) and dots (`.`) in both server names and herramienta names are replaced with underscores before registration. This ensures herramienta names are valid identifiers for LLM function-calling APIs.

For example, a server named `my-api` exposing a herramienta called `list-items.v2` becomes:

```text
mcp_my_api_list_items_v2
```

Keep this in mind when writing `include` / `exclude` filters — use the **original** MCP herramienta name (with hyphens/dots), not the sanitized version.

## OAuth 2.1 autenticación
