<!-- source: website/docs/inicio-rapido/termux.md -->
# Android / Termux

# Hermes on Android with Termux

This is the tested path for running Hermes Agente directly on an Android phone through [Termux](https://termux.dev/).
...
- Telegram puerta de enlace support (manual / best-effort background runs)
- MCP support
- Honcho memoria support
- ACP support
...
- Android may still suspend Termux background jobs, so puerta de enlace persistence is best-effort rather than a normal managed service

That does not stop Hermes from working well as a phone-native CLI agente — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
python -m pip install --upgrade pip setupherramientas wheel
```
...
If you only want the minimal core agente, this also works:
...
The tested Termux path skips Node/browser bootstrap on purpose. If you want to experiment with browser herramientaing later:
...
The browser herramienta automatically includes Termux directories (`/data/data/com.termux/files/usr/bin`) in its PATH search, so `agente-browser` and `npx` are discovered without any extra PATH configuración.

Treat browser / WhatsApp herramientaing on Android as experimental until documented otherwise.
...
Make sure the build herramientachain is installed:
